...
#include <mach/gpio.h>
int rate_tbl[8] = {0,48000,96000,192000,0,44100,88200,176400};
#endif
...
#ifdef CONFIG_SIGNAGE //[
	{RT5640_OUTPUT  , 0x8B8B},//OUTMIX -> OUTVOL
	{RT5640_MONO_MIXER , 0xC800},//OUTMIX-> MONOMIX
//	{RT5640_DRC_AGC_1 , 0x6206},//enable DRC to DAC
...
static const DECLARE_TLV_DB_SCALE(outmix_vol_tlv, 0, 300, 0);
static const DECLARE_TLV_DB_SCALE(outvol_vol_tlv, -1200, 150, 0);
static const DECLARE_TLV_DB_SCALE(mono_vol_tlv, 0, 600, 0);
...
static const char *rt5640_clsd_spk_ratio[] = {"1.94x", "2.00x", "2.11x", "2.22x",
	"2.33x", "2.44x", "2.55x", "2.66x", "2.77x", "3.00x", "3.30x"};
...
static const struct snd_kcontrol_new rt5640_snd_controls[] = {
	...
	SOC_SINGLE_TLV("OUTMIXL DACL1 GAIN", RT5640_OUT_L2_MIXER,RT5640_G_DAC_L1_OM_L_SFT,	6, 0, outmix_vol_tlv),	
	SOC_SINGLE_TLV("OUTMIXL DACR1 GAIN", RT5640_OUT_R2_MIXER,RT5640_G_DAC_R1_OM_R_SFT,	6, 0, outmix_vol_tlv),
	SOC_DOUBLE_TLV("OUTVOL GAIN", RT5640_OUTPUT,RT5640_L_VOL_SFT, RT5640_R_VOL_SFT,39, 0, outvol_vol_tlv),
	SOC_SINGLE_TLV("MONO GAIN", RT5640_MONO_MIXER,RT5640_G_MONOMIX_SFT,	1, 0, mono_vol_tlv),	
...
#define MX6SL_SD_BOOST		IMX_GPIO_NR(3, 24)	/* AMP SD_BOOST */
#define MX6SL_AMPSD			IMX_GPIO_NR(3, 30)	/* AMPSD 5V */
#define MX6SL_AD_LDO1_EN	IMX_GPIO_NR(4, 2)	/* AD_LDO1_EN */
#define MX6SL_AD_5V_EN		IMX_GPIO_NR(3, 27)
#define MX6SL_AD_3V3_EN		IMX_GPIO_NR(3, 29)
#define MX6SL_AD_1V8_EN		IMX_GPIO_NR(3, 31)
#define MX6SL_SPK_VDD		IMX_GPIO_NR(2,6)


static void lm45811_amp_power(int on)
{	
	if(on)
	{
		gpio_direction_output(MX6SL_AMPSD, on);
		gpio_direction_output(MX6SL_SD_BOOST, on);
	}
	else
	{
		gpio_direction_output(MX6SL_SD_BOOST, on);
...
static void rt5640_power_sequence(int on)
{
	if(on)
	{
		gpio_direction_output (MX6SL_AD_5V_EN, 1);
		msleep(1);
		gpio_direction_output (MX6SL_AD_1V8_EN,1);
		msleep(1);
		gpio_direction_output (MX6SL_AD_3V3_EN,1);
		msleep(1);
		gpio_direction_output (MX6SL_SPK_VDD,1);
		msleep(1);
		gpio_direction_output (MX6SL_AD_LDO1_EN,1);
		msleep(400);
	}
	else
	{
		msleep(200);
		gpio_direction_output (MX6SL_AD_LDO1_EN,0);
		msleep(1);
		gpio_direction_output (MX6SL_SPK_VDD,0);
		msleep(1);
		gpio_direction_output (MX6SL_AD_3V3_EN,0);
		msleep(1);
		gpio_direction_output (MX6SL_AD_1V8_EN,0);		
		msleep(1);
		gpio_direction_output (MX6SL_AD_5V_EN, 0);		
	}
}
#endif
...
static void rt5640_pmu_depop(struct snd_soc_codec *codec)
{
	...
	lm45811_amp_power(0);
	snd_soc_update_bits(codec, RT5640_MONO_DAC_MIXER,	RT5640_M_DAC_L1_MONO_L | RT5640_M_DAC_R1_MONO_R, 0);
	msleep(1);
...
#else
	snd_soc_update_bits(codec, RT5640_MONO_DAC_MIXER,	RT5640_M_DAC_L1_MONO_L | RT5640_M_DAC_R1_MONO_R, RT5640_M_DAC_L1_MONO_L | RT5640_M_DAC_R1_MONO_R);
	snd_soc_update_bits(codec, RT5640_MONO_OUT,	RT5640_L_MUTE , RT5640_L_MUTE);
	lm45811_amp_power(0);
#endif	
}
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if(channels == 1)
	{
		snd_soc_update_bits(codec, RT5640_MONO_MIXER,
				RT5640_M_OV_R_MM, RT5640_M_OV_R_MM);

		snd_soc_update_bits(codec, RT5640_OUTPUT,
				RT5640_L_VOL_MASK | RT5640_R_VOL_MASK , (0x7 << RT5640_L_VOL_SFT) | (0x7 << RT5640_R_VOL_SFT));
	}
	else
	{
		snd_soc_update_bits(codec, RT5640_MONO_MIXER,
		...
		snd_soc_update_bits(codec, RT5640_OUTPUT,
				RT5640_L_VOL_MASK | RT5640_R_VOL_MASK , (0xb << RT5640_L_VOL_SFT) | (0xb << RT5640_R_VOL_SFT));
	}
	msleep(500);
#if 0
	//set DRC
	...
	{
		if(sample_rate == rate_tbl[i])
		{
			reg_val = i;
			snd_soc_update_bits(codec, RT5640_DRC_AGC_1,
				RT5640_DRC_AGC_R_MASK, reg_val << RT5640_DRC_AGC_R_SFT);
		}
	}
#endif
...
#ifdef CONFIG_PM
#ifdef CONFIG_SIGNAGE
static u16 rt5640_suspend_reg[RT5640_VENDOR_ID2 + 1] = { 0x00 };
#endif
static int rt5640_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	...
	lm45811_amp_power(0);
	int i;
	for(i=RT5640_RESET ; i<= RT5640_VENDOR_ID2 ; i++)
	{
		rt5640_suspend_reg[i] = snd_soc_read(codec, i);
		//printk("0x%02x = 0x%04x\n",i,rt5640_suspend_reg[i]);
	}
#endif
...
static int rt5640_resume(struct snd_soc_codec *codec)
{
	...
	for(i=RT5640_RESET ; i<= RT5640_VENDOR_ID2 ; i++)
	{
		snd_soc_write(codec,i,rt5640_suspend_reg[i]);
