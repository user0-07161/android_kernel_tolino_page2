...
static struct rt5645_init_reg init_list[] = {
	...
	{ RT5645_A_JD_CTRL1	, 0x0201 },/* for combo jack 3.3v */
	...
	/* record */
	{ RT5645_IN2	, 0x0200 },/* IN1 boost 20db and signal ended mode */
...
int rt5645_headset_detect(struct snd_soc_codec *codec, int jack_insert)
{
	...
	if (jack_insert) {
		...
		snd_soc_write(codec, RT5645_CHARGE_PUMP, 0x0e06);

		if (codec->card->instantiated == 0) {
		...
		default:
			...
			snd_soc_update_bits(codec, RT5645_GEN_CTRL2, 0x8, 0x0);
...
int rt5645_button_detect(struct snd_soc_codec *codec)
{
	...
	// pr_debug("%s, RT5645_INT_IRQ_ST=%d", __func__, snd_soc_read(codec, RT5645_INT_IRQ_ST));
...
static const SOC_ENUM_SINGLE_DECL(
	rt5645_in2_mode_enum, RT5645_IN2,
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	rt5645_headset_detect(codec, jack_insert);
...
static const struct snd_kcontrol_new rt5645_snd_controls[] = {
	...
	SOC_SINGLE_TLV("IN2 Boost", RT5645_IN2,
...
#ifdef INTEL_TDM
	if (rt5645->v_id >= RT5645_VER_D)
		snd_soc_update_bits(codec, RT5645_BASE_BACK, 0x0004, 0x0004);
#endif
	return 0;
}
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		pr_debug("%s switch SND_SOC_BIAS_STANDBY\n",__func__);
	...
	case SND_SOC_BIAS_OFF:
		pr_debug("%s switch SND_SOC_BIAS_OFF\n",__func__);
...
static int rt5645_probe(struct snd_soc_codec *codec)
{
	...
	rt5645->v_id = snd_soc_read(codec, RT5645_VENDOR_ID) & 0xff;
	pr_info("v_id = %d\n", rt5645->v_id);
	...
	snd_soc_add_controls(codec, rt5645_snd_controls,
...
#ifdef JD1_FUNC
	snd_soc_update_bits(codec, RT5645_PWR_ANLG2, RT5645_PWR_JD1, RT5645_PWR_JD1);
	snd_soc_update_bits(codec, RT5645_PWR_MIXER, RT5645_PWR_LDO2, RT5645_PWR_LDO2);
#endif
...
static int rt5645_resume(struct snd_soc_codec *codec)
{
	...
	msleep(50);
	snd_soc_write(codec, RT5645_DIG_MISC, 0x2061);
	//snd_soc_write(codec, RT5645_DIG_MISC, 0x2060);
	snd_soc_write(codec, RT5645_PWR_ANLG1, 0xa8d2);
	msleep(50);
	snd_soc_write(codec, RT5645_PWR_ANLG1, 0xe8da);
	msleep(50);
	snd_soc_write(codec, RT5645_PWR_ANLG1, 0xe8d8);
	//snd_soc_write(codec, RT5645_PWR_ANLG1, 0);
	snd_soc_write(codec, RT5645_PWR_MIXER, 0x0002);

#ifdef JD1_FUNC
	snd_soc_write(codec, RT5645_A_JD_CTRL1, 0x0201);
	snd_soc_write(codec, RT5645_IRQ_CTRL2, 0x0200);
	msleep(10);
	status = rt5645_check_jd_status(codec);
	pr_debug("%s: jd_status=%d\n", __func__, status);
	if (!status)
