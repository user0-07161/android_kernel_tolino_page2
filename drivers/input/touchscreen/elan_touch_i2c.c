...
static inline int elan_touch_parse_xy(uint8_t *data, unsigned int *x, unsigned int *y)
{
	...
	*y <<= 1;
#else
	unsigned int screenX = ELAN_TS_HIGHT-1 , screenY = ELAN_TS_WIDTH-1; 
	unsigned int elanInternalX = 832, elanInternalY = 1280;  //default 1024*758
	...
	//printk(KERN_ERR"--- Original  x:%d , y:%d ---\n",_x,_y);
#if 1
	if( 0==gptHWCFG->m_val.bDisplayResolution ) {  // resolution 800*600
		...
		*x = _y * screenY / elanInternalY;
		...
		//printk(KERN_ERR"--- x:%d , y:%d Transfer  %d_%d---\n",_x,_y,*x,*y);
	}
#else if
...
static int __elan_touch_init(struct i2c_client *client)
{
	...
	else if(0x1f39==g_fw_id||0x1f3b==g_fw_id)
	{
		...
		if (40==gptHWCFG->m_val.bPCB || 49==gptHWCFG->m_val.bPCB || 
				52==gptHWCFG->m_val.bPCB ||	70==gptHWCFG->m_val.bPCB)
		{
	...
	//printk(KERN_ERR"==== IDX_PACKET_SIZE %d====\n",IDX_PACKET_SIZE);
	...
	if(1==gELAN_I2C_format) {
		...
		g_boot_code = 0;
		g_boot_code += (buf_recv[1] & 0x0f) *1000 ;
		g_boot_code += ((buf_recv[2] & 0xf0 )>>4) *100;
		g_boot_code += (buf_recv[2] & 0x0f ) *10;
		g_boot_code += (buf_recv[3] &0xf0)>>4 ;
		printk("[boot_code]:%d\n",g_boot_code);
...
static void elants_i2c_mt_event_5finger(struct i2c_client *client, uint8_t *buf)
{
	...
	if(n_fingers==0)
	{
		if(finger_press[0]){
			...
			finger_press[0] = 0;
			//printk(KERN_ERR"-----%d UP \n",i);
		}
	}

	for (i = 0; i < FIVE_FINGER_NUM && n_fingers; i++) {
		if (finger_state & 1) {
			...
			input_event(elan_touch_data.input, EV_ABS, ABS_MT_POSITION_X, x1);
			input_event(elan_touch_data.input, EV_ABS, ABS_MT_POSITION_Y, y1);
			input_event(elan_touch_data.input, EV_ABS, ABS_MT_PRESSURE, p);
			input_event(elan_touch_data.input, EV_ABS, ABS_MT_TOUCH_MAJOR, w);
			input_event(elan_touch_data.input, EV_ABS, ABS_MT_WIDTH_MAJOR, w);
			...
			input_report_abs(elan_touch_data.input, ABS_PRESSURE, w);	// Joseph 20101023
			input_report_key(elan_touch_data.input, BTN_TOUCH, 1);
			finger_press[i] = 1;
			tmp_press[i]= 1;
			last_x[i] = x1 ;
			last_y[i] = y1;
			//printk(KERN_ERR"-----%d down \n",i);
			n_fingers--;
		}
		finger_state >>= 1;
	}
...
static void elants_i2c_mt_event(struct i2c_client *client, uint8_t *buf)
{
	unsigned int n_fingers ;
	unsigned int tmp_press[MAX_CONTACT_NUM] = {0} ;
	unsigned int last_x[MAX_CONTACT_NUM] = {0};
...
static void elan_touch_report_data(struct i2c_client *client, uint8_t *buf)
{
	...
	//printk(KERN_ERR "====elan_touch_report_data gELAN_I2C_format:%d %x_%x_%x_%x_%x_%x_%x_%x_%x ====\n",gELAN_I2C_format,buf[0],buf[1],buf[2],buf[3],buf[4],buf[5],buf[6],buf[7],buf[8]);
	switch (buf[0]) {
	...
	case multi_coordinate_packet: {
		...
		//else if (finger_stat == 1) {
			...
			if(elan_touch_parse_xy(&buf[finger2_index], &x2, &y2)){
				//printk(KERN_ERR"-----X:%d , Y:%d -----LINE:557\n",x2,y2);
				...
				//printk(KERN_ERR"-----X:%d , Y:%d -----LINE:570\n",last_x2,last_y2);
				DBG_MSG ("[%s-%d] finger 2 up (%d, %d).\n", __func__, __LINE__, last_x2, last_y2);
				input_report_abs(elan_touch_data.input, ABS_MT_TRACKING_ID, 1);
				input_report_abs(elan_touch_data.input, ABS_MT_TOUCH_MAJOR, 0);
				input_report_abs(elan_touch_data.input, ABS_MT_WIDTH_MAJOR, 0);
	...
	case hello_packet:
		...
		break;
	case five_finger_packet:
		printk("[%s-%d] 5 Finger packet %02x:%02X:%02x:%02x:%02x:%02x:%02x:%02x received\n", __func__, __LINE__, buf[0], buf[1], buf[2], buf[3] , buf[4], buf[5], buf[6], buf[7]);
	
		elants_i2c_mt_event_5finger(elan_touch_data.input,buf);
		break;		
...
static int isNewIAPflow(void)
{
	...
	if( 0x56==gptHWCFG->m_val.bPCB && 0x0000 == g_fw_id) {// E60QV2
		if( 2502 == g_boot_code) // E60QV2 2nd TP
			newIAPflow = 1 ;

		// boot_code 5568 use old iap_cmd
	}
		       ...
		       const char *buf, size_t count)
{
	...
	if(47==gptHWCFG->m_val.bPCB || 54==gptHWCFG->m_val.bPCB|| 84==gptHWCFG->m_val.bPCB) { //ED0Q02, ED0Q1X , E70Q5X
		data = 0x10;
	}
...
// 
int get_elan_touch_tp_source(void)
{
	int tp_source = 0 ;
	if(g_boot_code==2502)	// BN5 2nd source , 2502
		tp_source = 2 ;
	else if(g_boot_code==5568) // BN5 1st source , 5568
		tp_source = 1 ;
	else
		printk(KERN_ERR"[ERROR] Unknow boot_code:%d \n",g_boot_code);

	return tp_source;
}
EXPORT_SYMBOL(get_elan_touch_tp_source);

static ssize_t boot_code_read(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	int tp_source = 0 ;
	if(g_boot_code==2502)	// BN5 2nd source , 2502
	...
	return sprintf(buf, "%d",tp_source);
}
...
static DEVICE_ATTR(tp_source, S_IRUGO, boot_code_read, NULL);
