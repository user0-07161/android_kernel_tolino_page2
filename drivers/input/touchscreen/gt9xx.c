...
#define DEFAULT_PANEL_W		1872
#define DEFAULT_PANEL_H		1404


/**********************************************************************
Goodix(E70Q34)KTOL				Customer UI 			NB

 (1404,0)    (0,0)		( 0,1404)   (0,0) 	        (0,0)  (0,1404)
    *********			     *********			    *********
    *       *			     *       *			    *       *
    *       *			     *       *			    *       *
    *       *			     *       *			    *       *
    *       *			     *       *			    *       *
    *********			     *********			    *********
 (1404,1872) (0,1872)		(1872,1404)(1872,0)	     (1872,0) (1872,1404)
...
static int gPEN_DOWN = 0;
extern int gIsCustomerUi;
static int g_fw_id, g_fw_version, g_sensor_option;
static int g_firmware_upgrade_enable = 0;               // 1: enable firmware upgrade
int g_firmware_ugrading = 0;                            // 1: upgrading... , 
//static unsigned long GT9XX_TS_X_MAX=DEFAULT_PANEL_W;
//static unsigned long GT9XX_TS_Y_MAX=DEFAULT_PANEL_H;
static unsigned long GT9XX_TS_WIDTH=DEFAULT_PANEL_W;
static unsigned long GT9XX_TS_HEIGHT=DEFAULT_PANEL_H;
int gDisplayWidth = DEFAULT_PANEL_W ;
int gDisplayHeight = DEFAULT_PANEL_H;

typedef enum GTP_STATE {
    GTP_UNKNOW=0,
    GTP_LIVE,
    GTP_SUSPEND,
    GTP_CLOSE
...
extern u8 gup_init_update_proc_ex(void);
#endif
...
int Get_Goodix_State(void)
{
    if(gGTPState==GTP_SUSPEND)
        return 0;   
    else
        return gpio_get_value(gtp_int_gpio)?0:1;
}

// sysfs
static ssize_t goodix_firmware_upgrade_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", g_firmware_upgrade_enable);
}

static ssize_t goodix_firmware_upgrade_store(struct device *dev,
				      ...
				      const char *buf, size_t count)
{
    ...
    if(g_firmware_ugrading)
    {
        printk(KERN_ERR"[GTP] Firmware upgrading ..... can't be stop \n");
        return count;
    }

    if(val==1)
    {
        g_firmware_upgrade_enable = 1 ;
        printk(KERN_ERR"[GTP] Enable firmware upgrade \n");
        ret = gup_init_update_proc_ex();
        if (ret < 0)
        {
    ...
    {
        g_firmware_upgrade_enable = 0 ;
        printk(KERN_ERR"[GTP] Disable firmware upgrade \n");
    }
    
    return count;
}

static DEVICE_ATTR(firmware_upgrade, S_IWUSR | S_IRUGO,
		   goodix_firmware_upgrade_show, goodix_firmware_upgrade_store);
...
#ifdef ANDROID
static inline int gt9xx_touch_parse_xy(uint8_t *data,uint16_t _x ,uint16_t _y , uint16_t *x, uint16_t *y)
{
	int gt9xxInternalY = GT9XX_TS_WIDTH;	// 1872 
	int gt9xxInternalX = GT9XX_TS_HEIGHT ;	// 1404 
	int screenX = gDisplayHeight-1 , screenY = gDisplayWidth-1;

	if ( screenX > gt9xxInternalX || screenY > gt9xxInternalY )
	{
		printk("[%s-%d]ERROR : screen size over gx9xx internal resolution!\n", __func__, __LINE__);
		return -1;
	}
	...
	if( 0==gptHWCFG->m_val.bDisplayResolution ) {  // resolution 800*600
		...
		*x = (_x * screenX / gt9xxInternalX);
		if (gIsCustomerUi) {
			*y = screenX - (_y * screenY / gt9xxInternalY);
		} else {
			*y = _y * screenY / gt9xxInternalY;
		}
		if (0x2422==g_fw_id) {
			*y = screenX - *y;
		}
		...
		*x = _y * screenY / gt9xxInternalY;
		if (gIsCustomerUi) 
			*y = screenX - (_x * screenX / gt9xxInternalX);
		else
			*y = (_x * screenX / gt9xxInternalX);			
	}
	return 1 ;
}
#endif
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
    ...
    {  
        // EA0Q04 , (10.3 , Top EPD)
        input_report_abs(ts->input_dev, ABS_MT_POSITION_X, GT9XX_TS_WIDTH - y);
        input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,GT9XX_TS_HEIGHT - x );
        //GTP_DEBUG("ID:%d, X:%d, Y:%d, W:%d", id, x, y, w);
    }
    else if(16==gptHWCFG->m_val.bDisplayPanel){
        // E70Q34 , ( Bottom EPD)
        input_report_abs(ts->input_dev, ABS_MT_POSITION_X, y);
        input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,x);
        //GTP_DEBUG("ID:%d, X:%d, Y:%d, W:%d", id, y, x, w);
    }
    else if(17==gptHWCFG->m_val.bDisplayPanel){
        // E70Q54 , ( TOP EPD)
        //input_report_abs(ts->input_dev, ABS_MT_POSITION_X, y);
        //input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,x);
        //GTP_DEBUG("ID:%d, X:%d, Y:%d, W:%d", id, y, x, w);
    }
      else if(0==gptHWCFG->m_val.bDisplayPanel){
        // E60QT4 , (6 LEFT EPD)
        input_report_abs(ts->input_dev, ABS_X,GT9XX_TS_WIDTH-y );
...
#else
    input_report_abs(ts->input_dev, ABS_PRESSURE,0);		// fake value (epd_ctrl need this value to draw)
    input_report_key(ts->input_dev, BTN_TOUCH, 0);
#endif
#endif
}
...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
   ...
   // set_bit(INPUT_PROP_DIRECT, ts->pen_dev->propbit);
...
#ifdef ANDROID
	input_set_abs_params(ts->pen_dev, ABS_X, 0, ts->abs_y_max, 0, 0);
	input_set_abs_params(ts->pen_dev, ABS_Y, 0, ts->abs_x_max, 0, 0);
	input_set_abs_params(ts->pen_dev, ABS_PRESSURE, 0, 2048, 0, 0); 
#else		
    ...
    ts->pen_dev->id.bustype = BUS_I2C;
   	...
   	if( 17==gptHWCFG->m_val.bTouchCtrl) {  // GOODIX HAVE EPN , 18: GOODIX
        ...
        if (ret)
        {
       	...
       	ret = sysfs_create_group(&ts->pen_dev->dev.kobj, &goodix_attr_group);
    	if (ret) {
            GTP_ERROR("Sysfs create group failed");
    	}
    }
}
...
static void gtp_pen_down(s32 x, s32 y, s32 w, s32 id)
{
    ...
    if(28==gptHWCFG->m_val.bDisplayPanel)
    {   // EA0Q04 , (10.3 , Top EPD)
        input_report_abs(ts->pen_dev, ABS_X, GT9XX_TS_WIDTH - y);
        input_report_abs(ts->pen_dev, ABS_Y, GT9XX_TS_HEIGHT - x);
        GTP_DEBUG("(%d)(%d, %d)[%d]", id, x, y, w);
    }
    else if(16 == gptHWCFG->m_val.bDisplayPanel){
        // E70Q34
        input_report_abs(ts->pen_dev, ABS_X, y);
        input_report_abs(ts->pen_dev, ABS_Y, x);
        GTP_DEBUG("(%d)(%d, %d)[%d]", id, y, x, w);
    }
    else if(17 == gptHWCFG->m_val.bDisplayPanel){
        // E70Q54
        //input_report_abs(ts->pen_dev, ABS_X, y);
    ...
    {   // EA0Q04 , (10.3 , Top EPD)
        ...
        // E70Q34
	    input_report_abs(ts->pen_dev, ABS_MT_POSITION_X,y );
	    input_report_abs(ts->pen_dev, ABS_MT_POSITION_Y,x );
    }
    else if(17 == gptHWCFG->m_val.bDisplayPanel){
        // E70Q54
	    input_report_abs(ts->pen_dev, ABS_MT_POSITION_X,y );
	...
	input_report_abs(ts->pen_dev, ABS_PRESSURE, w);
	input_report_abs(ts->pen_dev, BTN_TOUCH, 1);
	input_mt_sync(ts->pen_dev);
	//if(w!=0)
	//	printk(KERN_ERR "== pen_pressre :%d == \n",w);
#endif
...
static void gtp_hover_out(void) {
    struct goodix_ts_data *ts = i2c_get_clientdata(i2c_connect_client);
...
static void gtp_pen_up(s32 id)
{
	...
	input_report_abs(ts->pen_dev, ABS_PRESSURE, 0);// fake value (epd_ctrl need this value to draw)
	input_report_abs(ts->pen_dev, BTN_TOUCH, 0);
	input_sync(ts->pen_dev);
#endif
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if (touch_num)
    {
        for (i = 0; i < touch_num; i++)
        {
            ...
            if (id & 0x80)
            {
               	if( 17==gptHWCFG->m_val.bTouchCtrl) // GOODIX HAVE EPN , 18: GOODIX_NO_PEN
                {
#ifndef ANDROID	
                    GTP_DEBUG("Pen touch DOWN! x:%d , y:%d  , w:%d\n",input_x,input_y,input_w);
	    			if(!gIsCustomerUi)	// Different UI has different coordination (Ebrmain UI)
		    		{
                        if(28==gptHWCFG->m_val.bDisplayPanel)
                        {   // EA0Q04 , (10.3 , Top EPD)
                            input_report_abs(ts->input_dev, ABS_X, GT9XX_TS_WIDTH-input_y);
		                    input_report_abs(ts->input_dev, ABS_Y, input_x);
                        }
                        else if(16 == gptHWCFG->m_val.bDisplayPanel){  
                            // E70Q34 , (7.8 )
					        input_report_abs(ts->input_dev, ABS_X,input_y );
					        input_report_abs(ts->input_dev, ABS_Y,GT9XX_TS_HEIGHT-input_x );
                        }
                        else if(17 == gptHWCFG->m_val.bDisplayPanel){  
                            // E70Q54 , (7.8 )
					        input_report_abs(ts->input_dev, ABS_X,input_y );
...
#endif
				    {
                        ...
                        GTP_DEBUG("Pen x:%d , y:%d  , w:%d\n",input_x,input_y,input_w);

                        if ( gPEN_DOWN == 1 && input_w == 0 ) {
                            GTP_DEBUG("Pen up \n");
                            gtp_pen_up(0);
                        } else {
                            GTP_DEBUG("Pen hover in \n");
		    			    gtp_pen_down(input_x, input_y, input_w, 0);
                        }
				    }
				    pre_pen = 1;
        ...
        #endif
            {
				...
				if(!gIsCustomerUi)	// Different UI has different coordination (Ebrmain UI)
				{
                    //printk(KERN_ERR "---- touch down x%d,y:%d \n ",input_y,input_x);
                    GTP_DEBUG("Touch DOWN! x:%d , y:%d  , w:%d\n",input_x,input_y,input_w);
                    if(28==gptHWCFG->m_val.bDisplayPanel) {
                        // EA0Q04 , (10.3 , Top EPD)
					    input_report_abs(ts->input_dev, ABS_X, GT9XX_TS_WIDTH-input_y);
					    input_report_abs(ts->input_dev, ABS_Y, input_x);
		...
		if (pre_pen == 1)
        {
	        if( 17==gptHWCFG->m_val.bTouchCtrl)   // GOODIX HAVE EPN , 18: GOODIX_NO_PEN            
			{
...
#endif
			    {
                    ...
                    GTP_DEBUG("Pen hover out!");
					//when input_w != 0 and touch_num = 0, it means that pen leave without send pen up, add pen up event
                    if ( gPEN_DOWN == 1 )
                        gtp_pen_up(0);
                    gtp_hover_out();
				   // gtp_pen_up(0);
 ...
 */
    ...
    u8 cfg_info_KOTL[] = CTP_CFG_KOTL;      // EA0Q34
    u8 cfg_info_KOTL78[] = CTP_CFG_KOTL_7_8;  // E70Q34
    u8 cfg_info_KOTL6[] = CTP_CFG_KOTL_6;  // E60QT4

    GTP_INFO("PCB:%d , bPanelLaminationSrc:%d \n",gptHWCFG->m_val.bPCB,gptHWCFG->m_val.bPanelLaminationSrc);
      if(1==gptHWCFG->m_val.bPanelLaminationSrc)
    {
        if(28 == gptHWCFG->m_val.bDisplayPanel)  // EA0Q34
        {   
            memcpy(cfg_info_group0,cfg_info_KOTL,sizeof(cfg_info_KOTL));
            GTP_INFO("Using KOTL's config (EA0Q34 , 10)");
        } 
        else if(16 == gptHWCFG->m_val.bDisplayPanel) // E70Q34
        {
            memcpy(cfg_info_group0,cfg_info_KOTL78,sizeof(cfg_info_KOTL78));
            GTP_INFO("Using KOTL's config (E70Q34 , 7.8)");
        }
        else if(17 == gptHWCFG->m_val.bDisplayPanel) // E70Q54
        {
            memcpy(cfg_info_group0,cfg_info_KOTL78,sizeof(cfg_info_KOTL78));
            GTP_INFO("Using KOTL's config (E70Q54 , 7.8)");
        }        
        else if(0 == gptHWCFG->m_val.bDisplayPanel) // E60QT4
        {
            memcpy(cfg_info_group0,cfg_info_KOTL6,sizeof(cfg_info_KOTL6));
            GTP_INFO("Using KOTL's config (E60QT4 , 6)");
        }       
        else{
            GTP_ERROR("%s() , (KTOL) Unsupport PCB:%d \n",__FUNCTION__,gptHWCFG->m_val.bPCB);
            return -1;
        }
    }
    else{
        GTP_ERROR("%s() , Unsupport bPanelLaminationSrc \n",__FUNCTION__);
        return -1;
    }
...
#endif
	{
        ...
        GT9XX_TS_WIDTH = ts->abs_y_max ;    // 1872
        GT9XX_TS_HEIGHT = ts->abs_x_max ;    // 1404
...
//	gtp_int_gpio = GTP_INT_PORT;
	...
	if(77 == gptHWCFG->m_val.bPCB)  // EA0Q34
		gtp_int_gpio = irq_to_gpio(client->irq);
	else if(76==gptHWCFG->m_val.bPCB || 84==gptHWCFG->m_val.bPCB) // E70Q34 , E70Q54
		gtp_int_gpio = GTP_INT_PORT;
    else if(83 == gptHWCFG->m_val.bPCB) // E60QT4
        gtp_int_gpio = GTP_INT_PORT;
	printk ("[%s-%d] int %d, rst %d\n", __func__, __LINE__, gtp_int_gpio, gtp_rst_gpio);
#endif
    ...
    if (ret < 0)
    {
        ...
        printk ("[%s-%d] int %d, rst %d\n", __func__, __LINE__, gtp_int_gpio, gtp_rst_gpio);
        GTP_GPIO_OUTPUT(gtp_int_gpio, 0);       // If goodix not connect to PCB , must set INT_GPIO is 0  , or can't enter hibernation
        return -ENODEV;
    }
    ...
    if( 8 ==gptHWCFG->m_val.bDisplayResolution ) {  
        if(ts->abs_x_max != 1404 || ts->abs_y_max != 1872)
            GTP_ERROR("Panel's resolution %dx%d is not same as 1872x1404 ",ts->abs_x_max,ts->abs_y_max);
        gDisplayWidth = 1872 ;
        gDisplayHeight = 1404 ;
        //ts->int_trigger_type = GTP_INT_TRIGGER;    
    } 
    ...
    {
         if(ts->abs_x_max != 1072 || ts->abs_y_max != 1448)
            GTP_ERROR("Panel's resolution %dx%d is not same as 1448x1072 ",ts->abs_x_max,ts->abs_y_max);
        gDisplayWidth = 1448 ;
        gDisplayHeight = 1072 ;
        //ts->int_trigger_type = GTP_INT_TRIGGER;          
    }
    if( 14 ==gptHWCFG->m_val.bDisplayResolution ) {  
        if(ts->abs_x_max != 1440 || ts->abs_y_max != 1920)
            GTP_ERROR("Panel's resolution %dx%d is not same as 1920x1440 ",ts->abs_x_max,ts->abs_y_max);
        gDisplayWidth = 1920 ;
        gDisplayHeight = 1440 ;
        //ts->int_trigger_type = GTP_INT_TRIGGER;
    } 
...
#if GTP_AUTO_UPDATE
    #if 0   // Disable ((Start from enable firmware_upgrade )
