...
#define CY_CORE_STARTUP_RETRY_COUNT		1
...
static volatile unsigned long gdwJiffies_touch_proc_in_sleep ;
...
static irqreturn_t cyttsp5_irq(int irq, void *handle)
{
	...
	//printk("%s(%d) : sleep_state=%d\n",__func__,__LINE__,cd->sleep_state);

	if (!cyttsp5_check_irq_asserted(cd) ) {
		//printk("%s(%d) : skipped (irq not asserted) \n",__func__,__LINE__);
		return IRQ_HANDLED;
	}

	if(cd->sleep_state == SS_SLEEP_ON) {
		if(time_is_after_jiffies(gdwJiffies_touch_proc_in_sleep+3)) {
			//printk("%s(%d) : skipped (irq shortly interval) \n",__func__,__LINE__);
			return IRQ_HANDLED;
		}
	}
	...
	gdwJiffies_touch_proc_in_sleep = jiffies;
	return IRQ_HANDLED;
}
...
static ssize_t cyttsp5_manufacture_data_show(struct device *dev,
...
static struct device_attribute attributes[] = {
	__ATTR(manufacture_data, S_IRUGO, cyttsp5_manufacture_data_show, NULL),
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	// Fix bug: touch events are submitted after resuming system
	// device buffer the touch events when runtime suspend function is executed
