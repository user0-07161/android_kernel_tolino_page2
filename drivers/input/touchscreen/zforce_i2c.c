...
bool ir_touch_still_pressed(){
	return g_touch_pressed;
}

extern unsigned gMX6SL_IR_TOUCH_RST;
#define IR_TOUCH_RST		gMX6SL_IR_TOUCH_RST
...
//				printk ("[%s-%d] send cmd_LowSignalRequestY\n",__func__,__LINE__);
		...
		case 0x1E:
			if(8==gptHWCFG->m_val.bTouchCtrl || 11==gptHWCFG->m_val.bTouchCtrl) {
				...
				if ((5 == buf[46] && 2 == buf[47]) && 11!=gptHWCFG->m_val.bTouchCtrl) {
					printk ("[%s-%d] Warning!! using old format with new NN firmware, force apply V3 format. \n",__func__,__LINE__);
					gptHWCFG->m_val.bTouchCtrl = 11;
				}
			}
			break;
		case 0x25:
...
//	enable_irq_wake(zForce_ir_touch_data.client->irq);
...

	{
		if(1==gptHWCFG->m_val.bDisplayResolution) {
			...
			// 1920x1440
			ZFORCE_TS_WIDTH=1264;
			ZFORCE_TS_HIGHT=1680;
		}
		else {
			// 800x600 
			ZFORCE_TS_WIDTH=800;
