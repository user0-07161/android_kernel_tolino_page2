		...
		struct cyttsp5_touch *tch, int num_cur_tch)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		if(0==gptHWCFG->m_val.bUIStyle) {
			/* single touch event for ebrmain*/
			if(gptHWCFG->m_val.bPCB==77) //EA0Q04
			{
				printk(KERN_ERR"****** Resolution was wrong !! Need to check!!******\n");
				input_report_abs(md->input, ABS_X ,tch->abs[0]*1871/1280);
				input_report_abs(md->input, ABS_Y ,tch->abs[1]*1403/720);
			}			
			else{
