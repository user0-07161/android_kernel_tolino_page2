...
static int __devinit gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		//printk("%s() register \"%s\",code=%d \n",__func__,button->desc,button->code);
	}
	...
	input_set_capability(input, EV_KEY, KEY_PLAYER);
		...
		for (i = 0; i < pdata->nbuttons; i++) {
			...
			if (button->wakeup) {
				...
				if (gSleep_Mode_Suspend && (KEY_POWER != button->code)) {
					if ( (69==gptHWCFG->m_val.bPCB) ||
						(84==gptHWCFG->m_val.bPCB) )
					{
						// E60QQX / E70Q5X 
#ifdef CONFIG_ANDROID //[
						if(KEY_HOME == button->code)
#else //][!CONFIG_ANDROID
						if(KEY_F3 == button->code)
