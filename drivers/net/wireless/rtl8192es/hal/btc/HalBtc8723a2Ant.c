	...
	)
{
	...
	else
	{
		...
		if(retryCount == 0)  // no retry in the last 2-second duration
		{
			...
			if(up >= n)	// if ³sÄò n ­Ó2¬í retry count¬°0, «hœÕŒeWiFi duration
			{
				WaitCount = 0; 
				n = 3;
				up = 0;
		...
		else if (retryCount <= 3)	// <=3 retry in the last 2-second duration
		{
			...
			if (dn == 2)	// if ³sÄò 2 ­Ó2¬í retry count< 3, «hœÕ¯¶WiFi duration
			{
				if (WaitCount <= 2)
					m++; // Á×§K€@ªœŠbšâ­Ólevel€€šÓŠ^
				else
					m = 1;

				if ( m >= 20) //m ³Ì€j­È = 20 ' ³Ì€j120¬í recheck¬O§_œÕŸã WiFi duration.
					m = 20;
		...
		else  //retry count > 3, ¥u­n1Šž retry count > 3, «hœÕ¯¶WiFi duration
		{
			if (WaitCount == 1)
				m++; // Á×§K€@ªœŠbšâ­Ólevel€€šÓŠ^
			else
