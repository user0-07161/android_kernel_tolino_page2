...
static void usb_charger_work(struct work_struct *data)
{
	...
	if (1==gptHWCFG->m_val.bPMIC) {	// ricoh 
...
//			printk ("[%s-%d] DCD result 0x%X \n", __func__, __LINE__, dcd_result);
        ...
        if ( (SDP_PC_CHARGER != dcd_result) && (CDP_CHARGER != dcd_result) )
			msleep (500);
		
        if ( ((SDP_PC_CHARGER == dcd_result) || (CDP_CHARGER == dcd_result)) && (charger->dp_pullup))
