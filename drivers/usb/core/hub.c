		...
		struct usb_hub_status *data)
{
	...
	if(i>=USB_STS_RETRIES) {
		dev_warn(&hdev->dev,
			"%s() retried %d failed (status = %d)\n", __func__,i,status);
	}
	return status;
}
...
descriptor_error:
	...
	INIT_DELAYED_WORK(&hub->reset_work, reset_work);
	usb_get_intf(intf);
...
int usb_port_resume(struct usb_device *udev, pm_message_t msg)
{
 ...
 SuspendCleared:
	...
	if (status < 0) {
		...
		if(-71==status) {
			if(delayed_work_pending(&hub->reset_work)) {
				dev_warn(&udev->dev, "trying reset...\n");
			}
			else {
				dev_warn(&udev->dev, "protocol error,try reset...\n");
				cancel_delayed_work_sync(&hub->reset_work);
				schedule_delayed_work (&hub->reset_work, 10);
			}
		}
	}
	return status;
			...
			for (j = 0; j < 3; ++j) {
				...
				default:
					if (r == 0) {
						r = -EPROTO;
						dev_warn(&udev->dev,
							"MaxPacketSize0=%d , usb_control_msg retrun 0!!\n",buf->bMaxPacketSize0);
					}
					break;
				}
				if (r == 0)
