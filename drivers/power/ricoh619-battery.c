...
#define OCV_UNDER10P_DISABLED	0
#define OCV_UNDER10P_DEFAULT	1
#define OCV_UNDER10P_DYNAMIC	2
#define OCV_UNDER10P_DYNAMIC_LINEAR	2
#define OCV_UNDER10P_DYNAMIC_EXP	3
static int giU10PtableType = OCV_UNDER10P_DEFAULT;
static int gtOCV_table_U10_dynamic[11] = {0,};
...
//#define BATTERY_TEMP_0_VOL		3120	// 0 degrees
//#define BATTERY_TEMP_5_VOL		2914	// 5 degrees
//#define BATTERY_TEMP_8_VOL		2762	// 8 degrees
//#define BATTERY_TEMP_40_VOL	1259		// 40 degrees
//#define BATTERY_TEMP_48_VOL	976		// 48 degrees
//#define BATTERY_TEMP_50_VOL	902		// 50 degrees
//#else
...
static int ibat_table[][RICOH61x_IBAT_TABLE_NUM] = {	/* 85% - 100% */
			{370, 348, 326, 304, 282, 260, 238, 216, 194, 172, 150, 128, 107, 87, 68, 50},
		 	{370, 339, 299, 256, 210, 172, 145, 125, 110,  98,  87,  78,  70, 62, 55, 50} //E70Q5X
		 };
...
struct ricoh61x_battery_info {
	...
	int temp_high_lv1_charge;
	int temp_high_lv2_stop_charge;
	int temp_high_lv3_alarm;
...
static int giChgOffTempADC_offset=0;
...
static int _config_ricoh619_charger_params(struct device *bat_dev,int iChargerType)
{
	...
	{	// SDP PC
		...
		if(77 == gptHWCFG->m_val.bPCB) {  //EA0Q0X
			ricoh61x_write(bat_dev->parent, REGISET1_REG, 0x07);
			ricoh61x_write(bat_dev->parent, REGISET2_REG, 0xE7);
		}
		else {
			ricoh61x_write(bat_dev->parent, REGISET1_REG, 0x06);
			// RC5T619-1010 承認書.pdf
			/* 00000110
			   ---|||||
			      \\\\\_ Bit 4~0: ILIM_ADP[4:0] bit: Limit current setting values; 700 mA
			*/
			ricoh61x_write(bat_dev->parent, REGISET2_REG, 0xE6);
			/* 11100110
			   ||||||||
			   |||\\\\\_ Bit 4~0: ILIM_USB[4:0] bit: Limit current setting values; When <VBUS_TYPE> is SDP or “other” and <SDPOVRLIM>=0, the writing to this register is restricted to 500mA; 700 mA
			   ||\______ Bit 5: ILLPORTCHGEN: Set whether the charge operation is permitted or not, when <VBUS_TYPE> is “other”. “1”: 3000mA
			   |\_______ Bit 6: SDPLIMSEL bit: Set whether the charge operation is permitted or not, when <VBUS_TYPE> is “other”. “1”: 500mA
			   \________ Bit 7: SDPOVRLIM bit: Set maximum current value by ILIM_USB, when <VBUS_TYPE> is SDP or “other”. “1”: Permission
			*/
		}
		
		if(83 == gptHWCFG->m_val.bPCB) {  //E60QT4
			ricoh61x_write(bat_dev->parent, CHGISET_REG, val|0x05);
		}
		else
			ricoh61x_write(bat_dev->parent, CHGISET_REG, val|0x04);
			/* 11100100
			   ||-|||||
			   || \\\\\_ Bit 4~0: ICHG[4:0] bit: Current settings value for charging; 500 mA
			   \\_______ Bit 7~6: ICCHG[1:0] bit: Current settings at the charge completion; 200 mA
			*/
	}
		       ...
		       const char *buf, size_t count)
{
	...
	else if(0==strcmp(buf,"NO")) { // 70-90 mA
		giRICOH619_DCIN=NO_CHARGER_PLUGGED;
		// No charger .
		printk(KERN_INFO "#############################################\n", __func__);
		printk(KERN_INFO "PMU:%s set NO_CHARGER_PLUGGED 100mA charging.\n", __func__);
		ricoh61x_write(dev->parent, REGISET1_REG, 0x00); // 100 mA
		ricoh61x_write(dev->parent, REGISET2_REG, 0x00); // 100 mA/“0”:Prohibition/“0”:100mA/“0”:500mA
		ricoh61x_write(dev->parent, CHGISET_REG,  0x20); // %00100000: 50/100 mA
	...
	else if(0==strcmp(buf,"DISABLE")) { // 70-400 mA
		giRICOH619_DCIN=-1;
		// Disable charger .
		printk(KERN_INFO "############################\n", __func__);
		printk(KERN_INFO "PMU:%s set DISABLE charging.\n", __func__);
		int err;
		/* Disable charging */
		err = ricoh61x_clr_bits(dev->parent,CHGCTL1_REG, 0x03);
		if (err < 0) {
			dev_err(dev, "Error in writing the control register\n");
		}
		if(iLastChargerType!=giRICOH619_DCIN) {
			printk("%s():report DCIN=%d\n",__FUNCTION__,giRICOH619_DCIN?1:0);
			if(mxc_misc_report_usb) {
...
static int Battery_Table(void)
{
	...
	case 19: //PR-248899G-3000mA.C2
		...
		break;
	case 20: //EVE254385N-1000mA.C2
		BatteryTableFlageDef=12;
                printk(KERN_ERR"==== OCV TABLE 12 ====\n");
		break;                
	default:
	...
	if (84==gptHWCFG->m_val.bPCB){ //E70Q54
		BatteryTableFlageDef=11; //PR-285083N-1500mAh
	}
...
static void genOCVU10per_dynamic_tab(struct ricoh61x_battery_info *info,int voltage)
{
	int i;

	int iStep ;
	int u10_sqrt[11] = {1,316,447,548,632,707,775,837,894,948,1000};
	//int u10_sqrt[11] = {1000,948,894,837,775,707,632,548,447,316,1};

	gtOCV_table_U10_dynamic[0] = get_OCV_voltage(info, 0, ORIGINAL);
	if(info->fg_poff_vbat != 0){
		gtOCV_table_U10_dynamic[0] = info->fg_poff_vbat*1000;
	}
	printk(KERN_DEBUG"U10pDynamic[0]=%d\n",gtOCV_table_U10_dynamic[0]);
	if(voltage<=gtOCV_table_U10_dynamic[0]) {
		gtOCV_table_U10_dynamic[10] = get_OCV_voltage(info, 1, ORIGINAL);
	}
	else {
		gtOCV_table_U10_dynamic[10] = voltage;
	}
	iStep = (gtOCV_table_U10_dynamic[10]-gtOCV_table_U10_dynamic[0])/10;
	for(i=1;i<=9;i++) {
		if(OCV_UNDER10P_DYNAMIC_EXP==giU10PtableType) {
			gtOCV_table_U10_dynamic[i] = gtOCV_table_U10_dynamic[0]+\
				((gtOCV_table_U10_dynamic[10]-gtOCV_table_U10_dynamic[0])*u10_sqrt[i]/1000);
		}
		else {
			gtOCV_table_U10_dynamic[i] = gtOCV_table_U10_dynamic[i-1]+iStep;
		}
		printk(KERN_DEBUG"U10pDynamic[%d]=%d\n",i,gtOCV_table_U10_dynamic[i]);
	}
	printk(KERN_DEBUG"U10pDynamic[10]=%d\n",gtOCV_table_U10_dynamic[10]);
...
//	info->soca->temp_cc_delta_cap_mas = info->soca->temp_cc_delta_cap_mas - ( (fa_cap * 3600) * (temp_cc_delta_cap / 10000)) ;
	...
	if( (0==gtOCV_table_U10_dynamic[10]) && 
		 (info->soca->displayed_soc <= 1099) )
	{
		genOCVU10per_dynamic_tab(info , info->soca->Vbat_ave);
	}
	...
	if (capacity_remain <= 50){
		printk(KERN_ERR "PMU: %s : No6 :Hurry up!!! \n", __func__);
		info->soca->displayed_soc = info->soca->displayed_soc - 100;
		info->soca->displayed_soc = max(0, info->soca->displayed_soc);
		info->soca->hurry_up_flg = 1;
		return;
	...
	printk(KERN_ERR "MM[%s %d]: info->soca->status:%d info->soca->ready_fg:%d info->soca->displayed_soc: %d info-cur_voltage: %d\n", __func__, __LINE__ , info->soca->status, info->soca->ready_fg, info->soca->displayed_soc, info->cur_voltage);
	return;
}
...
static int getCapFromOriTable(struct ricoh61x_battery_info *info, int voltage, int currentvalue, int resvalue)
{
	...
	if (ocv_table[10] <= ocv) {
		...
		for (i = 1; i < 11; i++) {
			if (ocv_table[i] >= ocv) {
				if ( (OCV_UNDER10P_DEFAULT==giU10PtableType) && (i==1) )
				{
					// <=10% ocv voltage 
					capacity = getCapFromOriTable_U10per(info, voltage, currentvalue, resvalue);
				}
				else 
				if( (giU10PtableType>=OCV_UNDER10P_DYNAMIC) && 
						(info->soca->displayed_soc <= 1099) )
				{
					// dynamic <10% ocv table , <=10% ocv voltage || 10% capcacity .
					capacity = getCapFromOriTable_U10per(info, voltage, currentvalue, resvalue);
				}else{
					/* unit is 0.01% */
					capacity = Calc_Linear_Interpolation(
...
static int getCapFromOriTable_U10per(struct ricoh61x_battery_info *info, int voltage, int currentvalue, int resvalue)
{
	...
	int ocv_table_for_3000mAh_coff36[11] = {	3602154,
 							...
 							3698221,
 							3701000};
	int ocv_table_for_1500mAh_coff3650[11] = {	3719686,
							3724177,
							3729977,
							3733002,
							3749340,
	...
	int ocv_table_for_1000mAh[11] = {	        3375294,
                                                        3425013,
                                                        3454313,
                                                        3475113,
                                                        3488513,
	...
	if(giU10PtableType>=OCV_UNDER10P_DYNAMIC) {
		// dynamic table .
		ocv_table = gtOCV_table_U10_dynamic;
	}
	else 
	if(10 == gptHWCFG->m_val.bBattery) {// 1200mAh battery
 	...
 	else if (20 == gptHWCFG->m_val.bBattery)		// EVE254385N-1000mA.C2
        {
                //if(86==gptHWCFG->m_val.bPCB)
								{
		        ocv_table = ocv_table_for_1000mAh;
                 //       printk(KERN_ERR"==== OCV 0~10 =====\n");
                }      
        }
	...
	if (84==gptHWCFG->m_val.bPCB){ //E70Q54
		ocv_table = ocv_table_for_1500mAh_coff3650; //PR-285083N-1500mAh
	}
...
static int calc_soc_by_voltageMethod(struct ricoh61x_battery_info *info)
{
	...
	if(10 == gptHWCFG->m_val.bBattery) {// 1200mAh battery
	...
	else if (86 == gptHWCFG->m_val.bPCB) {// E60QVx
                printk(KERN_ERR"==== %s QV2 ====\n",__FUNCTION__);
		if(info->soca->Vbat_ave > 4100000) {
			soc = 10000;
		...
		if(info->soca->Vbat_ave >= info->soca->ocv_table[10] ) {
			soc = 10000;
		} else if(info->soca->Vbat_ave < (info->fg_poff_vbat * 1000)) {
			soc = 0;
		} else if(info->soca->Vbat_ave < info->soca->ocv_table[0]) {
			soc = 0;		
		} else {
#if 1
			int i;
...
//			if (info->soca->displayed_soc/100 < RICOH61x_ENTER_LOW_VOL) {
			if ((RICOH61x_SOCA_STABLE == info->soca->status)  || (75 != gptHWCFG->m_val.bPCB)) {
						...
						if(POWER_SUPPLY_STATUS_FULL == info->soca->chg_status)
						{
                            ...
                            ibat_soc_base = 10000 - (RICOH61x_IBAT_TABLE_NUM - 1) * 100 - 50;
                                                        ...
                                                        {       // Disable ibat_table
                                                                ...
                                                                printk(KERN_INFO "PMU: %s IBAT= %d: ibat_table[%d][%d%%]= %d: ibat_soc= %d ************\n",
                                                                        __func__, info->soca->Ibat_ave, info->ibat_num, (100 - RICOH61x_IBAT_TABLE_NUM + 1), ibat_table[info->ibat_num][0], ibat_soc);
                                                        }
                                                        else{
                                                                ibat_soc_base = 10000 - (RICOH61x_IBAT_TABLE_NUM - 1) * 100 - 50;
							        if (ibat_table[info->ibat_num][0] < info->soca->Ibat_ave) {
								        if (ibat_soc_base < info->soca->displayed_soc){
									        ibat_soc = ibat_soc_base;
								        } else {
									        ibat_soc = info->soca->displayed_soc;
								        ...
								        printk(KERN_INFO "PMU: %s IBAT= %d: ibat_table[%d][%d%%]= %d: ibat_soc= %d ************\n",
								        	 __func__, info->soca->Ibat_ave, info->ibat_num, (100 - RICOH61x_IBAT_TABLE_NUM + 1), ibat_table[info->ibat_num][0], ibat_soc);
							        } else if (ibat_table[info->ibat_num][RICOH61x_IBAT_TABLE_NUM-1] >= info->soca->Ibat_ave) {
								        ibat_soc = 9950;
								        printk(KERN_INFO "PMU: %s IBAT= %d: ibat_table[%d][100%%]= %d: ibat_soc= %d ************\n",
								        	 __func__, info->soca->Ibat_ave, info->ibat_num, ibat_table[info->ibat_num][RICOH61x_IBAT_TABLE_NUM-1], ibat_soc);
							        } else {
								        for (i = 1; i <= (RICOH61x_IBAT_TABLE_NUM-1); i++) {
									        if(ibat_table[info->ibat_num][i] <= info->soca->Ibat_ave) {
										        ibat_soc = Calc_Linear_Interpolation(
											        (i-1) * 100, ibat_table[info->ibat_num][i-1], i * 100,
											         ibat_table[info->ibat_num][i], info->soca->Ibat_ave);
										        ibat_soc += ibat_soc_base;

#ifdef LTS_DEBUG
										        printk(KERN_INFO "PMU: %s IBAT= %d: ibat_table[%d][%d%%]= %d, ibat_table[%d][%d%%]= %d: ibat_soc= %d: ************\n",
											         __func__, info->soca->Ibat_ave, info->ibat_num, (100 - RICOH61x_IBAT_TABLE_NUM + i), ibat_table[info->ibat_num][i-1],
											          info->ibat_num, (100 - RICOH61x_IBAT_TABLE_NUM + 1 + i), ibat_table[info->ibat_num][i], ibat_soc);
#endif
										        break;
									        }
								        }
	...
	if (RICOH61x_SOCA_DISP == info->soca->status) {
			...
			((10 != gptHWCFG->m_val.bBattery)&&info->soca->Ibat_ave >= -12) ) 
		{
				...
				&& (info->soca->Ibat_ave < 300))
			{
				...
				err = reset_FG_process(info);
				if (err < 0)
					dev_err(info->dev, "Error in writing the control rworkegister\n");				
				goto end_flow;
			}
		}
...
static int ricoh61x_init_charger(struct ricoh61x_battery_info *info)
{
	...
	if (49==gptHWCFG->m_val.bPCB || 69==gptHWCFG->m_val.bPCB || 84==gptHWCFG->m_val.bPCB)   //E60QDX, E60QQX, E70Q5x
...
//	mutex_lock(&info->lock);
	...
	if (0x04 & info->chg_stat1) {
		int err;
		printk("PMU:charge overtime int triggered...\n");
	...
	if (0x02 & info->chg_stat1) {
		queue_delayed_work(info->monitor_wqueue, &info->displayed_work, HZ/10);
	}
	info->chg_ctr = 0;
	info->chg_stat1 = 0;
	...
	if(!giRICOH619_DCIN) {
		gtOCV_table_U10_dynamic[10] = 0;// need re create dynamic table .
	}
...
static void battery_temp_irq_work(struct work_struct *work)
{
	...
	printk(KERN_ERR "PMU:%s In\n", __func__);

	power_supply_changed(&info->battery);

	ricoh61x_read(info->dev->parent, RICOH61x_ADC_VTHMDATAH, &val);
	bat_temp_vol = val*2500/255;
	printk(KERN_INFO "PMU:%s Battery temperature triggered %dmV+%d (VTHMDATA 0x%02X)**********\n", __func__, bat_temp_vol,giChgOffTempADC_offset, val);
	bat_temp_vol += giChgOffTempADC_offset;
	

	if (bat_temp_vol < info->temp_high_lv3_alarm) {
		printk (KERN_INFO "PMU:%s Battery temp > high_temp_alarm(%d) degrees !! \n",
			 	__func__,info->temp_high_lv3_alarm);
		/* Set VTHM threshold high voltage value = (voltage(V)*255)/(2.5) */
		val = (info->temp_high_lv3_alarm+giChgOffTempADC_offset) * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THH, val);
		/* Enable VTHM threshold high interrupt */
		ricoh61x_set_bits(info->dev->parent, RICOH61x_INT_EN_ADC2, 0x20);
		gBatt_ADC_isr_temp = 505;
	}
	else if (bat_temp_vol < info->temp_high_lv2_stop_charge) {
		printk (KERN_INFO "PMU:%s high_temp_stop_chg < Battery temp < high_temp_alarm \n",__func__);
		/* Set VTHM threshold low voltage value = (voltage(V)*255)/(2.5) */
		val = (info->temp_high_lv3_alarm+giChgOffTempADC_offset) * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THL, val);
		/* Enable VBAT threshold Low interrupt */
		ricoh61x_set_bits(info->dev->parent, RICOH61x_INT_EN_ADC1, 0x20);
		/* Set VTHM threshold high voltage value = (voltage(V)*255)/(2.5) */
		val = (info->temp_high_lv2_stop_charge+giChgOffTempADC_offset) * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THH, val);
		/* Enable VTHM threshold high interrupt */
		ricoh61x_set_bits(info->dev->parent, RICOH61x_INT_EN_ADC2, 0x20);
	...
	else if (bat_temp_vol < info->temp_high_lv1_charge) {
		printk (KERN_INFO "PMU:%s high_temp_rechg < Battery temp < high_temp_stop_chg\n",__func__);
		/* Set VTHM threshold low voltage value = (voltage(V)*255)/(2.5) */
		val = (info->temp_high_lv2_stop_charge+giChgOffTempADC_offset) * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THL, val);
		/* Enable VBAT threshold Low interrupt */
		ricoh61x_set_bits(info->dev->parent, RICOH61x_INT_EN_ADC1, 0x20);
		/* Set VTHM threshold high voltage value = (voltage(V)*255)/(2.5) */
		val = (info->temp_high_lv1_charge+giChgOffTempADC_offset) * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THH, val);
		/* Enable VTHM threshold high interrupt */
		ricoh61x_set_bits(info->dev->parent, RICOH61x_INT_EN_ADC2, 0x20);
		...
		/* Set VTHM threshold low voltage value = (voltage(V)*255)/(2.5) */
		val = (BATTERY_TEMP_0_VOL+giChgOffTempADC_offset) * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THL, val);
		/* Enable VBAT threshold Low interrupt */
		ricoh61x_set_bits(info->dev->parent, RICOH61x_INT_EN_ADC1, 0x20);
		...
		/* Set VTHM threshold low voltage value = (voltage(V)*255)/(2.5) */
		val = (BATTERY_TEMP_5_VOL+giChgOffTempADC_offset) * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THL, val);
		/* Enable VBAT threshold Low interrupt */
		ricoh61x_set_bits(info->dev->parent, RICOH61x_INT_EN_ADC1, 0x20);
		/* Set VTHM threshold high voltage value = (voltage(V)*255)/(2.5) */
		...
		/* Set VTHM threshold low voltage value = (voltage(V)*255)/(2.5) */
		val = (BATTERY_TEMP_8_VOL+giChgOffTempADC_offset) * 255 / 2500;
		ricoh61x_write(info->dev->parent, RICOH61x_ADC_VTHM_THL, val);
		/* Enable VBAT threshold Low interrupt */
		ricoh61x_set_bits(info->dev->parent, RICOH61x_INT_EN_ADC1, 0x20);
		/* Set VTHM threshold high voltage value = (voltage(V)*255)/(2.5) */
		...
		/* Enable VTHM threshold high interrupt */
		ricoh61x_set_bits(info->dev->parent, RICOH61x_INT_EN_ADC2, 0x20);
		if (bat_temp_vol > ((BATTERY_TEMP_8_VOL+info->temp_high_lv1_charge)/2))
...
static irqreturn_t adc_vsysl_isr(int irq, void *battery_info)
{
	...
	printk(KERN_ERR "PMU:%s\n", __func__);
	printk(KERN_ERR "PMU:%s Detect Low Battery Interrupt **********\n", __func__);
	// critical_low_flag = 1;
...
static irqreturn_t adc_vtherm_isr(int irq, void *battery_info)
{
	...
	printk(KERN_ERR "PMU:%s\n", __func__);
	printk(KERN_ERR "PMU:%s Detect Battery Temperature Interrupt **********\n", __func__);
			...
			struct device_attribute *attr, char *buf)
{
	...
	len = sprintf(buf+tot_len,"ORIGINAL_OCV_Tab[%d]\n",(int)info->num);tot_len += len;
	for (i = 0; i <= 10; i = i+1) {
		len = sprintf(buf+tot_len,"%d0p_voltage=%duV\n",
				  i, get_OCV_voltage(info, i, ORIGINAL)); tot_len += len;
	}

	len = sprintf(buf+tot_len,"USING_OCV_Tab[%d]\n",(int)info->num);tot_len += len;
	for (i = 0; i <= 10; i = i+1) {
		len = sprintf(buf+tot_len,"%d0p_voltage=%duV\n",
...
static DEVICE_ATTR(batt_params, 0644, show_batt_params, NULL);
...
static const char gszTestStates_temp_adc_offset[] = "temp_adc_offset";
static const char gszTestStates_temp_adc[] = "temp_adc";
...
static const char gszTestStates_prop_u10table_en[] = "u10ptable_en";
		       ...
		       const char *buf, size_t count)
{
	...
	else if(0==strcmp(cCmdA,gszTestStates_temp_adc_offset)) {
		
		giChgOffTempADC_offset = simple_strtol(cValA, NULL, 10);
		//printk("charge off temp adc offset=%d\n",giChgOffTempADC_offset);

	}
	else if(0==strcmp(cCmdA,gszTestStates_temp_adc)) {
		int bat_temp_vol;
		uint8_t val;

		ricoh61x_read(info->dev->parent, RICOH61x_ADC_VTHMDATAH, &val);
		bat_temp_vol = val*2500/255;
		printk(KERN_INFO "PMU:temperature ADC=%d (VTHMDATA 0x%02X)\n", 
				bat_temp_vol,val);
	}
	else if(0==strcmp(cCmdA,gszTestStates_prop_u10table_en)) {
		uint8_t val;
		val = (uint8_t)simple_strtol(cValA, NULL, 10);
		if(val) {
			giU10PtableType=val;
		}
		else {
			...
			struct device_attribute *attr, char *buf)
{
	...
	p += iLen;
	iLen = sprintf(p,"%s : threshold=%d,offset+(%d)\n",
			gszTestStates_temp_adc_offset,info->temp_high_lv2_stop_charge,giChgOffTempADC_offset);
	p += iLen;
	// next prop . 
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if (POWER_SUPPLY_STATUS_FULL == ret)
			if (49 == gptHWCFG->m_val.bPCB || gptHWCFG->m_val.bPCB>=68 || gptHWCFG->m_val.bPCB ==58) //E60QDX/models newer than E60QPX .   
	...
	case POWER_SUPPLY_PROP_TEMP:
		...
		if (info->soca->ready_fg) {
			if(49==gptHWCFG->m_val.bPCB || 69==gptHWCFG->m_val.bPCB || 77==gptHWCFG->m_val.bPCB
				||84==gptHWCFG->m_val.bPCB || 87==gptHWCFG->m_val.bPCB) {
				// E60QDx, E60QQX, EA0Q0x, E70Q5x , E70K02
				int bat_temp_val;
				int bat_temp_vol;
				...
				printk(KERN_DEBUG "PMU:%s Battery temperature %dmV (VTHMDATA 0x%02X)**********\n", __func__, bat_temp_vol);
				...
				else if ((info->temp_high_lv1_charge < bat_temp_vol))
					val->intval = 400-320*(bat_temp_vol-info->temp_high_lv1_charge)/(BATTERY_TEMP_8_VOL-info->temp_high_lv1_charge);
				else if ((info->temp_high_lv2_stop_charge < bat_temp_vol))
					val->intval = 480-80*(bat_temp_vol-info->temp_high_lv2_stop_charge)/(info->temp_high_lv1_charge-info->temp_high_lv2_stop_charge);
				else if ((info->temp_high_lv3_alarm < bat_temp_vol))
					val->intval = 500-20*(bat_temp_vol-info->temp_high_lv3_alarm)/(info->temp_high_lv2_stop_charge-info->temp_high_lv3_alarm);
				else
					val->intval = 510;
			}
			else
...
static __devinit int ricoh61x_battery_probe(struct platform_device *pdev)
{
	...
	info->temp_high_lv1_charge = BATTERY_TEMP_40_VOL;
	info->temp_high_lv2_stop_charge = BATTERY_TEMP_48_VOL;
	info->temp_high_lv3_alarm = BATTERY_TEMP_50_VOL;
	...
	switch(gptHWCFG->m_val.bPCB) {
		...
		case 84: //E70Q54
			...
			break;
		default:
			if( 87==gptHWCFG->m_val.bPCB ||/* >= E70K0X */ 
					86==gptHWCFG->m_val.bPCB ||/* >= E60QVX */ 
					74==gptHWCFG->m_val.bPCB ||/* >= E60K0X */ 
					75==gptHWCFG->m_val.bPCB ) /* >= E80K0X */ 
			{
				info->iEnableTempDetect = 1;
			}
			else if(gptHWCFG->m_val.bPCB>=93) 
			{ /* newer than E60K1X */
				info->iEnableTempDetect = 1;
			}
			else {
	...
	printk("%s() ibat_num=%d,TempDetect=%d\n", __func__, info->ibat_num,info->iEnableTempDetect);
...
//	info->battery.name = "battery";
	...
	ret = request_threaded_irq(charger_irq + RICOH61x_IRQ_FRTIMOVINT,
						NULL, charger_overtime_isr,
					IRQF_ONESHOT, "r5t61x_charger_timov",
								info);
	if (ret < 0) {
...
//						"r5t61x_adc_vsysl", info);
	...
	if(info->iEnableTempDetect) {
		...
		switch(gptHWCFG->m_val.bPCB) {
			case 75: // E80K0X 
				info->temp_high_lv1_charge = 852; // 38 degrees .
				info->temp_high_lv2_stop_charge = 745; // 44 degrees .
				info->temp_high_lv3_alarm = 545; // 50 degrees .
				break;
			default:
				break;
		}
...
//	}
	...
	add_test_states_sysfs(info->battery.dev);
	add_batt_params_sysfs(info->battery.dev);
...
static int __devexit ricoh61x_battery_remove(struct platform_device *pdev)
{
	...
	free_irq(charger_irq + RICOH61x_IRQ_FRTIMOVINT, &info);
...
static int calc_soc_by_currentMethod(struct ricoh61x_battery_info *info, unsigned long Period, bool *is_low_current)
{
	...
	if(info->soca->store_slp_state == 0) {
		...
		if (sleepCurrent < RICOH61x_SUS_CURRENT_THRESH) {
			static unsigned long sSuspendPeriod;
			...
			if (0 == info->soca->cc_delta) {
				sSuspendPeriod += Period;
				info->soca->cc_delta = calc_cc_value_by_sleepPeriod(info, sSuspendPeriod, sleepCurrent);
				if (-10 > info->soca->cc_delta)
					sSuspendPeriod = 0;
			}
			else {
					sSuspendPeriod = 0;
...
static int ricoh61x_battery_resume(struct device *dev)
{
	...
	printk(KERN_ERR "MM[%s %d]: info->soca->status:%d  info->soca->ready_fg:%d info->soca->displayed_soc: %d\n", __func__, __LINE__ , info->soca->status, info->soca->ready_fg, info->soca->displayed_soc);
	...
	if (info->entry_factory_mode) {
		...
		if (RICOH61x_SOCA_START == info->soca->status
			...
			if((info->soca->cc_delta - info->soca->suspend_cc) <= 0){
				...
				SUSPEND_PRINT(KERN_DEBUG "PMU: %s : soc_current(%d), soc_voltage(%d), Diff(%d), is_low_current(%d) ==========\n",
					 __func__, soc_current, soc_voltage, (soc_current - soc_voltage), is_low_current);

				// If difference is small, use current method. If not, use voltage method.
				if ((false == is_low_current) || ((24*60*60) > suspend_period_time) || ((soc_current - soc_voltage) < 1500) ) {
