...
static volatile int giDbgEnable;

DEFINE_SPINLOCK(ricoh619_spinlock);
				 ...
				 u8 reg, uint8_t val)
{
	...
	if(giDbgEnable) {
		dev_info(&client->dev, "ricoh61x: reg write  reg=0x%x, val=0x%x\n",
				reg, val);
	}
...
int ricoh619_power_off(void)
{
	...
	ricoh61x = i2c_get_clientdata(ricoh61x_i2c_client);

	if(!in_atomic_preempt_off()) {
		spin_lock(&ricoh619_spinlock);
		mutex_lock(&ricoh61x->io_lock);
	}
	...
	while (1)
	{
		...
		mdelay(1000);
		printk("%s() retry powering down ...\n",__func__);
	}

	if(!in_atomic_preempt_off()) {
		mutex_unlock(&ricoh61x->io_lock);
		spin_unlock(&ricoh619_spinlock);
	}
...
int ricoh619_restart(void)
{
	...
	if(__ricoh61x_read(ricoh61x_i2c_client, RICOH61x_PSWR, &reg_val)>=0) {
			reg_val |= 0x80;// RRESET=1 (RSTB to ERSTB , RSTB registers will not reset)
			__ricoh61x_write(ricoh61x_i2c_client, RICOH61x_PSWR, reg_val);
	}
	else {
		printk("%s(): PSWR read failed !!",__func__);
	}

	if(__ricoh61x_read(ricoh61x_i2c_client, RICOH61x_PWR_NOE_TIMSET, &reg_val)>=0) {
			printk(KERN_DEBUG"%s(): ONE_TIMSET=0x%02x\n",__func__,reg_val);
			reg_val &=~0x3; // clear OFF_PRESS_NOE fields value . 
			reg_val |= 0x3;// OFF_PRESS_NOE set to 3 secs .
			__ricoh61x_write(ricoh61x_i2c_client, RICOH61x_PWR_NOE_TIMSET, reg_val);
	}
	else {
		printk("%s(): N_OE_TIMESET read failed !!",__func__);
	}

	printk("%s(powering restart)",__func__);
	while(1) 
	{
		/* Repeat power ON after power off(Power Off/N_OE) */
		__ricoh61x_write(ricoh61x_i2c_client, RICOH61x_PWR_REP_CNT, 0x37);//reseto 16ms, 1000ms off time .

		/* Power OFF */
		__ricoh61x_write(ricoh61x_i2c_client, RICOH61x_PWR_SLP_CNT, 0x1);
		mdelay(1000);
		printk("%s(retry powering restart)",__func__);
	}
...
static int dbg_ricoh_show(struct seq_file *s, void *unused)
{
	...
	if(s) {
		...
		client = ricoh61x_i2c_client;

		printk("RICOH61x Registers\n");
		printk("------------------\n");
	}
...
static ssize_t regiset_read(struct device *dev, struct device_attribute *attr,char *buf)
{
	char reg_val, reg1_val;
	__ricoh61x_read(ricoh61x_i2c_client, 0xB6, &reg_val);
	 __ricoh61x_read(ricoh61x_i2c_client, 0xB7, &reg1_val);
	sprintf (buf, "reg 0xB6=0x%02X, reg 0xB7=0x%02X\n", reg_val, reg1_val);
	return strlen(buf);
}

static ssize_t regiset_write(struct device *dev, struct device_attribute *attr,
		       const char *buf, size_t count)
{
	...
	if (0xFF < val)
		...
		sprintf (buf, "reg 0xB6=0x%02X, reg 0xB7=0x%02X\n", val & 0x1F, val);
	  	...
	  	__ricoh61x_write(ricoh61x_i2c_client, 0xB6, val&0x1F);
	  	__ricoh61x_write(ricoh61x_i2c_client, 0xB7, val);
	}
	return strlen(buf);
}
...
{
	char reg_val;
	__ricoh61x_read(ricoh61x_i2c_client, 0xB3, &reg_val);
	sprintf (buf, "0x%02X\n", reg_val);
	return strlen(buf);
}

static ssize_t chgctl1_write(struct device *dev, struct device_attribute *attr,
		       const char *buf, size_t count)
{
	int val = simple_strtoul(buf,NULL,0);
	if (0xFF < val)
		...
		sprintf (buf, "reg 0xB3=0x%02X\n", val);
		
	  	__ricoh61x_write(ricoh61x_i2c_client, 0xB3, val);
	}
	return strlen(buf);
}
static ssize_t user_watchdog_read(struct device *dev, struct device_attribute *attr,char *buf)
{
	struct ricoh61x *ricoh = dev_get_drvdata(dev);

	sprintf(buf,"%d\n",ricoh->iWatchDogUserCounter);

	return strlen(buf);
}

static ssize_t user_watchdog_write(struct device *dev, struct device_attribute *attr,
		       const char *buf, size_t count)
{
	struct ricoh61x *ricoh = dev_get_drvdata(dev);
...
static DEVICE_ATTR(user_watchdog, 0666, user_watchdog_read, user_watchdog_write);
...
static void __init ricoh61x_debuginit(struct ricoh61x *ricoh)
{
	(void)debugfs_create_file("ricoh61x", S_IRUGO, NULL,
			ricoh, &debug_fops);
	if (device_create_file(ricoh->dev,&dev_attr_regiset)) {
	...
	if (device_create_file(ricoh->dev,&dev_attr_user_watchdog)) {
		pr_err("Can't create ricoh ntx attr sysfs !\n");
	}
}
#else
			      ...
			      const struct i2c_device_id *id)
{
	...
	ret = ricoh61x_read(ricoh61x->dev, RICOH61x_PWR_OFF_HIS, &val);
	if (ret < 0) {
		dev_err(ricoh61x->dev,"Error in reading PWER OFF HIS %d\n", ret);
		return ret;
	}
