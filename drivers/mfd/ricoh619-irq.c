...
static struct delayed_work sys_reboot_work;

static void sys_reboot_func(struct work_struct *work)
{
	printk("%s()\n",__func__);
	kernel_restart(0);
}
...
//						 __func__, i, int_sts[i], irq_mon_add[i]);
		...
		{
			u8 int_en_sys,pwron_times,wd;
			ricoh61x_read(ricoh61x->dev,RICOH61x_INT_EN_SYS, &int_en_sys);
			ricoh61x_read(ricoh61x->dev,RICOH61x_PWR_ON_TIMSET, &pwron_times);
			ricoh61x_read(ricoh61x->dev,RICOH61x_PWR_WD, &wd);
			printk(KERN_INFO"IN_TEN_SYS=0x%x,PWR_ON_TIMES=0x%x,int_sts=0x%x,wd=0x%x,when interrupt is comming\n",
					int_en_sys,pwron_times,int_sts[i],wd);
			if ((irq_clr_add[i] == RICOH61x_INT_IR_SYS) && (int_sts[i] & 0x40)) {
				isIrqHandled = 1;

				if(-1!=ricoh61x->iWatchDogUserCounter) {
					printk(KERN_INFO "PMU2: Watchdog timeout triggered.WatchDogUserCnt=%d ===\n",ricoh61x->iWatchDogUserCounter);
					if(0==ricoh61x->iWatchDogUserCounter--) {
#if 1
						//cancel_delayed_work_sync(&sys_reboot_work);
						if(!delayed_work_pending(&sys_reboot_work)) {
							schedule_delayed_work(&sys_reboot_work,1);
						}
#else
						panic("---WatchDogUserCount timeout---");
#endif
					}
				}
				else {
...
#ifdef WDOG_DISABLE_1ST_INT //[
				int_en_sys &= ~0x40;
				if( ricoh61x_write(ricoh61x->dev,RICOH61x_INT_EN_SYS,int_en_sys ) <0) {
					printk(KERN_ERR "%s():WDOG disable failed !\n", __func__);
				}
				else {
					printk(KERN_ERR "%s():WDOG disabled \n", __func__);
				}
#if !defined(INT_PWRON_OFF_ENABLE)//[
				if(ricoh61x_write(ricoh61x->dev, RICOH61x_PWR_REP_CNT, 0x00)<0) {//clear re-poweron timing
					printk(KERN_ERR "%s():clear WDOG timing failed !\n", __func__);
				}
#endif //]!defined(INT_PWRON_OFF_ENABLE)
			
#endif //] WDOG_DISABLE_1ST_INT
...
//				return IRQ_HANDLED;
			...
			if ((irq_clr_add[i] == RICOH61x_INT_IR_SYS) && (int_sts[i] & 0x20)) {
				isIrqHandled = 1;
				printk(KERN_INFO "PMU2: NOE_OFF triggered. ===============\n");
				//return IRQ_HANDLED;
			}

			if ((irq_clr_add[i] == RICOH61x_INT_IR_SYS) && (int_sts[i] & 0x01)) {
				isIrqHandled = 1;
				printk(KERN_INFO "PMU2: PWRON triggered. ===============\n");
				pwron_times |= 0x80;
				if( ricoh61x_write(ricoh61x->dev,RICOH61x_PWR_ON_TIMSET,pwron_times ) <0) {
					printk(KERN_ERR "%s():PWRON_TIME clear failed !\n", __func__);
				}
				//ricoh61x_set_bits(ricoh61x->dev,RICOH61x_PWR_ON_TIMSET,(0x1 <<7)); // clear PWRON off press . 
				//mdelay(10);printk(KERN_INFO"IRQ gpio=%d, cleared PWRON off \n",gpio_get_value(ricoh61x->irq_gpio));

				pwron_times &= ~0x80;
				if( ricoh61x_write(ricoh61x->dev,RICOH61x_PWR_ON_TIMSET,pwron_times ) <0) {
					printk(KERN_ERR "%s():PWRON_TIME active failed !\n", __func__);
				}
			...
			if ((irq_clr_add[i] == RICOH61x_INT_IR_SYS) && (int_sts[i] & 0x10)) {
				isIrqHandled = 1;
				iDoNotClrINT = 1;
				printk(KERN_INFO "PMU2: PWRON_OFF triggered. ===============\n");//
				if(ricoh61x_write(ricoh61x->dev, RICOH61x_PWR_REP_CNT, 0x00)<0) {//clear re-poweron timing
					printk(KERN_ERR "%s():clear WDOG timing failed !\n", __func__);
				}
#if 0
			...
			//printk(KERN_INFO"IRQ gpio=%d, interrupt handled \n",gpio_get_value(ricoh61x->irq_gpio));
		}
				...
				int irq_base)
{
	...
	INIT_DELAYED_WORK(&sys_reboot_work,sys_reboot_func);

	ricoh61x->iWatchDogUserCounter = -1;
...
#endif
	//ricoh61x->irq_en_reg[0] = 0x2e; // test enable int NOE/PREOT/PRVINDT/EXTIN. 
#ifdef INT_WDOG_ENABLE //[
	ricoh61x->irq_en_reg[0] |= 0x40; // enable WDOG . 
#endif //]INT_WDOG_ENABLE
#ifdef INT_PWRON_OFF_ENABLE //[
	ricoh61x->irq_en_reg[0] |= 0x11; // enable PWRON_OFF/PWRON . 
#endif //]INT_PWRON_OFF_ENABLE
