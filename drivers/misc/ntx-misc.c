...
#define NTX_IS_CHARGING		(gpio_is_valid(ntx_misc->chg_gpio) && gpio_get_value (ntx_misc->chg_gpio)?0:1)
#define NTX_ACIN_PG 		(gpio_is_valid(ntx_misc->acin_gpio)&&gpio_get_value (ntx_misc->acin_gpio)?0:1)
...
void fl_regulator_enable (int isEnable)
{
	...
	if (g_fl_regulator && !IS_ERR(g_fl_regulator) && (s_is_enabled != isEnable)) {
...
int msp430_battery(void)
{
	...
	if (gIsMSP430IntTriggered || !gLastBatValue || ((0 == gUSB_Change_Tick) && (time_after (jiffies, gLastBatTick)))) {
		...
		if (battValue>0) {
			...
			if (gpio_is_valid(ntx_misc->acin_gpio) && gpio_get_value (ntx_misc->acin_gpio) ) {// not charging
...
#endif//] CONFIG_MACH_MX6Q_NTX
	if (gpio_is_valid(ntx_misc->acin_gpio)&&!gpio_get_value (ntx_misc->acin_gpio)) {
		++g_acin_pg_debounce;
...
static int pmic_battery_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	g_acin_status = gpio_is_valid(ntx_misc->acin_gpio)&&gpio_get_value (ntx_misc->acin_gpio);
...
static int pmic_battery_resume(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(ntx_misc->acin_gpio) && g_acin_status != gpio_get_value (ntx_misc->acin_gpio)) {
...
static int pmic_battery_probe(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(ntx_misc->acin_gpio)) {
					...
					const struct i2c_device_id *id)
{
	...
	if( 1==gptHWCFG->m_val.bPMIC && 0!=gptHWCFG->m_val.bFrontLight) {
		...
		if (IS_ERR(g_fl_regulator)) {
			...
			if (IS_ERR(g_fl_regulator)) {
				g_fl_regulator = regulator_get(&client->dev, "vdd_fl_1_pwm");
				if (IS_ERR(g_fl_regulator)) {
					printk("%s, regulator \"vdd_fl_pwm\" not registered.(%d)\n", __func__, g_fl_regulator);
					//return -1;
				}
				else
					printk("%s, regulator found on channel 1\n", __func__);
			}
			else
