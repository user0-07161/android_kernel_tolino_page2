...
static int nfc_test_code(struct pn54x_dev *pn54x_dev)
{
	int ret = 0;
	...
	/* send reset code */
	ret = i2c_master_send(pn54x_dev->client, NCICoreReset, sizeof(NCICoreReset));
	...
	ret = recivce_nfc_ack_data(pn54x_dev->client, NCIResetAck, sizeof(NCIResetAck));
	...
	/* send NCI Core Init */
	ret = i2c_master_send(pn54x_dev->client, NCICoreInit, sizeof(NCICoreInit));
	...
	/* get version */
	ret = recivce_nfc_ack_data(pn54x_dev->client, NCIAck, sizeof(NCIAck));
		...
		const struct i2c_device_id *id)
{
	...
	ret = request_irq(client->irq, pn54x_dev_irq_handler,
				IRQF_TRIGGER_RISING, client->name, pn54x_dev);
	if (ret) {
...
#ifdef CONFIG_PM
static int nfc_pn54x_suspend(struct device *dev)
{
...
static int nfc_pn54x_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
...
static SIMPLE_DEV_PM_OPS(nfc_pn54x_pm, nfc_pn54x_suspend, nfc_pn54x_resume);
#endif
