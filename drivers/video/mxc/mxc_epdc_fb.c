...
#define MMDC_PROFILE 1

#define TCE_UNDERRUN_PREVENT_PATCH	2
#define TCE_UNDERRUN_PREVENT_PIXCLK	100000000
#define TCE_UNDERRUN_PREVENT_X_RES	1448
#define TCE_UNDERRUN_PREVENT_Y_RES	1072
#define TCE_UNDERRUN_RECOVERY	1

#define SW_RESET
#define MX6SL_QOS_ENABLED		1
...
uint32_t diff_time_stamp[2];

static struct hrtimer before_eof_hrtimer;
static ktime_t before_eof_ktime;


#if defined(CONFIG_SMP) && defined(CONFIG_MACH_MX6DL_NTX) //[
#define TCE_UNDERRUN_PREVENT_MCORE_PATCH 	0
#define EXTRA_CORES		0 //[0|1] 0 : disable , 1 : 1 extra core 
#endif //]defined(CONFIG_SMP) && defined(CONFIG_MACH_MX6DL_NTX)
...
//#endif //]FW_IN_RAM
	...
	struct completion before_eof_event;
	unsigned long ulJiffies_eof;
	int eof_sync_period;
	...
	u64 lut_status;
	u32 lut_submit_jiffies[64];
	u32 lut_submit_wfm[64];
...
#if (EXTRA_CORES>0)	//[
	//spinlock_t cores_lock[EXTRA_CORES];
	//struct mutex cores_active_mutex[EXTRA_CORES];
	int iLockSig[EXTRA_CORES] ;
	int iLockThreadStat[EXTRA_CORES] ;
	int iLastCPU;
#endif //] (EXTRA_CORES>0) 

	struct timeval submit_work_tv[2];
	int submit_work_tv_idx;
	

	struct completion working_buf_compl;
	int waiting_for_working_buf_compl;

	bool in_recovery;
	bool is_drop_updates;
	unsigned long ulJiffies_working_buf_busy;
	u32 wb_busy;
};
...
#if (EXTRA_CORES>0)	//[
typedef struct tagEXTRA_CORE_DATA {
	int iCoreIdx;
	struct mxc_epdc_fb_data *fb_data;
} EXTRA_CORE_DATA;
static int extra_cores_proc(EXTRA_CORE_DATA *I_ptExtra_core_data)
{
	int iCoreIdx = I_ptExtra_core_data->iCoreIdx;
	struct mxc_epdc_fb_data *fb_data = I_ptExtra_core_data->fb_data;
	int cpu;
	...
	if(iCoreIdx>=EXTRA_CORES) {
		printk(KERN_ERR"Core index(%d) should < %d\n",iCoreIdx,EXTRA_CORES);
		do_exit(0);
		return 0;
	}

	cpu = cpumask_next(fb_data->iLastCPU,cpu_online_mask);
	cpumask_set_cpu(cpu,&cpu_mask);
	
	iChk = sched_setaffinity(0,&cpu_mask);
	if(iChk<0) {
	...
	fb_data->iLockThreadStat[iCoreIdx]=1;

	EPDC_VPRINT(fb_data,4,"extra core[%d]/%d @ cpu:%d started !\n",iCoreIdx,EXTRA_CORES,cpu);
	for(;;)
	{
		EPDC_VPRINT(fb_data,6,"extra core[%d]/%d sleep !\n",iCoreIdx,EXTRA_CORES);
		//mutex_lock(&fb_data->cores_active_mutex[iCoreIdx]);
		while(!fb_data->iLockSig[iCoreIdx]) {
			msleep(10);
		}

		//cpu = get_cpu();
		EPDC_VPRINT(fb_data,6,"extra core[%d]/%d core lock@cpu%d !\n",
				iCoreIdx,EXTRA_CORES,raw_smp_processor_id());
		//spin_lock(&fb_data->cores_lock[iCoreIdx]);
		//smp_send_reschedule(0);
		
		fb_data->iLockThreadStat[iCoreIdx]=2;
		for(;fb_data->iLockSig[iCoreIdx];) {
			mdelay(10);
			schedule_timeout(1);
		}

		fb_data->iLockThreadStat[iCoreIdx]=3;
		//put_cpu();

		EPDC_VPRINT(fb_data,6,"extra core[%d]/%d @cpu%d core unlocked !\n",
				iCoreIdx,EXTRA_CORES,raw_smp_processor_id());
		//spin_unlock(&fb_data->cores_lock[iCoreIdx]);
		
		//mutex_unlock(&fb_data->cores_active_mutex[iCoreIdx]);
	}
	EPDC_VPRINT(fb_data,4,"extra core[%d]/%d stoped !\n",iCoreIdx,EXTRA_CORES);

	fb_data->iLockThreadStat[iCoreIdx]=0;

	do_exit(0);
	return 0;
}

static int init_extra_cores(struct mxc_epdc_fb_data *fb_data)
{
	int iChk,iRet;
	int i;
	static EXTRA_CORE_DATA tExtra_core_data;

	iRet = 0;


	fb_data->iLastCPU = cpumask_first(cpu_online_mask);
	for(i=0;i<EXTRA_CORES;i++) {
		//mutex_init(&fb_data->cores_active_mutex[i]);
		//mutex_lock(&fb_data->cores_active_mutex[i]);
		//fb_data->cores_lock[i] = __SPIN_LOCK_UNLOCKED(fb_data->cores_lock[i]);
		tExtra_core_data.iCoreIdx = i;
		tExtra_core_data.fb_data = fb_data;
		iChk = kernel_thread(extra_cores_proc,&tExtra_core_data,CLONE_KERNEL);
		if(iChk<0) {
			printk(KERN_ERR"create extra core[%d] thread failed !\n",i);
		}
...
static void start_extra_cores(struct mxc_epdc_fb_data *fb_data)
{
#if 0
	int i;
	//int iChk;
	...
	for(i=0;i<EXTRA_CORES;i++) {
		mutex_unlock(&fb_data->cores_active_mutex[i]);
	}
#endif
}
static void stop_extra_cores(struct mxc_epdc_fb_data *fb_data)
{
#if 0
	int i;
	for(i=0;i<EXTRA_CORES;i++) {
...
static void lock_extra_cores(struct mxc_epdc_fb_data *fb_data)
{
	int i;
	//EPDC_VPRINT(fb_data,6,"%s()\n",__FUNCTION__);
	for(i=0;i<EXTRA_CORES;i++) {
		//spin_lock(&fb_data->cores_lock[i]);
		fb_data->iLockSig[i] = 1;
	}
}
static void unlock_extra_cores(struct mxc_epdc_fb_data *fb_data)
{
	int i;
	//EPDC_VPRINT(fb_data,6,"%s()\n",__FUNCTION__);
	for(i=0;i<EXTRA_CORES;i++) {
		//spin_unlock(&fb_data->cores_lock[i]);
		fb_data->iLockSig[i] = 0;
	}
}
#else //][!(EXTRA_CORES>0)
#define init_extra_cores(fb_data)		0
#define start_extra_cores(fb_data)		
#define stop_extra_cores(fb_data)		
#define lock_extra_cores(fb_data)		
#define unlock_extra_cores(fb_data)		
#endif //] (EXTRA_CORES>0)
...
static long get_timediff(struct timeval tv1, struct timeval tv2)
{
	/*
	printk("tv1 %ld, %ld.    tv2 %ld, %ld.\n",
...
#define MMDC_PROFILE_START	1
#define MMDC_PROFILE_STOP		0
#define MMDC_PROFILE_CALC		2

#ifdef MMDC_PROFILE //[
static void __iomem *mmdc0_base;
#define MMDC0_MAPPCR0        (mmdc0_base + 0x410)
#define MMDC0_MAPPCR1        (mmdc0_base + 0x414)
#define MMDC0_MADPSR0        (mmdc0_base + 0x418)
#define MMDC0_MADPSR1        (mmdc0_base + 0x41C)
#define MMDC0_MADPSR2        (mmdc0_base + 0x420)
#define MMDC0_MADPSR3        (mmdc0_base + 0x424)
#define MMDC0_MADPSR4        (mmdc0_base + 0x428)
#define MMDC0_MADPSR5        (mmdc0_base + 0x42C)

#if 0
static void __iomem *mmdc1_base;
#define MMDC1_MAPPCR0        (mmdc1_base + 0x410)
#define MMDC1_MAPPCR1        (mmdc1_base + 0x414)
#define MMDC1_MADPSR0        (mmdc1_base + 0x418)
#define MMDC1_MADPSR1        (mmdc1_base + 0x41C)
#define MMDC1_MADPSR2        (mmdc1_base + 0x420)
#define MMDC1_MADPSR3        (mmdc1_base + 0x424)
#define MMDC1_MADPSR4        (mmdc1_base + 0x428)
#define MMDC1_MADPSR5        (mmdc1_base + 0x42C)
#endif


static inline int mmdc_profile(int iType,const char *title)
{

	static int mmdc_profiling ;
	static unsigned long mmdc_profile_start_jiffies ;
	...
	if(MMDC_PROFILE_START==iType)
	{
		...
		// EPDC 11'b0000xxxx011
		// PXP  11'b0000xxxx110
		// GPU  11'b00xxxxxx111

		//__raw_writel(0x00000000, MMDC0_MAPPCR1);
		__raw_writel(0x3, MMDC0_MAPPCR1); // EPDC 11'b0000xxxx011
		__raw_writel(0xA, MMDC0_MAPPCR0);
		//start profiling
		__raw_writel(0x1, MMDC0_MAPPCR0);

		mmdc_profile_start_jiffies = jiffies ;

		mmdc_profiling = 1;
		ret = 1;
	}
	else if(MMDC_PROFILE_STOP==iType || MMDC_PROFILE_CALC==iType){
		int data_load;
		unsigned int busy_cycles;
		unsigned int total_cycles;
		unsigned int mmdc0_mappcr0_org,mmdc0_mappcr0_tmp;
		unsigned long jiffies_profile_curr;
		...
		// load result 
		mmdc0_mappcr0_org = __raw_readl(MMDC0_MAPPCR0);
		mmdc0_mappcr0_tmp = mmdc0_mappcr0_org|0x4; // pause
		__raw_writel(mmdc0_mappcr0_tmp, MMDC0_MAPPCR0); 
		total_cycles = __raw_readl(MMDC0_MADPSR0);
		busy_cycles = __raw_readl(MMDC0_MADPSR1);

		if(MMDC_PROFILE_STOP==iType) {
			mmdc0_mappcr0_tmp = mmdc0_mappcr0_org&~0x1; // stop
			__raw_writel(mmdc0_mappcr0_tmp, MMDC0_MAPPCR0); 
			mmdc_profiling = 0;
		}
		else {
			__raw_writel(mmdc0_mappcr0_org, MMDC0_MAPPCR0); 
		}

		data_load = ((busy_cycles*100)/total_cycles);

		jiffies_profile_curr = jiffies ;

#if 0
		EPDC_VPRINT(g_fb_data,4,"[mmdc profile] %s : epdc in mmdc loading %d%% in %dms \n",
				title,data_load,jiffies_to_msecs(jiffies_profile_curr-mmdc_profile_start_jiffies));
#endif
...
static inline void epdc_set_temp(u32 temp)
{
	...
	EPDC_VPRINT(g_fb_data,3,"%s(),temp_idx=%hd\n",__FUNCTION__,temp);
}
...
{
	EPDC_VPRINT(g_fb_data,2,"%s(),x=%hd,y=%hd\n",__FUNCTION__,x,y);
...
{
	EPDC_VPRINT(g_fb_data,2,"%s(),w=%hd,h=%hd\n",__FUNCTION__,width,height);
			       ...
			       bool use_test_mode, u32 np_val)
{
	u32 reg_val = 0;
#if 0
	if(g_fb_data && (1==g_fb_data->tce_underrun_occurred) )
	{
		EPDC_VPRINT(g_fb_data,0,"%s skipped when tce underrun recovering .\n",__FUNCTION__)
		return ;
	}
#endif

	mmdc_profile(MMDC_PROFILE_STOP,__func__);
	mmdc_profile(MMDC_PROFILE_START,__func__);
	...
	if(0!=waveform_mode) {
		g_fb_data->lut_w[lut_num] = g_fb_data->latest_update_region.width;
		g_fb_data->lut_h[lut_num] = g_fb_data->latest_update_region.height;
		...
		g_fb_data->lut_submit_jiffies[lut_num] = jiffies;
		g_fb_data->lut_submit_wfm[lut_num] = waveform_mode;
	}
	...
	diff_time_stamp[0] = get_uSecs();

  //  DBG_MSG("\e[31m[%s %d] submit EPDC_UPD_CTRL\e[0m\n",__func__, __LINE__);
	...
	if(g_fb_data) {
		...
		g_fb_data->luts_updating |= (u64)1<<lut_num;
	}
...
static void epdc_init_settings_ex(struct mxc_epdc_fb_data *fb_data,int iProcType)
{
	...
	reg_val = EPDC_FIFOCTRL_ENABLE_PRIORITY | 
	    ((200 << EPDC_FIFOCTRL_FIFO_INIT_LEVEL_OFFSET) &
	...
	EPDC_VPRINT(fb_data,1,"hwcfg@(%p),display bus width=0x%02x\n",gptHWCFG,gptHWCFG->m_val.bDisplayBusWidth);
	...
	switch(gptHWCFG->m_val.bDisplayBusWidth) 
	{
		...
		*/
		...
		EPDC_VPRINT(fb_data,1,"16bits display bus width \n");
		reg_val =
		...
		EPDC_VPRINT(fb_data,1,"8bits mirror display bus width \n");
		...
		EPDC_VPRINT(fb_data,1,"16bits mirror display bus width \n");
	...
	default:
		...
		EPDC_VPRINT(fb_data,1,"8bits normal display bus width \n");
		/*
		 * EPDC_TCE_CTRL
		 * VSCAN_HOLDOFF = 4
		 * VCOM_MODE = MANUAL
...
static void _pmic_on(void)
{
	if(8==gptHWCFG->m_val.bDisplayCtrl) {
		...
		int iChk;
		iChk = sy7636_output_power(1,0);
		if(iChk<0) {
			printk(KERN_ERR "%s(%d):[warning] output power from SY7636 fail ,errno=%d !\n",
				__FILE__,__LINE__,iChk);
		}
	}
	else 
...
//	fb_data->updates_active = true;
	...
	if(fb_data->waveform_vcd_buffer) {
		...
		if (fb_data->wfm < 256) {
			...
			if(19==gptHWCFG->m_val.bDisplayCtrl || 22==gptHWCFG->m_val.bDisplayCtrl) {
				sy7636_vcom_get_cached(&vcom_mV);
			}
			else {
...
#ifdef TCE_UNDERRUN_PREVENT_MCORE_PATCH // .
	// stop cores to avoid TCE underrun
	...
	{
		lock_extra_cores(fb_data);
		start_extra_cores(fb_data);
	}
#endif //]TCE_UNDERRUN_PREVENT_MCORE_PATCH
...
static void epdc_powerdown(struct mxc_epdc_fb_data *fb_data)
{
	...
	if( fb_data->cur_mode->vmode->pixclock >= TCE_UNDERRUN_PREVENT_PIXCLK && 
			...
			fb_data->cur_mode->vmode->yres>=TCE_UNDERRUN_PREVENT_Y_RES )
	{
		unlock_extra_cores(fb_data);
		stop_extra_cores(fb_data);
	}
#endif //]TCE_UNDERRUN_PREVENT_MCORE_PATCH
	...
	if(19==gptHWCFG->m_val.bDisplayCtrl || 22==gptHWCFG->m_val.bDisplayCtrl) {
		int iChk;
		iChk = sy7636_output_power(0,0);
		if(iChk<0) {
			printk(KERN_ERR "%s(%d):[warning] disable power from SY7636 fail ,errno=%d !\n",
				__FILE__,__LINE__,iChk);
		}
	}
	else 
...
int mxc_epdc_wait_for_frameend(struct fb_info *info) {
	...
	if(time_is_before_jiffies(fb_data->ulJiffies_eof))
	{
    EPDC_VPRINT(fb_data,5,"\e[35m[%s %d]&fb_data->before_eof_event:%x\e[0m\n",__func__, __LINE__,&fb_data->before_eof_event);

		ret = wait_for_completion_timeout(&fb_data->before_eof_event,msecs_to_jiffies(100));

    if (!ret) {
    	dev_err(fb_data->dev, "Missed before_eof_event event!\n");
    	hrtimer_cancel(&before_eof_hrtimer);
    }
	}
...
EXPORT_SYMBOL( mxc_epdc_wait_for_frameend);
				   ...
				   struct mxc_epdc_fb_data *fb_data)
{
...
#endif
	{
		...
		if (hist_stat & 0x1) {
...
#else
					upd_desc_list->upd_data.waveform_mode =
						giNTX_waveform_modeA[NTX_WFM_MODE_GLD16];
#endif
				}
				else {
					#if 1
					upd_desc_list->upd_data.waveform_mode =
						giNTX_waveform_modeA[NTX_WFM_MODE_GLR16];
					#else
					upd_desc_list->upd_data.waveform_mode =
			...
			if(fb_data->buf_pix_fmt == EPDC_FORMAT_BUF_PIXEL_FORMAT_P5N) {
				if( (upd_desc_list->upd_data.waveform_mode==fb_data->wv_modes.mode_aa)&& 
						(fb_data->wv_modes.mode_gl16!=fb_data->wv_modes.mode_aa) )
				{
					upd_desc_list->is_aa=1;
					upd_data_list->update_desc->upd_data.flags &= ~EPDC_FLAG_USE_AAD;
				}
				else if((upd_desc_list->upd_data.waveform_mode==fb_data->wv_modes.mode_aad)&& 
						(fb_data->wv_modes.mode_gc16!=fb_data->wv_modes.mode_aad))
				{
					upd_desc_list->is_aa=1;
					upd_data_list->update_desc->upd_data.flags |= ~EPDC_FLAG_USE_AAD;
				}
				...
				struct mxc_epdc_fb_data *fb_data)
{
	...
	EPDC_VPRINT(fb_data,3,"%s():upd rect merge,a@%d(%d,%d,%d,%d)+b@%d(%d,%d,%d,%d)=>a(%d,%d,%d,%d)\n",
...
static inline long epdc_submit_work_diffus(struct mxc_epdc_fb_data *fb_data)
{
	long diffus = 0;
	int idx_curr=fb_data->submit_work_tv_idx%2;
	int idx_prev=idx_curr?0:1;
	diffus = get_timediff(fb_data->submit_work_tv[idx_curr],
			fb_data->submit_work_tv[idx_prev]);
	return diffus;
}
static inline long epdc_submit_work_curr_diffus(struct mxc_epdc_fb_data *fb_data)
{
	struct timeval tv;
	int idx_curr=fb_data->submit_work_tv_idx%2;
	long diffus = 0;
	do_gettimeofday(&tv);
	diffus = get_timediff(tv,fb_data->submit_work_tv[idx_curr]);
	return diffus;
}
...
static void epdc_submit_work_func(struct work_struct *work)
{
	...
	if (fb_data->is_drop_updates) {
		EPDC_VPRINT(fb_data,0,"%s(%d) dropping updates !!\n",__FUNCTION__,__LINE__);
		return ;
	}
	...
	list_for_each_entry_safe(next_update, temp_update,
		 ...
		 */
		next_update->update_desc->upd_data.update_mode = UPDATE_MODE_PARTIAL;
		...
		if (!upd_data_list) {
			...
			case MERGE_OK:GALLEN_DBGLOCAL_RUNLOG(4);
				...
				EPDC_VPRINT(fb_data,3,"Update merged [collision]\n");
			...
			case MERGE_FAIL:GALLEN_DBGLOCAL_RUNLOG(5);
				...
				EPDC_VPRINT(fb_data,3,"Update not merged [collision]\n");
				break;
			case MERGE_BLOCK:GALLEN_DBGLOCAL_RUNLOG(6);
				dev_dbg(fb_data->dev,
					"Merge blocked [collision]\n");
				EPDC_VPRINT(fb_data,3,"Merge blocked [collision]\n");
				end_merge = true;
				break;
			}
	...
	if (!((fb_data->upd_scheme == UPDATE_SCHEME_QUEUE) &&
		...
		*/
		if (!upd_data_list &&
			...
			mutex_unlock(&fb_data->queue_mutex);
			GALLEN_DBGLOCAL_ESC();
			dev_warn(fb_data->dev,"no free buffers !!\n");
			return;
		}
		...
		list_for_each_entry_safe(next_desc, temp_desc,
			...
			if (!upd_data_list) {
				...
				case MERGE_OK:GALLEN_DBGLOCAL_RUNLOG(14);
					dev_dbg(fb_data->dev,
						"Update merged [queue]\n");
					EPDC_VPRINT(fb_data,3,"Update merged [queue]\n");
					list_del_init(&next_desc->list);
					kfree(next_desc);
					break;
				case MERGE_FAIL:GALLEN_DBGLOCAL_RUNLOG(15);
					...
					EPDC_VPRINT(fb_data,3,"Update not merged [queue]\n");
					break;
	...
	if (!upd_data_list) {
		mutex_unlock(&fb_data->queue_mutex);
		GALLEN_DBGLOCAL_ESC();
		EPDC_VPRINT(fb_data,3,"update list empty !!\n");
		return;
	}
	...
	submit_work_diffus = epdc_submit_work_curr_diffus(fb_data);
	//EPDC_VPRINT(fb_data,1,"%s:submitting work diffus=%d\n",__func__,submit_work_diffus);
	/*
	...
	if ((fb_data->epdc_fb_var.rotate == FB_ROTATE_UR) &&
		...
		do_gettimeofday(&fb_data->submit_work_tv[++fb_data->submit_work_tv_idx%2]);
        ...
        if(  ((upd_data_list->update_desc->upd_data.waveform_mode!=fb_data->wv_modes.mode_a2)&&
              (upd_data_list->update_desc->upd_data.waveform_mode!=fb_data->wv_modes.mode_du)) &&
              fb_data->cur_mode->vmode->pixclock >= TCE_UNDERRUN_PREVENT_PIXCLK &&
				fb_data->cur_mode->vmode->xres>=TCE_UNDERRUN_PREVENT_X_RES && 
				fb_data->cur_mode->vmode->yres>=TCE_UNDERRUN_PREVENT_Y_RES )
		{
			int i,iMaxWaitCnt=500;
			//int iUPD_RES = fb_data->active_updating_w*fb_data->active_updating_h;
			int iUPD_RES = TCE_UNDERRUN_PREVENT_X_RES*fb_data->active_updating_h;
			u64 lut_status = fb_data->lut_status;
			int iActLuts;
			u32 dwBlockFlags=0;
			int epdc_loading,block_loading=50 ;
			int queue_locked = 0;
			int ilock_lvl=0;
			...
			epdc_loading = mmdc_profile(MMDC_PROFILE_CALC,__func__);

			if( iUPD_RES >= ((TCE_UNDERRUN_PREVENT_X_RES*TCE_UNDERRUN_PREVENT_Y_RES)/iActLuts)
			...
			if(epdc_loading>=block_loading) {
				EPDC_VPRINT(fb_data,1,"epdc block L1:%d%%\n",epdc_loading);
				dwBlockFlags |= 0x2;
				ilock_lvl = 1;
			}

			if((submit_work_diffus<1000) && (dwBlockFlags&0x1)) {
				dwBlockFlags |= 0x4;
			}
			...
			if(dwBlockFlags)
			{
				...
				EPDC_VPRINT(fb_data,1,"%s():blkflags=0x%x,loading=%d%%,waiting for lut%d/%d complete to submit update ...\n",
					__FUNCTION__,dwBlockFlags,epdc_loading,fb_data->lastest_lut_num,iActLuts);


				for(i=0;i<iMaxWaitCnt;i++) {
					epdc_loading = mmdc_profile(MMDC_PROFILE_CALC,__func__);
					/*
					if(0!=fb_data->tce_underrun_occurred) {
						EPDC_VPRINT(fb_data,0,"%s() skipped when tce underrun recovering \n",__FUNCTION__);
						GALLEN_DBGLOCAL_ESC();
						return;
					}
					*/
					if(epdc_loading>=block_loading+20) {
						if(3!=ilock_lvl) {
							EPDC_VPRINT(fb_data,1,"epdc blocking L3:%d%%\n",epdc_loading);
						}
						ilock_lvl=3;
					}
					else 
					if(epdc_loading>=block_loading+10) {
						if(2!=ilock_lvl) {
							EPDC_VPRINT(fb_data,1,"epdc blocking L2:%d%%\n",epdc_loading);
						}
						ilock_lvl=2;
					}
					else {
						if(1!=ilock_lvl) {
							EPDC_VPRINT(fb_data,1,"epdc blocking L1:%d%%\n",epdc_loading);
						}
						ilock_lvl=1;
					}

					if(ilock_lvl>=2) {
						// level 2 : disable user space enqueue .
						if(!queue_locked) { mutex_lock(&fb_data->queue_mutex);queue_locked = 1;}
					}
					else {
						// level 1 : enable user space enqueue .
						if(queue_locked) { mutex_unlock(&fb_data->queue_mutex);queue_locked = 0;}
					}

					EPDC_VPRINT(fb_data,1,"%d%%",epdc_loading);
#if 0
					// system will halt .
				...
				if(i>=iMaxWaitCnt) {
					EPDC_VPRINT(fb_data,0,"\n%s(): waiting for lut%d complete %dms timeout !!\n",__FUNCTION__,fb_data->lastest_lut_num,i);
				}
				else {
					//EPDC_VPRINT(fb_data,ilock_lvl>1?0:1,"\n%s(): lut%d wait %dms done, submitting update !!\n",__FUNCTION__,fb_data->lastest_lut_num,i);
		...
		if(fb_data->in_recovery) {
			dev_warn(fb_data->dev,"%s() skipped when epdc recovering \n",__func__);
			return ;
		}
	...
	if (fb_data->cur_update != NULL) {
		GALLEN_DBGLOCAL_RUNLOG(22);
		EPDC_VPRINT(fb_data,3,"working buf busy!\n");
		fb_data->ulJiffies_working_buf_busy = jiffies;
	 ...
	 */
	EPDC_VPRINT(fb_data,3,"%s(%d):upd_order=%d,is_aa=%d,wb_pause=%d,flags=0x%x\n",
		__FUNCTION__,__LINE__,
		upd_data_list->update_desc->update_order,	
	...
	if(fb_data->in_recovery) {
		dev_warn(fb_data->dev,"%s(%d) skipped when epdc recovering \n",__func__,__LINE__);
		mutex_unlock(&fb_data->queue_mutex);
		return ;
	}
	...
	if (upd_data_list->update_desc->is_aa &&
		...
		if(FB_ROTATE_CW==fb_data->epdc_fb_var.rotate || 
			FB_ROTATE_CCW==fb_data->epdc_fb_var.rotate)
		{
		...
		EPDC_VPRINT (fb_data,3," --- AAl(v2.2.0) Algo Exec Time = %d\n", getTimeDiff(aa_time_stamp[0], aa_time_stamp[1]));
	  ...
	  aa_time_stamp[0] = get_uSecs();
		...
		/* collect the system time data */
		aa_time_stamp[1] = get_uSecs();
		EPDC_VPRINT (fb_data,3," --- AA-D(v2.1.0) Algo Exec Time = %d\n", getTimeDiff(aa_time_stamp[0], aa_time_stamp[1]));
	...
	if( g_fb_data->rev != 20) {
		...
		dev_dbg(fb_data->dev, "upd_region: l = %d, "
						"t = %d, w = %d, h = %d\n",
						upd_region->left, upd_region->top,
	...
	if (!epdc_any_luts_available()) {
		...
		EPDC_VPRINT(fb_data,4, "no luts available!\n");
	...
	if (ret && fb_data->tce_prevent && (fb_data->rev < 20)) {
		...
		dev_warn(fb_data->dev, "Waiting for LUT15\n");
	...
	if(fb_data->in_recovery) {
		dev_warn(fb_data->dev,"%s(%d) submit_update skipped when epdc recovering \n",__func__,__LINE__);
	}
	else {
#if 1

    if ( upd_data_list->update_desc->upd_data.flags & EPDC_FLAG_ENABLE_FRAMEEND_INT ) {
        DBG_MSG("\e[34m[%s %d] initial eof intr\e[0m\n",__func__, __LINE__);

        init_completion(&fb_data->eof_event);
		epdc_eof_intr(true);

		DBG_MSG("[%s %d] initial eof intr wait end\n",__func__, __LINE__);
    }
#endif
				   ...
				   struct fb_info *info)
{
	...
	EPDC_VPRINT(fb_data,1,"\e[32m%s():upd update mode=%s,wf mode=%d,left=%d,top=%d,w=%d,h=%d,flag=0x%x,marker=%d,jiffies=%u\e[0m\n",
			__FUNCTION__,
	...
	if ( upd_data->flags & EPDC_FLAG_ENABLE_FRAMEEND_INT ) {
	    DBG_MSG("[%s %d]EPDC_FLAG_ENABLE_FRAMEEND_INT\n",__func__, __LINE__);
		init_completion(&fb_data->before_eof_event);
		fb_data->ulJiffies_eof = jiffies;
	}
	...
	if(0==upd_data->update_region.width||0==upd_data->update_region.height) {
		EPDC_VPRINT(fb_data,3,
			"Update region width/heigth cannot zero ,aborting update.\n");
		GALLEN_DBGLOCAL_ESC();
		return 0;
	}
	...
	if(fb_data->wb_busy) {
		if(time_is_before_jiffies(fb_data->ulJiffies_working_buf_busy+60)) {
			EPDC_VPRINT(fb_data,0,"epdc recover after 600ms working buf busy \n");
			//fb_data->verbose_lvl = 20;
...
#if 0
	if(TCE_STATE_NORMAL==giTCE_State)
	{
		int epdc_loading,block_loading=70 ;
		int ilock_lvl=0;
		...
		do {
			...
			if(epdc_loading>=block_loading) {
				if(!ilock_lvl) {
					EPDC_VPRINT(fb_data,0,"%s(%d):epdc loading %d +++\n",__FUNCTION__,__LINE__,epdc_loading);
					ilock_lvl = 1;
				}
			}
			else {
			...
			if(epdc_loading>fb_data->epdc_loading) {
				mdelay(10);
			}
			else {
				msleep(10);
			...
			fb_data->epdc_loading = epdc_loading;
		}while(ilock_lvl);
	...
	if (fb_data->upd_scheme != UPDATE_SCHEME_SNAPSHOT) {
		...
		queue_work_on(0,fb_data->epdc_submit_workqueue,
						...
						struct fb_info *info)
{
	...
	EPDC_VPRINT(fb_data,2,"%s() : fb_info@%p,marker_data@%p,update_marker=%d\n",
	...
	EPDC_VPRINT(fb_data,3,"%s(): waiting marker %d for completion\n",__FUNCTION__,next_marker->update_marker);
			     ...
			     unsigned long arg)
{
	...
	case MXCFB_SEND_UPDATE:GALLEN_DBGLOCAL_RUNLOG(4);
		//printk("\n==> MXCFB_SEND_UPDATE\n");															 
		{
			...
			if(g_fb_data->is_drop_updates) {
				EPDC_VPRINT(g_fb_data,0,"skip sending update MXCFB_SEND_UPDATE when dropping updates \n");
				return 0;
			}
	...
	case MXCFB_SEND_UPDATE_ORG:GALLEN_DBGLOCAL_RUNLOG(4);
		//printk("\n==> MXCFB_SEND_UPDATE\n");															 
		{
			...
			/* just drop out when reset */
			if(g_fb_data->is_drop_updates) {
				EPDC_VPRINT(g_fb_data,0,"skip sending update MXCFB_SEND_UPDATE_ORG when dropping updates \n");
				return 0;
			}
...
#else//][!MX50_IOCTL_IF
	case MXCFB_WAIT_FOR_UPDATE_COMPLETE2:
    ...
    case MXCFB_WAIT_FOR_FRAMEEND:
		//printk("\n==>skip MXCFB_GET_PWRDOWN_DELAY\n");ret=0;break;															 
		{
            DBG_MSG("[%s %d]\n",__func__, __LINE__);
            ret = mxc_epdc_wait_for_frameend(info);
...
static irqreturn_t mxc_epdc_irq_handler(int irq, void *dev_id)
{
	...
	EPDC_VPRINT(fb_data,20,"\e[36m[%s %d] ints_fired:%x luts1_ints_fired:%x luts2_ints_fired:%x\e[0m\n",__func__, __LINE__,ints_fired,luts1_ints_fired,luts2_ints_fired);

	if ( !fb_data->in_recovery && epdc_is_working_buffer_complete()) {
    	diff_time_stamp[1] = get_uSecs();
    	uint32_t epdc_upd_ctrl_to_wb_complete_timediff = getTimeDiff(diff_time_stamp[0], diff_time_stamp[1]) ;
    	EPDC_VPRINT(fb_data,20,"WB process time = %d\n", epdc_upd_ctrl_to_wb_complete_timediff);
    	if ( __raw_readl(EPDC_IRQ_MASK) & EPDC_IRQ_FRAME_END_IRQ ) {
      		if ( epdc_upd_ctrl_to_wb_complete_timediff < 100  ) {
      		    DBG_MSG("cancel before_eof_hrtimer\n");
       		    if ( !epdc_luts_status(fb_data->rev) ) {
    				DBG_MSG("complete before_eof_event\n");
	                complete(&fb_data->before_eof_event);
				}
        	    hrtimer_cancel(&before_eof_hrtimer);
        	    hrtimer_start( &before_eof_hrtimer, before_eof_ktime, HRTIMER_MODE_REL );
							fb_data->ulJiffies_eof = jiffies;
            }
        }
    }
	...
	if (__raw_readl(EPDC_IRQ) & EPDC_IRQ_TCE_UNDERRUN_IRQ) {
		...
		if(!fb_data->in_recovery) {
			epdc_loading = mmdc_profile(MMDC_PROFILE_CALC,__func__);
			dev_err(fb_data->dev,
				"TCE underrun! Will continue to update panel,irq_stat=0x%x,loading=%d%%,diffus=%ld,jiffies=%u,TCE_state=%d\n",
				__raw_readl(EPDC_IRQ),epdc_loading,epdc_submit_work_diffus(fb_data),jiffies,giTCE_State);


			for(i=0;i<64;i++) {
				if(fb_data->luts_updating&(u64)(1ULL<<i)) {
					dev_err(fb_data->dev,"%s():lut %d updating ,w=%d,h=%d,jiffies=%u,wfm=%d\n",
							__func__,i,fb_data->lut_w[i],fb_data->lut_h[i],fb_data->lut_submit_jiffies[i],fb_data->lut_submit_wfm[i]);
				}
			}
		
			/* Clear TCE underrun IRQ */
			__raw_writel(EPDC_IRQ_TCE_UNDERRUN_IRQ, EPDC_IRQ_CLEAR);
			fb_data->tce_underrun_occurred=1;
			//fb_data->verbose_lvl=100;
			giTCE_State = TCE_STATE_UNDERRUN;
		}
		else {
			dev_warn(fb_data->dev,"TCE underrun in recovery ...\n");
		}
	}
	...
	if (epdc_signal_eof()) {
		GALLEN_DBGLOCAL_RUNLOG(7);
		EPDC_VPRINT(fb_data,20,"\e[33m[%s %d] frame end\e[0m\n",__func__, __LINE__);
		if(1!=fb_data->tce_underrun_occurred) {
		hrtimer_start( &before_eof_hrtimer, before_eof_ktime, HRTIMER_MODE_REL );
		}
	...
	EPDC_VPRINT(fb_data,20, "EPDC interrupts fired = 0x%x, "
...
static void epdc_recovery(struct mxc_epdc_fb_data *fb_data)
{
	...
	printk("resending last rect update ...\n");
...
static void epdc_intr_work_func(struct work_struct *work)
{
	...
	epdc_wb_busy = fb_data->wb_busy = epdc_is_working_buffer_busy();
	epdc_irq0_stat = __raw_readl(EPDC_IRQ);
	...
	//if(fb_data->tce_underrun_occurred) 
	{
		EPDC_VPRINT(fb_data,10,"irq_stat=0x%llx,irq0_stat=0x%x\n",epdc_irq_stat,epdc_irq0_stat);
		EPDC_VPRINT(fb_data,2,"luts_status=0x%llx,updating=0x%llx\n",epdc_luts_status(fb_data->rev),fb_data->luts_updating);
		EPDC_VPRINT(fb_data,10,"upd_done=0x%x\n",epdc_upd_done);
		EPDC_VPRINT(fb_data,10,"luts_avail=0x%x\n",epdc_luts_avail);
		EPDC_VPRINT(fb_data,10,"luts_canelled=0x%x\n",epdc_lut_cancelled);
		EPDC_VPRINT(fb_data,10,"epdc_collision=0x%x\n",epdc_collision);
		EPDC_VPRINT(fb_data,2,"wb_busy=0x%x,waiting wb=%d,lut=%d\n",
				epdc_wb_busy,fb_data->waiting_for_wb,fb_data->waiting_for_lut);
		EPDC_VPRINT(fb_data,2,"loading=%d%%\n",mmdc_profile(MMDC_PROFILE_CALC,__func__));
	}
	...
	if(1==fb_data->tce_underrun_occurred) 
	{
		...
		/* Clear TCE underrun IRQ */
		fb_data->tce_underrun_occurred=2;
		return ;
	}
	...
	for (i = 0; i < fb_data->num_luts; i++) {
...
#if 1
		if(fb_data->lut_w[i]>fb_data->active_updating_w) {
			//dev_warn(fb_data->dev,"[WARN] cur update w(%d)>updating w(%d)\n",fb_data->lut_w[i],fb_data->active_updating_w);
			fb_data->active_updating_w = 0;
		}
		else {
			fb_data->active_updating_w -= fb_data->lut_w[i];
		}
		if(fb_data->lut_h[i]>fb_data->active_updating_h) {
			//dev_warn(fb_data->dev,"[WARN] cur update h(%d)>updating h(%d)\n",fb_data->lut_h[i],fb_data->active_updating_h);
			fb_data->active_updating_h = 0;
		}
		else {
			fb_data->active_updating_h -= fb_data->lut_h[i];
		}
#endif
		...
		if (!wb_lut_done) {
			...
			list_for_each_entry_safe(next_marker, temp,
				...
				EPDC_VPRINT(fb_data,3, "Signaling marker %d\n",
	...
	if ((!wb_pause && epdc_wb_busy) || (wb_pause && !epdc_upd_done)) {
		...
		mutex_unlock(&fb_data->queue_mutex);
		GALLEN_DBGLOCAL_ESC();
		EPDC_VPRINT(fb_data,4,"%s() exit - wb busy\n",__FUNCTION__);
		return;
	}

	if(fb_data->waiting_for_working_buf_compl) {
		fb_data->waiting_for_working_buf_compl = 0;
		complete(&fb_data->working_buf_compl);
	}
	...
	if (epdc_waiting_on_wb) {
		...
		if (fb_data->cur_update->update_desc->upd_data.flags
			...
			*/
			...
			list_for_each_entry_safe(next_marker, temp,
				...
				if (next_marker->waiting) {
					GALLEN_DBGLOCAL_RUNLOG(26);
					EPDC_VPRINT(fb_data,3,"%s()[%d]:update_complete @ marker %d\n",\
						__FUNCTION__,__LINE__,next_marker->update_marker);
					complete(&next_marker->update_completion);
				}
				else {
		...
		if (epdc_collision & update_is_valid) {
			...
			if ((fb_data->upd_scheme != UPDATE_SCHEME_SNAPSHOT)
				...
				EPDC_VPRINT(fb_data,3,"Adj coll region: l = %d, "
					"t = %d, w = %d, h = %d\n",
	...
	if (fb_data->upd_scheme != UPDATE_SCHEME_SNAPSHOT) {
		...
		GALLEN_DBGLOCAL_ESC();
		EPDC_VPRINT(fb_data,10,"%s() exit - non-snapshot done\n",__FUNCTION__);
		return;
	}
	...
	if (!epdc_luts_avail) {
		...
		mutex_unlock(&fb_data->queue_mutex);
		GALLEN_DBGLOCAL_ESC();
		EPDC_VPRINT(fb_data,10,"%s() exit - no luts avaiable\n",__FUNCTION__);
		return;
	}
	...
	if (epdc_next_lut_15 && fb_data->tce_prevent && (fb_data->rev < 20)) {
		...
		mutex_unlock(&fb_data->queue_mutex);
		GALLEN_DBGLOCAL_ESC();
		EPDC_VPRINT(fb_data,10,"%s() exit - LUT15 waiting\n",__FUNCTION__);
		return;
	}
	...
	if (fb_data->cur_update == NULL) {
		...
		if (list_empty(&fb_data->upd_buf_queue)) {
			...
			mutex_unlock(&fb_data->queue_mutex);
			GALLEN_DBGLOCAL_ESC();
			EPDC_VPRINT(fb_data,10,"%s() exit - no pending updates \n",__FUNCTION__);
			return;
		} else {
	...
	mutex_unlock(&fb_data->queue_mutex);
	GALLEN_DBGLOCAL_END();
	EPDC_VPRINT(fb_data,10,"%s() exit - snapshot\n",__FUNCTION__);
	return;
}
...
//	unsigned char bWFM_REV ;
	...
	EPDC_VPRINT(fb_data,0,"curmode pixclk=%d,w=%d,h=%d\n",
	...
	{
		extern void mx6_set_epdc_parent_clk(unsigned long clk_hz);
		printk("%s():changing the epdc parent clk :=440MHz \n",__FUNCTION__);
		mx6_set_epdc_parent_clk(440000000);
	}
#endif
...
//		printk("%s() : awv=0x%02X\n",__FUNCTION__,awv);
	...
	DBG_MSG("[%s]waveform:vir_p=%p,phy_p=%p,sz=%d,trt_entries=%d,luts=0x%x\n",__FUNCTION__,\
	...
	if ((wv_file->wdh.luts & 0xC) == 0x4) {
		printk("5 bits waveform,luts=0x%x\n",wv_file->wdh.luts);
		fb_data->buf_pix_fmt = EPDC_FORMAT_BUF_PIXEL_FORMAT_P5N;
	}
	else {
		printk("4 bits waveform,luts=0x%x\n",wv_file->wdh.luts);
...
enum hrtimer_restart before_eof_hrtimer_callback( struct hrtimer *timer )
{
    DBG_MSG("[%s %d]\n",__func__, __LINE__);
    struct mxc_epdc_fb_data *fb_data = g_fb_data;
    complete(&fb_data->before_eof_event);
...
static void epdc_recover(struct mxc_epdc_fb_data *fb_data)
{
	...
	// 3.0.35
	extern struct clk *mx6_get_epdc_parent_clk(void);

	pixel_clk_src = mx6_get_epdc_parent_clk();
	...
	dev_info(fb_data->dev, "lock queues\n");
	/* lock queues */
	mutex_lock(&fb_data->queue_mutex);

	dev_info(fb_data->dev, "reseting IRQs ...\n");
	...
	dev_info(fb_data->dev, "waiting end of frame ...\n");
	ret = wait_for_completion_timeout(&fb_data->eof_event,
		msecs_to_jiffies(50));
	if (!ret) {
	...
	if (!IS_ERR(pixel_clk_src)) {
		dev_info(fb_data->dev, "disable pixel clock source ...\n");
		clk_disable(pixel_clk_src);
	}

	dev_info(fb_data->dev, "soft reset epdc ...\n");
	/* Hold EPDC under Reset until we clean-up */
	__raw_writel(EPDC_CTRL_SFTRST | EPDC_CTRL_CLKGATE, EPDC_CTRL_SET);

	dev_info(fb_data->dev, "disabling epdc clocks ...\n");
	/* Disable clock */
	clk_disable(fb_data->epdc_clk_axi);
	clk_disable(fb_data->epdc_clk_pix);
	...
	dev_info(fb_data->dev, "set flags appropriate state ...\n");
	...
	if (!IS_ERR(pixel_clk_src)) {
		clk_enable(pixel_clk_src);
	}

	dev_info(fb_data->dev, "recover epdc H/W ...\n");
	...
	fb_data->luts_updating = 0;


	/* restore driver to operational state before returning */
	fb_data->in_recovery = false;
	dev_info(fb_data->dev, "epdc_recover finished \n");
	dump_epdc_reg();


	dev_info(fb_data->dev, "unlock queue ...\n");
	/* unlock queue */
	mutex_unlock(&fb_data->queue_mutex);

	dev_info(fb_data->dev, "resending a full update ...is_drop_updates=%d\n",fb_data->is_drop_updates);
	/* Now, request full screen update */
...
int __devinit mxc_epdc_fb_probe(struct platform_device *pdev)
{
	...
	hrtimer_init( &before_eof_hrtimer, CLOCK_MONOTONIC, HRTIMER_MODE_REL );
	before_eof_hrtimer.function = &before_eof_hrtimer_callback;
	before_eof_ktime = ktime_set( 0, 8000000 );
...
#ifdef MMDC_PROFILE //[
	mmdc0_base = ioremap(MMDC_P0_BASE_ADDR, SZ_32K);
	if (mmdc0_base == NULL) {
		dev_err(&pdev->dev, "mmdc0 base remap failed !!\n");
	}
#endif //] MMDC_PROFILE
	...
	if (2 == gptHWCFG->m_val.bUIStyle) { //Android
		if ( 5 == gptHWCFG->m_val.bDisplayPanel || 
				 ...
				 17 == gptHWCFG->m_val.bDisplayPanel /* 7.8" Top EPD(17) */ ||
				 28 == gptHWCFG->m_val.bDisplayPanel) 
		{ //6 top, 6.8 bottom, 13.3 left , 7.8 bottom , 10.3 top .
...
#if defined(CONFIG_MACH_MX6SL_NTX) && defined(MX6SL_QOS_ENABLED) //[
	{
	...
	if (qosc_base == NULL) {
		...
		__raw_writel(0x0f020222, qosc_base + MX6SL_EPD_OFFSET_ADDR + MX6SL_QOS_WR); // Write QOS, init = 7 with red flag
		__raw_writel(0x0f020f22, qosc_base + MX6SL_EPD_OFFSET_ADDR + MX6SL_QOS_RD); // Read QOS, init = 7 with red flag
	...
	fb_data->epdc_submit_workqueue = alloc_workqueue("EPDC Submit",
					...
					WQ_CPU_INTENSIVE|WQ_NON_REENTRANT , 1);
	...
	if (device_create_file(info->dev, &epdc_attrs[0]))
