...
static ssize_t epdc_mmdc_loading_read(struct device *dev, struct device_attribute *attr,char *buf);
static ssize_t epdc_mmdc_loading_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
...
static DEVICE_ATTR(epdc_mmdc_loading, 0666, epdc_mmdc_loading_read, epdc_mmdc_loading_write);
...
static ssize_t store_update(struct device *device,
			     ...
			     const char *buf, size_t count);
static DEVICE_ATTR(update, 0666, NULL, store_update);
...
static DEVICE_ATTR(reset, 0666, show_reset, store_reset);
		       ...
		       const char *buf, size_t count)
{
	...
	while(*pc!='\0' && *pc!='\x0a' && *pc!='\x0d') 
	{
		if (*pc < '0' || *pc > '9') {
			...
			else if( 0==strcmp(buf,"epdc-reupd") || 0==strcmp(buf,"epdc-clean")) {
				struct mxcfb_update_data update;

				if(0==strcmp(buf,"epdc-clean")) {
					memset(g_fb_data->info.screen_base,0, \
							g_fb_data->info.var.xres_virtual*g_fb_data->info.var.yres_virtual);
				}
				
				memset(&update, 0x0, sizeof(struct mxcfb_update_data));
				update.update_region.left = 0;
				update.update_region.width = g_fb_data->epdc_fb_var.xres;
				update.update_region.top = 0;
				update.update_region.height = g_fb_data->epdc_fb_var.yres;
				update.update_mode = UPDATE_MODE_FULL;
				update.temp = TEMP_USE_AMBIENT;
				update.waveform_mode = g_fb_data->wv_modes.mode_gc16;
				update.update_marker = 0;
				update.flags = 0;
				mxc_epdc_fb_send_update(&update, (struct fb_info *)g_fb_data);

			}
			else if( 0==strcmp(buf,"epdc-splitupd") ) {
				ntx_epdc_pmic_exception_rect_reupdate(g_fb_data,&g_fb_data->info);
			}
			else if( 0==strcmp(buf,"epdc-dumpregs") ) {
				dump_epdc_reg();
			}
...
static ssize_t epdc_mmdc_loading_read(struct device *dev, struct device_attribute *attr,char *buf)
{
	int epdc_loading = mmdc_profile(MMDC_PROFILE_CALC,__func__);
	sprintf (buf,"%d%%\n",epdc_loading);
	return strlen(buf);
}
static ssize_t epdc_mmdc_loading_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
...
static ssize_t tce_state_read(struct device *dev, struct device_attribute *attr,char *buf)
{
	if(1==g_fb_data->tce_underrun_occurred) {
		sprintf (buf,"%s\n","underrun");
	}
	else if(2==g_fb_data->tce_underrun_occurred) {
		sprintf (buf,"%s\n","recovered");
	}
	else {
		switch (giTCE_State) {
		       ...
		       const char *buf, size_t count)
{
	...
	if (gSleep_Mode_Suspend) {
	    printk("[%s %d] due to hibernation, control epdc pmic by driver\n", __func__, __LINE__);
	    return -1;
	}
	...
	else if(0==iOn && g_fb_data->updates_active == false) {
		if(iDelay_ms<=0) {
...
static const struct attribute *sysfs_epdc_attrs[] = {
	...
	&dev_attr_epdc_mmdc_loading.attr,
...
static int ntx_epdc_pmic_exception_rect_reupdate(struct mxc_epdc_fb_data *fb_data,struct fb_info *info)
{
	...
	if(FB_ROTATE_UD==fb_data->epdc_fb_var.rotate || FB_ROTATE_UR==fb_data->epdc_fb_var.rotate) {
		...
		dwX = 0;
		dwY = 0;
		dwW = g_fb_data->epdc_fb_var.xres;
		dwH = g_fb_data->epdc_fb_var.yres;
	}
	else {
		dwX = 0;
		dwY = 0;
		dwH = g_fb_data->epdc_fb_var.xres;
		dwW = g_fb_data->epdc_fb_var.yres;
	}
...
static void ntx_epdc_pmic_exception(int iEvt)
{
	...
	{
		if(0==gtEPD_PMIC_exception_rect.width || 0==gtEPD_PMIC_exception_rect.height) {
			return ;
		}
