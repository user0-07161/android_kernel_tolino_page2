...
#include <linux/input.h>
	...
	#define TPS65185_PLATFORM_MX6DL		1
	#define TPS65185_PLATFORM_MX6		1
#elif defined(CONFIG_MACH_MX6SL_NTX)//[
	#define TPS65185_PLATFORM_MX6SL		1
	#define TPS65185_PLATFORM_MX6		1
...
#if defined(TPS65185_PLATFORM_MX6DL)//[
	#include <mach/iomux-mx6dl.h>
#elif defined(TPS65185_PLATFORM_MX6SL)//[
...
#if defined(TPS65185_PLATFORM_MX6DL)//[
	#define GPIO_TPS65185_PWRUP			IMX_GPIO_NR(2,19) // EPDC_PWRCTRL1
	#define GPIO_TPS65185_PWRUP_PADCFG		MX6DL_PAD_EIM_A19__GPIO_2_19	

	#define GPIO_TPS65185_WAKEUP		IMX_GPIO_NR(2,20) // EPDC_PWRCTRL0
	#define GPIO_TPS65185_WAKEUP_PADCFG		MX6DL_PAD_EIM_A18__GPIO_2_20	

	#define GPIO_TPS65185_VCOMCTRL	IMX_GPIO_NR(3,17) // EPDC_VCOM0
	#define GPIO_TPS65185_VCOMCTRL_PADCFG		MX6DL_PAD_EIM_D17__GPIO_3_17	

	#define GPIO_TPS65185_VIN								IMX_GPIO_NR(3,29) // PWRALL
	#define GPIO_TPS65185_VIN_PADCFG				MX6DL_PAD_EIM_D29__GPIO_3_29	

//	#define GPIO_TPS65185_EP_3V3_IN								
//	#define GPIO_TPS65185_EP_3V3_IN_PADCFG					

		//  GPIO input
	#define GPIO_TPS65185_PWRGOOD							IMX_GPIO_NR(2,21) // EPDC_PWRSTAT
	#define GPIO_TPS65185_PWRGOOD_INT_PADCFG			MX6DL_PAD_EIM_A17__GPIO_2_21_PUINT
	#define GPIO_TPS65185_PWRGOOD_GPIO_PADCFG			MX6DL_PAD_EIM_A17__GPIO_2_21_FLOAT

	#define GPIO_TPS65185_INT						IMX_GPIO_NR(2,25) // EPDC_PWRCTRL2
	#define GPIO_TPS65185_INT_PADCFG		 MX6DL_PAD_EIM_OE__GPIO_2_25_PUINT
	#define GPIO_TPS65185_INT_GPIO_PADCFG		 MX6DL_PAD_EIM_OE__GPIO_2_25_FLOAT
...
static int tps65185_gpio_init(void)
{
	...
	mdelay(5);
	gdwSafeTick_To_TurnON_RailPower = jiffies+msecs_to_jiffies(40);

#ifdef GPIO_TPS65185_VIN_1
	mxc_iomux_v3_setup_pad(GPIO_TPS65185_VIN_1_PADCFG);
	gpio_request(GPIO_TPS65185_VIN_1, "tps65185_VIN_1");
	gpio_direction_output(GPIO_TPS65185_VIN_1, 1);
	mdelay(5);
	gdwSafeTick_To_TurnON_RailPower = jiffies+msecs_to_jiffies(40);
#endif
...
static int tps65185_reg_init(int I_iIsEP_3V3_ON)
{
	...
	if( (8==gptHWCFG->m_val.bDisplayResolution) || 
			( (5==gptHWCFG->m_val.bDisplayResolution) && gptHWCFG->m_val.bPCB>=74) || 
			(14==gptHWCFG->m_val.bDisplayResolution) ) 
	{
		// change power on seqence for EPD 1872x1404 (ED078KH1) .
		// EPD 1448x1072 (ED060KH6) && PCB >= E60K0X . 
		// EPD 1920x1440 .
...
int tps65185_init(int iPort,int iEPDTimingType)
{
	...
	for(iChipIdx=0;iChipIdx<TOTAL_CHIPS;iChipIdx++) {
		...
		for(iDetectRetryCnt=1;iDetectRetryCnt<=iDetectRetryMax;iDetectRetryCnt++)
		{
		...
		gpI2C_clientA[iChipIdx] = i2c_new_device(gpI2C_adapter, &gtTPS65185_BIA[iChipIdx]);
			...
			if(NULL == gpI2C_clientA[iChipIdx]) {
				ERR_MSG("[Error] %s : detecting failed ! retry #%d/%d\n",
						__FUNCTION__,iDetectRetryCnt,iDetectRetryMax);
				msleep(10);
				continue;
			}
			else {
...
#if 1
			// measured on Q92 .
			gtTPS65185_DataA[0].dwEP3V3_Off_Ticks = msecs_to_jiffies(8000);// times wait VEE to 0V .
#else
			gtTPS65185_DataA[0].dwEP3V3_Off_Ticks = 200;// times wait VEE >= -7.4V .
#endif
		}
		else if(8==gptHWCFG->m_val.bDisplayResolution || 14==gptHWCFG->m_val.bDisplayResolution ) {
			// resolution is 1872x1404/1920x1440 ...
			gtTPS65185_DataA[0].dwEP3V3_Off_Ticks = msecs_to_jiffies(9000);//measured on E70Q02 .
		}
		else if(1==gptHWCFG->m_val.bDisplayResolution) {
			// resolution is 1024x758 ...
			gtTPS65185_DataA[0].dwEP3V3_Off_Ticks = msecs_to_jiffies(4000);//measured on Q32 .
		}
		else if(0==gptHWCFG->m_val.bDisplayResolution) {
			// resolution is 800x600 ...
			gtTPS65185_DataA[0].dwEP3V3_Off_Ticks = msecs_to_jiffies(6500);//measured on Q92 .
		}
		else if(3==gptHWCFG->m_val.bDisplayResolution) {
			// resolution is 1440x1080 ...
			if (42==gptHWCFG->m_val.bPCB) {
				gtTPS65185_DataA[0].dwEP3V3_Off_Ticks = msecs_to_jiffies(2000);//measured on E60Q62 .
			}
			else {
				gtTPS65185_DataA[0].dwEP3V3_Off_Ticks = msecs_to_jiffies(9000);//measured on E60QM2 .
			}
		}
		else {
...
int tps65185_get_temperature(int *O_piTemperature)
{
	...
	//udelay(1);
	ulTimeoutTick = jiffies + msecs_to_jiffies(500);
	do {
	...
	else 
	{
		do {
			...
			if(!tps65185_is_panel_poweron()) {
				...
				wait_for_completion_timeout(&tps65185_pwrgood_on_completion,
	...
	else 
	{
		do {
			...
			if(tps65185_is_panel_poweron()) {
				...
				wait_for_completion_timeout(&tps65185_pwrgood_off_completion,
					msecs_to_jiffies(5000));
...
int tps65185_chg_mode(unsigned long *IO_pdwMode,int iIsWaitPwrOff)
{
	...
	case TPS65185_MODE_ACTIVE:GALLEN_DBGLOCAL_RUNLOG(0);
		...
		if(TPS65185_MODE_SLEEP==dwCurrent_mode || TPS65185_MODE_UNKOWN==dwCurrent_mode) {
			GALLEN_DBGLOCAL_RUNLOG(3);
			i2c_lock_adapter(gpI2C_clientA[0]->adapter);
			gpio_direction_output(GPIO_TPS65185_WAKEUP, 1);
			i2c_unlock_adapter(gpI2C_clientA[0]->adapter);
...
int tps65185_vcom_set(int I_iVCOM_mv,int iIsWriteToFlash)
{
	...
	if(iIsWriteToFlash) {
...
#else//!][
		{
			...
			//udelay(1);
			ulTimeoutTick = jiffies + msecs_to_jiffies(1000);
			do {
...
int tps65185_ONOFF(int iIsON)
{
	...
	if(!(6==gptHWCFG->m_val.bDisplayCtrl||7==gptHWCFG->m_val.bDisplayCtrl||0x11==gptHWCFG->m_val.bDisplayCtrl)) {
...
#ifdef GPIO_TPS65185_VIN_1
		gpio_direction_output(GPIO_TPS65185_VIN_1, 0);
#endif
