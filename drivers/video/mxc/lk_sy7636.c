...
static int _sy7636_get_reg(unsigned char bRegAddr,unsigned char *O_pbRegVal)
{
	...
	if (iChk < 0) {
		ERR_MSG("%s(%d):%s i2c_master_send fail (addr=0x%x) !\n",
				__FILE__,__LINE__,__FUNCTION__,bRegAddr);
		iRet = SY7636_RET_I2CTRANS_ERR;
	}
	...
	if (iChk < 0) {
		ERR_MSG("%s(%d):%s i2c_master_recv fail (addr=0x%x) !\n",
				__FILE__,__LINE__,__FUNCTION__,bRegAddr);
		iRet = SY7636_RET_I2CTRANS_ERR;
		//dump_stack();
	}
...
static irqreturn_t sy7636_PG_int(int irq, void *dev_id)
{
	...
	if(!SY7636_REG_FLD_GET(OPM,RAILS_ON)) {
		//DBG_MSG("%s(%d):skipped non power on state interrupt \n",
		//		__FUNCTION__,__LINE__);
		return IRQ_HANDLED;
	}
...
static void sy7636_PG_func(struct work_struct *work)
{
	...
	do {
		...
		if((unsigned short)-1==wFaultPG) {
			ERR_MSG("%s(%d) : read faults failed , retry #%d/%d !\n",
					__FUNCTION__,__LINE__,iRetryCnt,iRetryMaxCnt);
			continue;
		}
...
static int _sy7636_gpio_init(void)
{
	...
	gpio_direction_output(GPIO_SY7636_VIN,INIT_POWER_STATE);
...
static int _sy7636_output_en(int iIsEnable)
{
	...
	if(gptSY7636_data->iIsOutputEnabled == iIsEnable) {
		...
		if(iIsEnable) {
			...
			for(iRailsON_RetryCnt=0;
					iRailsON_RetryCnt<iRetryMax;
					iRailsON_RetryCnt++) 
			{
				if(EN_ON!=gpio_get_value(GPIO_SY7636_EN)) {
					_sy7636_EN(1);
...
int sy7636_suspend(void)
{
	...
	if(gSleep_Mode_Suspend) {
		...
		if(1==gptSY7636_data->iWakeEabled) {
			disable_irq_wake(gpio_to_irq(GPIO_SY7636_PWRGOOD));
			gptSY7636_data->iWakeEabled=0;
		}
	}
...
void sy7636_resume(void)
{
	...
	if(gSleep_Mode_Suspend) {
		...
		if(0==gptSY7636_data->iWakeEabled) {
			enable_irq_wake(gpio_to_irq(GPIO_SY7636_PWRGOOD));
			gptSY7636_data->iWakeEabled=1;
		}
...
int sy7636_init(int iPort)
{
	...
	if(36==gptHWCFG->m_val.bPCB || 40==gptHWCFG->m_val.bPCB) {
		...
		else if(8==gptHWCFG->m_val.bDisplayResolution || 3==gptHWCFG->m_val.bDisplayResolution || 14==gptHWCFG->m_val.bDisplayResolution) 
		{
			// 1872x1404 || 1440x1080 || 1920x1440 .
...
#ifdef SY7636_ENABLE_PG_INT //[
	if(request_irq(gptSY7636_data->iPG_IRQ, sy7636_PG_int, IRQF_TRIGGER_FALLING|IRQF_ONESHOT, "SY7636_PG_INT", gptSY7636_data)) 
	{
