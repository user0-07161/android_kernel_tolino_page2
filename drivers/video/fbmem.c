	...
	case FBIOGET_VSCREENINFO:
		...
		#ifdef _FORCE_REPORT_4BIT //[ gallen test .
		{
			if(gptHWCFG&&0==gptHWCFG->m_val.bUIStyle) {
				...
				pvar->blue.length=4;
#if 0
				if (15==gptHWCFG->m_val.bDisplayResolution) {
					// this panel original size is 1264x1680 , so we don't rotate the resolution 
					pvar->xres_virtual = xres_org;
					pvar->yres_virtual = yres_org;
					pvar->xres = xres_org;
					pvar->yres = yres_org;
				}
				else 
#endif
				{
					pvar->xres_virtual = yres_org;
					pvar->yres_virtual = xres_org;
					pvar->xres = yres_org;
					pvar->yres = xres_org;
				}
			}
