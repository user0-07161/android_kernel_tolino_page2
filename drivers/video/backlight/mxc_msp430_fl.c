...
#include "../../../arch/arm/mach-mx6/ntx_firmware_parser.h"
...
static struct device_attribute dev_attr_fw = \
__ATTR(fw,0644,fw_show,NULL);
...
static int mxcfl_set_intensity(struct backlight_device *bd)
{
	...
	if(gptHWCFG->m_val.bFrontLight) {
		if (2==gptHWCFG->m_val.bFL_PWM||6==gptHWCFG->m_val.bFL_PWM||7==gptHWCFG->m_val.bFL_PWM)
			...
			fl_tlc5947_percentageEx(brightness);
		}			
#endif //] CONFIG_BACKLIGHT_TLC5947
		else
...
static int __devinit mxcfl_probe(struct platform_device *pdev)
{
	...
	if(0==gptHWCFG->m_val.bFrontLight) {
		printk("%s(%d): front light is disabled !\n",__FUNCTION__,__LINE__);
		return -EIO;
	}

	devdata = kzalloc(sizeof(struct mxcfl_dev_data), GFP_KERNEL);
	if (!devdata) {
		printk("%s(%d): memory not enought !\n",__FUNCTION__,__LINE__);
		return -ENOMEM;
	}
	...
	if (IS_ERR(bd)) {
		ret = PTR_ERR(bd);
		printk("%s(%d) register backlight devices failed !!!\n",__FUNCTION__,__LINE__);
		goto err0;
	}
	...
	rval = device_create_file(&pdev->dev, &dev_attr_fw);
	if (rval < 0) {
