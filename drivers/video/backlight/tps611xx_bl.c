...
static int tps611xx_bl_init(struct tps611xx_bl_data *pchip)
{
	/*
	 * ES Detection Window
	 *   - ES detect delay
...
static int tps611xx_backlight_probe(struct platform_device *pdev)
{
	...
	pchip->bled =
	    backlight_device_register(TPS611XX_NAME,pchip->dev, 
					   pchip,
...
static int __init tps611xx_backlight_init(void)
{
	return platform_driver_register(&tps611xx_backlight_driver);
}
module_init(tps611xx_backlight_init);

static void __exit tps611xx_backlight_exit(void)
{
	platform_driver_unregister(&tps611xx_backlight_driver);
}
module_exit(tps611xx_backlight_exit);
