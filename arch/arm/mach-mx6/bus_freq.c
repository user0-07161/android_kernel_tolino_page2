				...
				struct device_attribute *attr, char *buf)
{
	int i;
	char cbuf[20];
	void __iomem *mmdc_base = ioremap(MMDC_P0_BASE_ADDR, SZ_32K);
	...
	for (i=0; i<= 0x8C0; i+=4) {
		...
		sprintf (cbuf,"%08x ", readl_relaxed(mmdc_base+i));
		strcat (buf,cbuf);
	}
	strcat (buf,"\n");

	mmdc_base = ioremap(MMDC_P1_BASE_ADDR, SZ_32K);
	strcat (buf,"\n================== mmdc1 registers ==================");
	for (i=0; i<= 0x8C0; i+=4) {
		if (0x44 == i || 0x444 ==i) {
			strcat (buf,"\n");
...
static int __devinit busfreq_probe(struct platform_device *pdev)
{
	...
	err = sysfs_create_file(&busfreq_dev->kobj, &dev_attr_mmdc.attr);
	if (err) {
		dev_err(busfreq_dev,
		       "Unable to register sysdev entry for MMDC");
		return err;
	}
