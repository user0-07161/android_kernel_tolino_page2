 ...
 */
#ifdef CONFIG_IMX_HAVE_PLATFORM_IMX_EPDC //[
int epdc_enabled=1;
#else //][!CONFIG_IMX_HAVE_PLATFORM_IMX_EPDC
int epdc_enabled;
#endif //] CONFIG_IMX_HAVE_PLATFORM_IMX_EPDC
	...
	unsigned long ckih1, unsigned long ckih2)
{
	...
	*/
	clk_set_parent(&gpu3d_shader_clk, &mmdc_ch0_axi_clk);
	clk_set_rate(&gpu3d_shader_clk, 49500000);
	if (cpu_is_mx6dl()) {
...
void mx6_set_epdc_parent_clk(unsigned long clk_hz)
{
	if( 440000000==clk_hz )
	{
		// 440MHz . 
		clk_set_rate(&pll5_video_main_clk, 440000000);
	}
	if( 320000000==clk_hz )
	{
		// 320MHz . 
		clk_set_rate(&pll5_video_main_clk, 320000000);
	}
	else {
		clk_set_rate(&pll5_video_main_clk, 650000000);
	}
...
struct clk *mx6_get_epdc_parent_clk(void)
{
	return &pll5_video_main_clk;
}
