...
static GPIODATA gtNTX_PWR_GPIO_data = {
	...
	.uGPIO = gMX6SL_PWR_SW,
	.szName = "PWR_SW",
	.tPADCtrl = MX6DL_PAD_KEY_ROW1__GPIO_4_9,
...
int gWakeup_has_gpio_isr = 0 ;
int gPower_key_dwon = 0;
...
static unsigned int ntx_gpio_dir[7],ntx_gpio_insuspend_dir[7];
...
void ricoh_suspend_state_sync(void)
{
   ...
   4509,   4811,   5059,   5367,   5651,   5971,   6313,   6663,   7036,   7441,// 91-100
	...
	switch (gptHWCFG->m_val.bPCB) {
	case 77:
		hiber_current = 1870; // @ Android 5.10.3224 . 
		break;
	default:
...
void ntx_gpio_suspend (void)
{
	...
	if (gSleep_Mode_Suspend) {
		if(17==gptHWCFG->m_val.bDisplayCtrl) {
			tps65185_ONOFF(0);
		}
		else if(18==gptHWCFG->m_val.bDisplayCtrl) {
			fp9928_ONOFF(0);
		}
		else if(22==gptHWCFG->m_val.bDisplayCtrl) {
			sy7636_ONOFF(0);
		}
	...
	if (gSleep_Mode_Suspend) {
		...
		// Set NVCC_GPIO & NVCC_EIM pads.
		ntx_gpio_insuspend_dir[0]=0x000003ff;
		ntx_gpio_insuspend_dir[1]=0xffff0000;
		ntx_gpio_insuspend_dir[2]=0xffffffff;
		ntx_gpio_insuspend_dir[3]=0x0000ffe0;
		ntx_gpio_insuspend_dir[4]=0x00000015;
		ntx_gpio_insuspend_dir[5]=0x80000040;
		ntx_gpio_insuspend_dir[6]=0x00003800;
    	...
    	for (i = 0; i < ARRAY_SIZE(ntx_suspend_enter_pads); i++) {
			...
			*p = ntx_suspend_exit_pads[i] = ntx_suspend_enter_pads[i];
			if(1==gptHWCFG->m_val.bPMIC) {
				...
				if( ((*p)==(MX6DL_PAD_KEY_COL3__GPIO_4_12)) ||
						((*p)==(MX6DL_PAD_KEY_ROW3__GPIO_4_13)) ||
						((*p)==(MX6DL_PAD_KEY_COL1__GPIO_4_8)) )
				{
					continue ;
				}

				// BAT_LOW_INT,PMU_INT,PWR_SW
				if(((*p)==(MX6DL_PAD_KEY_COL4__GPIO_4_14)) ||
						((*p)==(MX6DL_PAD_KEY_COL2__GPIO_4_10)) ||
						((*p)==(MX6DL_PAD_KEY_ROW1__GPIO_4_9)))
				{
				   	*p &= ~MUX_PAD_CTRL_MASK;
			...
			if((*p) == MX6DL_PAD_GPIO_2__GPIO_1_2)
			{
				...
				if ( (*p) == MX6DL_PAD_GPIO_5__GPIO_1_5 ||
					 (*p) == MX6DL_PAD_GPIO_6__GPIO_1_6 ||
					 (*p) == MX6DL_PAD_KEY_ROW2__GPIO_4_11 ||
					 (*p) == MX6DL_PAD_EIM_D29__GPIO_3_29) 
				{
					// pull down
	                *p &= ~MUX_PAD_CTRL_MASK;
		...
		ntx_gpio_insuspend_dir[0] &= ~dwDisableBit; // GP0 .
#endif
		...
		ntx_gpio_dir[5] = __raw_readl(base+4);
		__raw_writel( ntx_gpio_dir[5]&(~ntx_gpio_insuspend_dir[5]), base+4);

		base = IO_ADDRESS(GPIO7_BASE_ADDR);
		ntx_gpio_dir[6] = __raw_readl(base+4);
		__raw_writel( ntx_gpio_dir[6]&(~ntx_gpio_insuspend_dir[6]), base+4);
#if 0
printk ("ntx_gpio_dir[0] %08X to %08X\n", ntx_gpio_dir[0], ntx_gpio_dir[0]&(~ntx_gpio_insuspend_dir[0]));
printk ("ntx_gpio_dir[1] %08X to %08X\n", ntx_gpio_dir[1], ntx_gpio_dir[1]&(~ntx_gpio_insuspend_dir[1]));
printk ("ntx_gpio_dir[2] %08X to %08X\n", ntx_gpio_dir[2], ntx_gpio_dir[2]&(~ntx_gpio_insuspend_dir[2]));
printk ("ntx_gpio_dir[3] %08X to %08X\n", ntx_gpio_dir[3], ntx_gpio_dir[3]&(~ntx_gpio_insuspend_dir[3]));
printk ("ntx_gpio_dir[4] %08X to %08X\n", ntx_gpio_dir[4], ntx_gpio_dir[4]&(~ntx_gpio_insuspend_dir[4]));
#endif
	}
	return;
...
void ntx_gpio_resume (void)
{
	if (gSleep_Mode_Suspend) {
        ...
        base = IO_ADDRESS(GPIO6_BASE_ADDR);
        __raw_writel( ntx_gpio_dir[5], base+4);

        base = IO_ADDRESS(GPIO7_BASE_ADDR);
        __raw_writel( ntx_gpio_dir[6], base+4);
