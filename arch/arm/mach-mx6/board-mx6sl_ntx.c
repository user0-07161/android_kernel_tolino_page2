...
#include <linux/platform_data/nxp_pn5xx.h>
#include <linux/platform_data/nxp_pn7462.h>
...
#ifdef CONFIG_TOUCHSCREEN_FTS_5426
#include "../../../drivers/input/touchscreen/focaltech_touch/focaltech_core.h"
#else
...
#endif //]CONFIG_BACKLIGHT_TLC5947 
...
volatile unsigned gMX6SL_RS485_DIR = IMX_GPIO_NR(4, 6);	/* RS485_DIR */
...
volatile unsigned gMX6SL_VBUS_PWR_EN = -1;	
volatile int giISD_3V3_ON_Ctrl = -1;

// AudioCodec chip control pins . 
volatile unsigned gMX6SL_AD_1V8_ON = -1;
volatile int giAD_1V8_ON_Ctrl = -1;
volatile unsigned gMX6SL_AD_3V3_ON = -1;
volatile int giAD_3V3_ON_Ctrl = -1;
volatile unsigned gMX6SL_AD_5V_ON = -1;
volatile int giAD_5V_ON_Ctrl = -1;
volatile unsigned gMX6SL_AD_LDO_ON = -1;
volatile int giAD_LDO_ON_Ctrl = -1;

volatile unsigned gMX6SL_AUD_AMP = -1;
volatile int giAUD_AMP_ON_Ctrl = -1;
volatile unsigned gMX6SL_AUD_BOOST = -1;
volatile int giAUD_BOOST_ON_Ctrl = -1;
static int giDelay_ms_AMP_to_Boost = 500;

volatile unsigned gMX6SL_SPK_VDD = -1;
volatile int giSPK_VDD_Ctrl = -1;
...
//C13Q08 pwm
extern const struct imx_mxc_pwm_data imx6sl_mxc_pwm_data[] __initconst;
...
static int mx6_ntx_s1v3s3xx_cs[] = {
	-1,	
};
static const struct spi_imx_master mx6_ntx_s1v3s3xx_spi_data __initconst = {
	.chipselect     = mx6_ntx_s1v3s3xx_cs,
	.num_chipselect = ARRAY_SIZE(mx6_ntx_s1v3s3xx_cs),
};
...
static struct s1v3s3xx_platform_data s1v3s3xx_plat_data  = {
	.gpio_TTS_INT = IMX_GPIO_NR(4,3),
	.gpio_TTS_RST = IMX_GPIO_NR(3,29),
	.gpio_TTS_STB = IMX_GPIO_NR(3,31),
//	.gpio_TTS_CS = IMX_GPIO_NR(4,15),
	.gpio_TTS_CS = IMX_GPIO_NR(2,22),
	.gpio_AMP = IMX_GPIO_NR(3,30),
	.gpio_BOOST = IMX_GPIO_NR(3,24),
};

static struct spi_board_info s1v3s3xx_spi_board_info[] __initdata = {
	{
	/* The modalias must be the same as spi device driver name */
	.modalias	= S1V3S3XX_NAME,
	.max_speed_hz	= 1200000,
	.bus_num	= 0,
//	.bus_num	= 1,
	.chip_select	= 0,
	.platform_data	= &s1v3s3xx_plat_data,
	},
};
...
static struct mxc_audio_platform_data alc5645_data;

static struct platform_device mx6_ntx_audio5645_device = {
	.name = "imx-alc5645",
};
...
static int ntx_audio_io_init(void)
{
	...
	if( (-1!=gMX6SL_AD_1V8_ON) && gpio_is_valid(gMX6SL_AD_1V8_ON)) {
		if(gMX6SL_AD_1V8_ON==IMX_GPIO_NR(4,0)) {
			printk("AD_1V8_ON@GPIO_4_0\n");
			mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_COL4__GPIO_4_0_SION_OUT);//  .
		}
		else if(gMX6SL_AD_1V8_ON==IMX_GPIO_NR(3,25)) {
			printk("AD_1V8_ON@GPIO_3_25\n");
			mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_ROW0__GPIO_3_25_SION_OUT);//  .
		}
		else if(gMX6SL_AD_1V8_ON==IMX_GPIO_NR(3,31)) {
			printk("AD_1V8_ON@GPIO_3_31\n");
			mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_ROW3__GPIO_3_31_SION_OUT);//  .
		}
	}

	if( (-1!=gMX6SL_AD_LDO_ON) && gpio_is_valid(gMX6SL_AD_LDO_ON)) {
		if(gMX6SL_AD_LDO_ON==IMX_GPIO_NR(4,2)) {
			printk("AD_LDO_ON@GPIO_4_2\n");
			mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_COL5__GPIO_4_2_SION_OUT);// .
		}
	}

	if( (-1!=gMX6SL_AD_5V_ON) && gpio_is_valid(gMX6SL_AD_5V_ON)) {
		if(gMX6SL_AD_5V_ON==IMX_GPIO_NR(3,27)) {
			printk("AD_5V_ON@GPIO_3_27\n");
			mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_ROW1__GPIO_3_27_SION_OUT);// 
		}
	}

	if( (-1!=gMX6SL_AD_3V3_ON) && gpio_is_valid(gMX6SL_AD_3V3_ON)) {
		if(gMX6SL_AD_3V3_ON==IMX_GPIO_NR(3,29)) {
			printk("AD_3V3_ON@GPIO_3_29\n");
			mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_ROW2__GPIO_3_29_SION_OUT);// 
		}
		else if(gMX6SL_AD_3V3_ON==IMX_GPIO_NR(3,30)) {
			printk("AD_3V3_ON@GPIO_3_30\n");
			mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_COL3__GPIO_3_30_SION_OUT);// 
		}
		else if(gMX6SL_AD_3V3_ON==IMX_GPIO_NR(3,27)) {
			printk("AD_3V3_ON@GPIO_3_27\n");
			mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_ROW1__GPIO_3_27_SION_OUT);// 
		}
	}

	if( (-1!=gMX6SL_SPK_VDD) && gpio_is_valid(gMX6SL_SPK_VDD)) {
		if(gMX6SL_SPK_VDD==IMX_GPIO_NR(2,6)) {
			printk("SPK_VDD@GPIO_2_6\n");
			mxc_iomux_v3_setup_pad(MX6SL_PAD_EPDC_BDR1__GPIO_2_6);// 
		}		
	}
	

	if(gptHWCFG) {
		if(90 == gptHWCFG->m_val.bPCB || 0x6B == gptHWCFG->m_val.bPCB) { //MXXQ4X, U13Q56

			if( (-1!=gMX6SL_AUD_AMP) && gpio_is_valid(gMX6SL_AUD_AMP)) {
				if(gMX6SL_AUD_AMP==IMX_GPIO_NR(3,30)) {
					mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_COL3__GPIO_3_30_SION_OUT);// amp .
				}
				gpio_request (gMX6SL_AUD_AMP, "ALC5640_AMP");
				gpio_direction_output(gMX6SL_AUD_AMP,giAUD_AMP_ON_Ctrl?0:1);
			}

			if( (-1!=gMX6SL_AUD_BOOST) && gpio_is_valid(gMX6SL_AUD_BOOST)) {
				if(gMX6SL_AUD_BOOST==IMX_GPIO_NR(3,24)) {
					mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_COL0__GPIO_3_24_SION_OUT);// amp .
				}
				gpio_request (gMX6SL_AUD_BOOST, "ALC5640_BOOST");
				gpio_direction_output(gMX6SL_AUD_BOOST,giAUD_BOOST_ON_Ctrl?0:1);
			}
		}
	}
...
#ifdef CONFIG_SIGNAGE
	int rate;
	rate = clk_round_rate(pll4,786432000);
	clk_set_rate(pll4, rate);	
#endif	
...
#ifdef CONFIG_SIGNAGE
       gMX6SL_AUD_AMP=IMX_GPIO_NR(3,30);
       gMX6SL_AUD_BOOST=IMX_GPIO_NR(3,24);
	   gMX6SL_SPK_VDD=IMX_GPIO_NR(2,6);
       giAUD_AMP_ON_Ctrl = 1;
       giAUD_BOOST_ON_Ctrl =1;
#endif
...
static int ntx_amp_enable(int enable)
{
	int iRet = 0;
	//printk("%s() en=%d\n",__func__,enable);

	if(gptHWCFG) {
		if(90==gptHWCFG->m_val.bPCB || 0x6B==gptHWCFG->m_val.bPCB) {
			if(enable) {
				gpio_direction_output(gMX6SL_AUD_BOOST,giAUD_BOOST_ON_Ctrl?1:0);
				msleep(giDelay_ms_AMP_to_Boost);
				gpio_direction_output(gMX6SL_AUD_AMP,giAUD_AMP_ON_Ctrl?1:0);
			}
			else {
				gpio_direction_output(gMX6SL_AUD_AMP,giAUD_AMP_ON_Ctrl?0:1);
				msleep(giDelay_ms_AMP_to_Boost);
...
static struct mxc_audio_platform_data alc5640_data = {
	...
	.amp_enable = ntx_amp_enable,
};

struct clk *mx6_audio_mclk;

static int audio_clk_enable(int enable)
{
	if (enable)
		clk_enable(mx6_audio_mclk);
	else
		clk_disable(mx6_audio_mclk);

	return 0;
}


static int mxc_alc5645_init(void)
{
	struct clk *pll4;
	int rate;

	mx6_audio_mclk = clk_get(NULL, "extern_audio_clk");
	if (IS_ERR(mx6_audio_mclk)) {
		pr_err("can't get ntx_audio_mclk clock.\n");
		return PTR_ERR(mx6_audio_mclk);
	}
	...
	clk_enable(pll4);
	clk_set_parent(mx6_audio_mclk, pll4);

	rate = clk_round_rate(mx6_audio_mclk,4000000);
	clk_set_rate(mx6_audio_mclk, rate);
	alc5645_data.sysclk = clk_get_rate(pll4);

	mxc_iomux_v3_setup_pad( MX6SL_PAD_KEY_ROW5__GPIO_4_3_PULLHIGH );
...
static struct mxc_audio_platform_data alc5645_data = {
	...
	.src_port = 3,
	.ext_port = 2,
	.hp_gpio = IMX_GPIO_NR(4, 3),
	.hp_active_low = 0,
	.mic_gpio = -1,
	.mic_active_low = 1,
	.init = mxc_alc5645_init,
	.clock_enable = audio_clk_enable,
};
...
static int __init imx_init_alc5645(void)
{
	// ALC5645 codec .
	mxc_register_device(&mx6_ntx_audio5645_device,
		 &alc5645_data);
	imx6q_add_imx_ssi(1, &mx6_sabresd_ssi_pdata);
...
static struct fts_ts_platform_data fts_5426_data ={
	.max_touch_number=10,
	.x_max = 1448,
	.y_max = 1072,
};
...
static struct i2c_board_info i2c_alc5645_codec_binfo = {
	.type = "rt5645",
	.addr = 0x1A,
};
...
static struct pn544_i2c_platform_data nfc_pn5xx = {
	.irq_gpio = MX6SL_NFC_INT,
	.ven_gpio = MX6SL_NFC_VEN,
	.firm_gpio = -1,
	.clkreq_gpio = -1,
};

static struct i2c_board_info i2c_nxp_nfc_pn5xx_binfo = {
	 .type = "nxp_nfc_pn5xx",
	 .addr = 0x28,
	 .platform_data = &nfc_pn5xx,
};

static struct pn7462_i2c_platform_data nfc_pn7462 = {
	.irq_gpio = ACEP13_NFC_INT,
	.ven_gpio = ACEP13_NFC_VEN,
	.firm_gpio = -1,
	.clkreq_gpio = -1,
	.led1_gpio = ACEP13_LED1,
	.led2_gpio = ACEP13_LED2,
	.led3_gpio = ACEP13_LED3,
	.led4_gpio = ACEP13_LED4,
	.sled1_gpio = ACEP13_SLED1,
	.sled2_gpio = ACEP13_SLED2,
};

static struct i2c_board_info i2c_nxp_nfc_pn7462_binfo = {
	 .type = "nxp_nfc_pn7462",
	 .addr = 0x28,
	 .platform_data = &nfc_pn7462,
};

static struct gpio_led acep13_gpio_leds[] = {
      GPIO_LED(ACEP13_LED1, "acep13_led1", 1, 1, "none"),
      GPIO_LED(ACEP13_LED2, "acep13_led2", 1, 1, "none"),
      GPIO_LED(ACEP13_LED3, "acep13_led3", 1, 1, "none"),
      GPIO_LED(ACEP13_LED4, "acep13_led4", 1, 1, "none"),
      GPIO_LED(ACEP13_SLED1, "acep13_sled1", 1, 1, "none"),
      GPIO_LED(ACEP13_SLED2, "acep13_sled2", 1, 1, "none"),
};

static struct gpio_led_platform_data acep13_gpio_leds_data = {
        .leds           = acep13_gpio_leds,
        .num_leds       = ARRAY_SIZE(acep13_gpio_leds),
};

static struct platform_device acep13_gpio_led_device = {
        .name           = "leds-gpio",
        .id             = -1,
        .num_resources  = 0,
        .dev            = {
                .platform_data = &acep13_gpio_leds_data,
        }
};


static struct platform_device acep13_buzzer_pwm = {
        .name   = "pwm-beeper",
        .id     = 1,
        .dev    = {
                .platform_data = 2,
...
static struct i2c_board_info i2c_nxp_ccg_ucsi_binfo = {
	 .type = "ccgx-ucsi",
	 .addr = 0x8,
};

struct tps611xx_platform_data tps61165_pdata = {
	.rfa_en = 0,
	.en_gpio_num = ACEP13_LEDCTRL,
};

static struct platform_device acep13_tps61165 = {
	.name = "tps61165",
	.dev = {
		.platform_data = &tps61165_pdata,
	},
};
...
#ifndef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_DEVICETREE_SUPPORT
{
	...
	if(2 !=gptHWCFG->m_val.bUIStyle || 74 == gptHWCFG->m_val.bPCB || 75==gptHWCFG->m_val.bPCB || 87==gptHWCFG->m_val.bPCB ) { //E60K00 E80K02 E70K0x
...
static struct regulator_consumer_supply ricoh619_ldo7_supply_0[] = {
	...
	REGULATOR_SUPPLY("vdd_fl_1_pwm", "1-0043"),
...
static struct i2c_board_info i2c_it7236_binfo = {
	.type = "it7236_touchkey",
	//.type = "ite,7236",
	.addr = 0x46,
	.irq = gpio_to_irq(MX6SL_TK_INT),
	.platform_data = MX6SL_TK_INT,
};
...
static int mx6sl_ntx_fec_power_hibernate(struct phy_device *phydev)
{
	printk("%s()\n",__func__);
	gpio_direction_output(MX6_BRD_FEC_PWR_EN, 0);
	return 0;
...
static int mx6sl_ntx_fec_suspend(struct phy_device *phydev)
{
	if( phydev && PHY_UP==phydev->state ) {
		printk("%s() : phy_state=%d,link=%d\n",__func__,phydev->state,phydev->link);
	}
	else {
		//printk("%s(), turns off FEC power\n",__func__);
		gpio_direction_output(MX6_BRD_FEC_PWR_EN, 0);
	}
	return 0;
}
static int mx6sl_ntx_fec_resume(struct phy_device *phydev)
{
	if( phydev && PHY_UP==phydev->state ) {
		printk("%s() : phy_state=%d,link=%d\n",__func__,phydev->state,phydev->link);
	}
	else {
		printk("%s(), turns on FEC power\n",__func__);
		gpio_direction_output(MX6_BRD_FEC_PWR_EN, 1);
	}
	return 0;
}
...
//	.power_hibernate	= mx6sl_ntx_fec_power_hibernate,
	.suspend	= mx6sl_ntx_fec_suspend,
	.resume	= mx6sl_ntx_fec_resume,
...
static ssize_t elan_touch_virtualkeys_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf,
		__stringify(EV_KEY) ":"
		__stringify(KEY_BACK) ":1088:90:160:180"
		":" __stringify(EV_KEY) ":"
		__stringify(KEY_MENU) ":1088:270:160:180"
		":" __stringify(EV_KEY) ":"
		__stringify(KEY_HOMEPAGE) ":1088:450:160:180"
		":" __stringify(EV_KEY) ":"
		__stringify(KEY_SEARCH) ":1088:630:160:180"
		"\n");
}

static struct kobj_attribute elan_touch_virtualkeys_attr = {
	.attr = {
		.name = "virtualkeys.elan-touch",
		.mode = S_IRUGO,
	},
	.show = &elan_touch_virtualkeys_show,
};

static struct attribute *elan_touch_properties_attrs[] = {
	&elan_touch_virtualkeys_attr.attr,
	NULL
};

static struct attribute_group elan_touch_properties_attr_group = {
	.attrs = elan_touch_properties_attrs,
};

static void elan_touch_add_board_properties(void)
{
	if(86==gptHWCFG->m_val.bPCB) // add virtual key board_properties
	{
		struct kobject *properties_kobj;
		int ret = 0;
			...
			if (properties_kobj)
				ret = sysfs_create_group(properties_kobj,
						&elan_touch_properties_attr_group);
			if (!properties_kobj || ret)
				pr_err("%s: failed to create board_properties\n", __func__);
	}
}
...
static struct gpio_keys_button gpio_key_TTS_Signage[] = {
	GPIO_BUTTON(IMX_GPIO_NR(4, 10),KEY_SOUND, 1, "TTS", 1, 50),			// 
//#ifdef CONFIG_ANDROID //[
	GPIO_BUTTON(IMX_GPIO_NR(4, 8), KEY_POWER, 1, "power", 1, 1),
//#endif //]CONFIG_ANDROID
};
static struct gpio_keys_button gpio_key_1234_Signage[] = {
	GPIO_BUTTON(IMX_GPIO_NR(1, 31),KEY_1, 1, "1", 1, 50),			// 
	GPIO_BUTTON(IMX_GPIO_NR(2, 2),KEY_2, 1, "2", 1, 50),			// 
	GPIO_BUTTON(IMX_GPIO_NR(2, 0),KEY_3, 1, "3", 1, 50),			// 
	GPIO_BUTTON(IMX_GPIO_NR(1, 23),KEY_4, 1, "4", 1, 50),			// 
//#ifdef CONFIG_ANDROID //[
	GPIO_BUTTON(IMX_GPIO_NR(4, 8), KEY_2, 1, "power", 1, 1),
//#endif //]CONFIG_ANDROID
};
...
static struct gpio_keys_button gpio_key_acep13[] = {
	GPIO_BUTTON(IMX_GPIO_NR(1, 31),KEY_1, 1, "1", 1, 50),			// 
	GPIO_BUTTON(IMX_GPIO_NR(2, 2),KEY_2, 1, "2", 1, 50),			// 
	GPIO_BUTTON(IMX_GPIO_NR(2, 1),KEY_3, 1, "3", 1, 50),			// 
	GPIO_BUTTON(IMX_GPIO_NR(2, 0),KEY_4, 1, "4", 1, 50),			//
	GPIO_BUTTON(IMX_GPIO_NR(5, 7),KEY_5, 1, "5", 1, 50),			//
	GPIO_BUTTON(IMX_GPIO_NR(1, 23),KEY_6, 1, "6", 1, 50),			//
	GPIO_BUTTON(IMX_GPIO_NR(1, 26),KEY_7, 1, "7", 1, 50),			//
};

static struct gpio_keys_button gpio_key_2button_door[] = {
	GPIO_BUTTON(IMX_GPIO_NR(4, 8), KEY_POWER, 1, "power", 1, 1),
	GPIO_BUTTON(IMX_GPIO_NR(1, 30), KEY_F1, 1, "button_sw1", 1, 1),
	GPIO_BUTTON(IMX_GPIO_NR(2, 12), KEY_F2, 1, "button_sw2", 1, 1),
	GPIO_BUTTON(IMX_GPIO_NR(2, 0), KEY_F3, 1, "door", 1, 1),
};
...
{
	return (gi_wifi_power_status)?1:0;
}

void ntx_set_wifi_rst(bool val) {
        gpio_direction_output (gMX6SL_WIFI_RST, val);
}
EXPORT_SYMBOL(ntx_set_wifi_rst);
...
int set_wifi_3v3(int on)
{
	...
	if (72==gptHWCFG->m_val.bPCB || 74==gptHWCFG->m_val.bPCB || 75==gptHWCFG->m_val.bPCB || 82==gptHWCFG->m_val.bPCB  // M31Q0x/E60K0x/E80K0x/M35QMx
		|| 83==gptHWCFG->m_val.bPCB || 86==gptHWCFG->m_val.bPCB || 87==gptHWCFG->m_val.bPCB || 90==gptHWCFG->m_val.bPCB	 //E60QTx/E60QVx/E70K0x/MXXQ4X
		|| 0x6B == gptHWCFG->m_val.bPCB) //U13Q56
...
int set_wifi_sdio_pins(int wifion)
{
	int status = -EINVAL;
...
{
	mxc_iomux_v3_setup_multiple_pads(mx6sl_ntx_uart4_pads,
					ARRAY_SIZE(mx6sl_ntx_uart4_pads));
...
int ntx_wifi_power_ctrl (int isWifiEnable)
{
	...
	if(isWifiEnable){  // TURN ON WIFI
		...
		if (11==gptHWCFG->m_val.bWifi || 12==gptHWCFG->m_val.bWifi || 14==gptHWCFG->m_val.bWifi && gpio_is_valid(gMX6SL_WIFI_DIS)) {
...
void ntx_vbus_power(int iIsON)
{
#ifdef CONFIG_USB_EHCI_ARC //[
	if(-1!=gMX6SL_VBUS_PWR_EN && gpio_is_valid(gMX6SL_VBUS_PWR_EN)) {
		printk("%s(%d) \n",__FUNCTION__,iIsON);
		gpio_direction_output (gMX6SL_VBUS_PWR_EN, iIsON?1:0);
	}
#else //][!CONFIG_USB_EHCI_ARC
	printk("%s() skipped not usb host !\n",__FUNCTION__);
#endif //] CONFIG_USB_EHCI_ARC
}

void ntx_ACEP_12V_ctrl(int iIsON)
{
	printk("%s(%d) \n",__FUNCTION__,iIsON);
	if ((90 == gptHWCFG->m_val.bPCB && 0x0 == gptHWCFG->m_val.bPCB_REV && 3 == gptHWCFG->m_val.bPCB_LVL) //C13Q08
		|| 0x6B==gptHWCFG->m_val.bPCB) // U13Q56
	{
		gpio_direction_output (IMX_GPIO_NR(4, 3), iIsON);
	}
}
...
void ntx_ite8951_power(int iIsON)
{
	if(14==gptHWCFG->m_val.bDisplayCtrl) {
		...
		else
		{
			...
			//if(gpio_is_valid(gMX6SL_USB_HUB_RST)) 
			{
				//udelay(20);
	...
	if(82==gptHWCFG->m_val.bPCB) //M35QE0
	{
		unsigned int i = IMX_GPIO_NR(3,24);
		if(iIsON)
		{
			msleep(10);
...
void ntx_ite8951_reset(int iIsON)
{
	if(82==gptHWCFG->m_val.bPCB) //M35QE0
		gpio_direction_output (IMX_GPIO_NR(3,24), iIsON?1:0);
}
...
void ntx_rs485_dir(int iIsON)
{
	gpio_direction_output (gMX6SL_RS485_DIR, iIsON?1:0);
}
...
#ifdef CONFIG_TOUCHSCREEN_GOODIX_TOUCH //[
extern int Get_Goodix_State(void);
#endif //]CONFIG_TOUCHSCREEN_GOODIX_TOUCH
...
#ifdef CONFIG_TOUCHSCREEN_GOODIX_TOUCH //[
		if(17==gptHWCFG->m_val.bTouchCtrl||18==gptHWCFG->m_val.bTouchCtrl ) {
			// Goodix Driver must call Get_Goodix_State()
            ret = Get_Goodix_State();
            if (ret != 0)
                return ret;
		}
#endif //]CONFIG_TOUCHSCREEN_GOODIX_TOUCH
        ret = gpio_get_value(gMX6SL_IR_TOUCH_INT)?0:1;
        if ( ret != 0 ) {
			printk ("[%s-%d] touch_int high, failed to suspend\n", __func__, __LINE__);
    		return ret;
		}
	}
   
	if(6==gptHWCFG->m_val.bRSensor) {
		//for gsensor
		ret = gpio_get_value(kx122_info.gpio_int1)?0:1;
		if (ret != 0) {
			printk("gsensor trigger -> fail to go to suspend\n");
			return ret;
		} 
	}
#ifdef CONFIG_KEYBOARD_IT7236 //[
	{
		extern int IT7236_check_can_suspend(void);
		if(0==IT7236_check_can_suspend()) {
			return -1;
		}
	}
#endif //] CONFIG_KEYBOARD_IT7236
	return ret;
}
...
static void ntx_gpio_init(void)
{
	...
	if(31==gptHWCFG->m_val.bPCB||32==gptHWCFG->m_val.bPCB) {
		...
		if(NTXHWCFG_TST_FLAG(gptHWCFG->m_val.bPCB_Flags2,2)) {
			...
			//if(gptHWCFG->m_val.bLAN)
			{
				...
				if((90==gptHWCFG->m_val.bPCB&&gptHWCFG->m_val.bPCB_REV>=0x10))
				{
					mxc_iomux_v3_setup_multiple_pads(mx6sl_ntx_lcd_spi1_pads,
						ARRAY_SIZE(mx6sl_ntx_lcd_spi1_pads));
				}

				if((90==gptHWCFG->m_val.bPCB&&1==gptHWCFG->m_val.bPCB_LVL)) {
					// MXXQ4X .
					// SPI2 for epson TTS module s1v3s3xx . 	
					mxc_iomux_v3_setup_multiple_pads(mx6sl_ntx_spi2_spi2_pads,
						ARRAY_SIZE(mx6sl_ntx_spi2_spi2_pads));
				}
				else {
		...
		if(!NTXHWCFG_TST_FLAG(gptHWCFG->m_val.bPCB_Flags,0)) {
			...
			// GPIO key .
			if(8 != gptHWCFG->m_val.bFL_PWM) // TLC5947 , 3_29 is FL_PWR_ON
			{
		...
		if(NTXHWCFG_TST_FLAG(gptHWCFG->m_val.bPCB_Flags2,2)) {
			...
			//if(gptHWCFG->m_val.bLAN) 
			{
				// IOs@SPI1&SPI2
				if((90==gptHWCFG->m_val.bPCB&&1==gptHWCFG->m_val.bPCB_LVL)) {
					// test PCBA, MXXQ4X(SANDIPIPER_B)
					gMX6SL_NTX_ACIN_PG = -1;
				}
				else {
		...
		if(NTXHWCFG_TST_FLAG(gptHWCFG->m_val.bPCB_Flags2,0)) {
			...
			if(3==gptHWCFG->m_val.bTouchType||54==gptHWCFG->m_val.bPCB||84==gptHWCFG->m_val.bPCB||86==gptHWCFG->m_val.bPCB)
			{
				// C touch PCB design || ED0Q1X || E70Q54 || E60QVX . 
				gMX6SL_IR_TOUCH_RST = IMX_GPIO_NR(5, 13);
				elan_touch_add_board_properties();
			}
			if(84==gptHWCFG->m_val.bPCB){
				mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_COL1__GPIO_3_26_PULLHIGH);
			}
		...
		if(10==gptHWCFG->m_val.bBattery) {
			...
			if (0x43 == gptHWCFG->m_val.bPCB ) {
				...
				gptHWCFG->m_val.bEPD_Flags |= 0x02;
                        ...
                        if( (16==gptHWCFG->m_val.bDisplayPanel||17==gptHWCFG->m_val.bDisplayPanel||18==gptHWCFG->m_val.bDisplayPanel||19==gptHWCFG->m_val.bDisplayPanel) &&
				!NTXHWCFG_TST_FLAG(gptHWCFG->m_val.bEPD_Flags,1)) 
                        {
		...
		else if(15==gptHWCFG->m_val.bBattery || 17==gptHWCFG->m_val.bBattery 
				|| 18==gptHWCFG->m_val.bBattery	|| 19==gptHWCFG->m_val.bBattery) {
			// set VFCHG to 4.35V
			...
			ricoh_battery_data.type[0].ch_vbatovset = 1;	// set VBATOVSET
                ...
                else if(20==gptHWCFG->m_val.bBattery )
                {
                        if(86==gptHWCFG->m_val.bPCB){
        		        ricoh_battery_data.alarm_vol_mv = 3250;		// set battery critical to 3.25V
	        	        ricoh_battery_data.type[0].fg_poff_vbat = 3350;		// set battery 0% to 3.35V   
                                printk("==== Set cut-off :%d ====\n",ricoh_battery_data.type[0].fg_poff_vbat);
                                printk("==== Set alarm :%d ====\n",ricoh_battery_data.alarm_vol_mv);
                        }
                }
        else if(1==gptHWCFG->m_val.bBattery){
           if (0x54 == gptHWCFG->m_val.bPCB ){// E70Q54
				ricoh_battery_data.alarm_vol_mv = 3650;         // set battery critical to 3.65V
				ricoh_battery_data.type[0].fg_poff_vbat = 3700; // set battery 0% to 3.7V
            }
        }
		...
		if(NTXHWCFG_TST_FLAG(gptHWCFG->m_val.bPCB_Flags2,0)) {
			...
			else if(75==gptHWCFG->m_val.bPCB || 87==gptHWCFG->m_val.bPCB) {
				// E80K0X / E70K0x
				if(4==gptHWCFG->m_val.bLed||5==gptHWCFG->m_val.bLed) {
					// white/green LED only
					gMX6SL_CHG_LED = IMX_GPIO_NR(4, 17);
					gMX6SL_ACT_LED = IMX_GPIO_NR(4, 16);
					gMX6SL_ON_LED = IMX_GPIO_NR(4, 17);
				}
				else {
					gMX6SL_CHG_LED = IMX_GPIO_NR(4, 16);
			...
			else if(86==gptHWCFG->m_val.bPCB){
				// E60QV0
				gMX6SL_ON_LED = IMX_GPIO_NR(5, 7);
		...
		if(14==gptHWCFG->m_val.bDisplayCtrl) {
			...
			if(NTXHWCFG_TST_FLAG(gptHWCFG->m_val.bPCB_Flags2,2)) {
				...
				if (90 == gptHWCFG->m_val.bPCB && 0x0 == gptHWCFG->m_val.bPCB_REV && 3 == gptHWCFG->m_val.bPCB_LVL) // C13Q08
				{
					//ITE power init in uboot , just request gpio
					gpio_request (IMX_GPIO_NR(4, 1), "ACEP_5V");
					gpio_request (IMX_GPIO_NR(4, 3), "ACEP_12V");

					//USB_5V_EN
					gpio_request (IMX_GPIO_NR(4, 11), "USB_5V_EN");
					gpio_direction_output (IMX_GPIO_NR(4, 11), 1);
				}
				else if(gptHWCFG->m_val.bLAN)
				{

					mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_ROW4__GPIO_4_1_SION);
					if (0x90 == gptHWCFG->m_val.bPCB) //MXXQ4X
					{
						gMX6SL_VBUS_PWR_EN = IMX_GPIO_NR(4, 1); // DBG_PWR_EN . 
					} 
					else 
					{ 
						//U13Q56
						gMX6SL_VBUS_PWR_EN = IMX_GPIO_NR(4, 3); // DBG5_PWR_EN . 
					}
					gpio_request (gMX6SL_VBUS_PWR_EN, "VBUS_PWR_EN");
					//gpio_direction_output (gMX6SL_VBUS_PWR_EN, 1);
					...
					if((90==gptHWCFG->m_val.bPCB&&1==gptHWCFG->m_val.bPCB_LVL)) {
						//MXXQ4X (SANDIPIPER) revB .
						gMX6SL_USB_HUB_RST = -1;
					}
					else {
			...
			//if(82==gptHWCFG->m_val.bPCB&&1==gptHWCFG->m_val.bPCB_LVL) {
				// M35QEX revB (add keys).
			//}
		...
		{
			// MXXQ4X .
			mxc_iomux_v3_setup_pad(MX6SL_PAD_KEY_COL7__GPIO_4_6);
			gpio_request (gMX6SL_RS485_DIR, "RS485_DIR");
			gpio_direction_output (gMX6SL_RS485_DIR, 0);

			printk("add it7236 board info \n");
			mxc_iomux_v3_setup_pad(MX6SL_PAD_ECSPI2_MISO__GPIO_4_14_PUINT);
			gpio_request (MX6SL_TK_INT, "IT7236_INT");
			gpio_direction_input (MX6SL_TK_INT);
			i2c_register_board_info(0,&i2c_it7236_binfo,1);

		}

		if (90 == gptHWCFG->m_val.bPCB && 0x0 == gptHWCFG->m_val.bPCB_REV && 3 == gptHWCFG->m_val.bPCB_LVL )
		{
			mxc_iomux_v3_setup_multiple_pads(mx6sl_ntx_acep13_pads, ARRAY_SIZE(mx6sl_ntx_acep13_pads));
			i2c_register_board_info(0,&i2c_nxp_nfc_pn7462_binfo,2);
			platform_device_register(&acep13_tps61165);
			//platform_device_register(&acep13_buzzer_pwm);
		}
		...
		if(4==gptHWCFG->m_val.bMobile) {
			...
			if ( (6 == gptHWCFG->m_val.bAudioCodec) || (2 == gptHWCFG->m_val.bAudioCodec) ) {
				// Audio codec is ALC5640 or ALC5645 .
				gMX6SL_LTE_DISABLE = IMX_GPIO_NR(1, 18);
				gMX6SL_LTE_INT = IMX_GPIO_NR(1, 20);
				gMX6SL_LTE_RST = IMX_GPIO_NR(1, 19);
				mxc_iomux_v3_setup_pad(MX6SL_PAD_EPDC_D11__GPIO_1_18);
				mxc_iomux_v3_setup_pad(MX6SL_PAD_EPDC_D12__GPIO_1_19);
				mxc_iomux_v3_setup_pad(MX6SL_PAD_EPDC_D13__GPIO_1_20);
			}
			else {
	...
	if (87==gptHWCFG->m_val.bPCB) {
		printk("i2c register NFC start\n");
		//gpio_request(MX6SL_NFC_INT, "MX6SL_NFC_INT");
		//gpio_direction_input (MX6SL_NFC_INT);
		i2c_register_board_info(0,&i2c_nxp_nfc_pn5xx_binfo,1);
	}
	...
	if(47==gptHWCFG->m_val.bPCB || 54==gptHWCFG->m_val.bPCB|| 83==gptHWCFG->m_val.bPCB || 84==gptHWCFG->m_val.bPCB )
	{ //ED0Q02, ED0Q1X , E70Q5X , E60QTX , E60QVX . 
		i2c_elan_touch_binfo.addr = 0x10;
	}

#ifdef CONFIG_TOUCHSCREEN_FTS_5426
	printk(KERN_ERR"[FT5426] Set irq_gpio\n");
	fts_5426_data.irq_gpio = gMX6SL_IR_TOUCH_INT;
	fts_5426_data.reset_gpio = MX6SL_PAD_SD1_DAT2__GPIO_5_13;
#else
	...
	if (11==gptHWCFG->m_val.bWifi || 12==gptHWCFG->m_val.bWifi) { // 8821CS
		if (64==gptHWCFG->m_val.bPCB ||  72 ==gptHWCFG->m_val.bPCB|| 82==gptHWCFG->m_val.bPCB || 90==gptHWCFG->m_val.bPCB //C31Q0X | M31Q0X | M3EQ0X | MXXQ4X(SANDIPIPER) .
		 	|| 0x6B == gptHWCFG->m_val.bPCB) //U13Q56
		{
	...
	} else if(14 == gptHWCFG->m_val.bWifi) { //8723DS
	...
	if(8==gptHWCFG->m_val.bFL_PWM) {	
		...
		if(36 == gptHWCFG->m_val.bPCB || 40 == gptHWCFG->m_val.bPCB){
			// E60Q32 || E60Q50 // KEY_ROW2 is KL25_INT
		}
		else{
...
static void __init mx6_ntx_init(void)
{
	...
	else if(64==gptHWCFG->m_val.bPCB || 72 ==gptHWCFG->m_val.bPCB || 82 == gptHWCFG->m_val.bPCB || 90 == gptHWCFG->m_val.bPCB) {
		// C31Q0X | M31Q0X | M35QMX | MXXQ4X(SANDIPIPER) .
...
//	imx6q_add_imx_snvs_rtc();
	...
	if (90==gptHWCFG->m_val.bPCB && gptHWCFG->m_val.bPCB_REV<0x10)
	{
		// MXXQ4X(SANDIPIPER) i2c0 bitrate change to 80KHz for TTS 
		mx6_ntx_i2c0_data.bitrate = 80000;
	}
	...
	if (4==gptHWCFG->m_val.bTouchType) {
		...
		if(14==gptHWCFG->m_val.bTouchCtrl) { // CYTT21X
            if (74 == gptHWCFG->m_val.bPCB) { // E60K00
				...
				i2c_register_board_info(1,&i2c_fts_touch_binfo,1);
            }
			else if(60 == gptHWCFG->m_val.bPCB || 75 == gptHWCFG->m_val.bPCB || 87==gptHWCFG->m_val.bPCB) {// E60QM2 , SLL's PCB + SL's CPU , E80K02-MX6SL/E70K02-MX6SL
				...
				if(0==gptHWCFG->m_val.bUIStyle||2==gptHWCFG->m_val.bUIStyle)	{ // Ebrmain|Android
					if(75 == gptHWCFG->m_val.bPCB ) {// E80K02 
						_cyttsp5_mt_platform_data.flags =   CY_MT_FLAG_INV_X | CY_MT_FLAG_FLIP ;
                    } else if ( 87 == gptHWCFG->m_val.bPCB ) {// E70K00
                        if(2==gptHWCFG->m_val.bUIStyle)
                            _cyttsp5_mt_platform_data.flags =   CY_MT_FLAG_FLIP  ;
                        else
						    _cyttsp5_mt_platform_data.flags =  CY_MT_FLAG_INV_X   ;
                    }
				}
				else { // KOBO UI
					if(75 == gptHWCFG->m_val.bPCB) // E80K02 
						_cyttsp5_mt_platform_data.flags =   CY_MT_FLAG_INV_X | CY_MT_FLAG_FLIP | CY_MT_FLAG_INV_Y  ;
				}
		...
		else if (20==gptHWCFG->m_val.bTouchCtrl){ // FOCAL FT5426
			if (74 == gptHWCFG->m_val.bPCB) { // E60K00
				printk(KERN_INFO"[TP][FT5426] register\n");
				// C touch @ I2C2
				i2c_register_board_info(1,&i2c_fts_touch_binfo,1);
			}
		}
			...
			if(55==gptHWCFG->m_val.bPCB||59==gptHWCFG->m_val.bPCB||67==gptHWCFG->m_val.bPCB||
				 70==gptHWCFG->m_val.bPCB||71==gptHWCFG->m_val.bPCB||84==gptHWCFG->m_val.bPCB ||83==gptHWCFG->m_val.bPCB||86==gptHWCFG->m_val.bPCB) 
			{
				// E70Q0X|E60QLX|E70Q1X|E70Q2X|T05R02|E70Q50|E60QT4|E60QVX .
	...
	if(1==gptHWCFG->m_val.bPMIC){ // [ Ricoh PMIC
		...
		else if(2==gptHWCFG->m_val.bFrontLight) {
			// TABLE0+
			// FL_3V3 Enabled .
			if(gptHWCFG->m_val.bPCB>=80 && 1==gptHWCFG->m_val.bPMIC)
			{
				// PMIC is Ricoh && PCBA newer than E60QU0 . 
		...
		if (58==gptHWCFG->m_val.bPCB || 61==gptHWCFG->m_val.bPCB || 67<=gptHWCFG->m_val.bPCB) {
			...
			if (58==gptHWCFG->m_val.bPCB) { // E60QJx
				ricoh_battery_data.alarm_vol_mv = 3400;		// set battery critical to 3.1mV
				ricoh_battery_data.type[0].fg_poff_vbat = 3400;		// set battery 0% to 3.4V
			}
		}

		if(6==gptHWCFG->m_val.bAudioCodec) {
			// ALC5645 AU1V8 .
			printk("ldo8_1v8 ouput 1v8\n");
			pdata_ldo8_0.regulator.constraints.min_uV = 1800*1000;
			pdata_ldo8_0.regulator.constraints.max_uV = 1800*1000;
			pdata_ldo8_0.init_uV = 1800*1000;
			pdata_ldo8_0.init_enable = 1;
			pdata_ldo8_0.sleep_uV = 1800*1000;
		}
	...
	else if(6==gptHWCFG->m_val.bRSensor) // KX122
	{
		...
		else if(0x6B==gptHWCFG->m_val.bPCB) //U13Q56
		{
			kx122_info.gpio_int1 = MX6SL_EP_WAKEUP;
		}

		if(75==gptHWCFG->m_val.bPCB || 87==gptHWCFG->m_val.bPCB || 0x6B == gptHWCFG->m_val.bPCB){ //E80K02 / E70K02 /U13Q56
			// G sensor @ I2C1
...
//	imx6q_add_imx_snvs_rtc();
	...
	if(1==gptHWCFG->m_val.bTTS) {
		// S1V3S3XX .
		if(90==gptHWCFG->m_val.bPCB) {
			// MXXQ4X(SANDIPIPER)
			if(gptHWCFG->m_val.bPCB_REV>=0x10) {
				// MXXQ4XA1 . 
				printk("MXXQ4X A1 spi1 driver device register...\n");
				imx6q_add_ecspi(0, &mx6_ntx_s1v3s3xx_spi_data);
				if(spi_register_board_info(s1v3s3xx_spi_board_info,
					ARRAY_SIZE(s1v3s3xx_spi_board_info)) < 0 )
				{
					printk(KERN_ERR"MXXQ4X spi1 device register failed ...\n");
				}
			}
			else
			if(1==gptHWCFG->m_val.bPCB_LVL) {
				printk("MXXQ4X B spi2 driver device register...\n");
				s1v3s3xx_spi_board_info[0].bus_num = 1;
				s1v3s3xx_plat_data.gpio_TTS_CS = IMX_GPIO_NR(4,15),
				//imx6q_add_ecspi(0, &mx6_ntx_s1v3s3xx_spi_data);
				imx6q_add_ecspi(1, &mx6_ntx_s1v3s3xx_spi_data);

				if(spi_register_board_info(s1v3s3xx_spi_board_info,
					ARRAY_SIZE(s1v3s3xx_spi_board_info)) < 0 )
				{
					printk(KERN_ERR"MXXQ4X spi2 device register failed ...\n");
				}
			}
		}
	}
...
//		gpio_direction_output(MX6_BRD_LCD_PWR_EN, 1);
	...
	if (90 == gptHWCFG->m_val.bPCB || 0x6B==gptHWCFG->m_val.bPCB) //U13Q56
	{
		uart4_init();
	}
	...
	if (90 == gptHWCFG->m_val.bPCB && 0x0 == gptHWCFG->m_val.bPCB_REV && 3 == gptHWCFG->m_val.bPCB_LVL ) //C13Q08
	{
		imx_add_mxc_pwm(&imx6sl_mxc_pwm_data[0]);
		platform_device_register(&acep13_buzzer_pwm);
		if (0 == gptHWCFG->m_val.bACEP_PWR)  // 5V 
			platform_device_register(&acep13_gpio_led_device);
	}
	...
	if (14 != gptHWCFG->m_val.bDisplayCtrl) {	// 
		...
		{ // QOS enabled . 
			...
			base = ioremap(GPV0_BASE_ADDR, SZ_1M);
			__raw_writeb(0xF, base + 70 * 0x1000 + 0x100);
			__raw_writeb(0xF, base + 70 * 0x1000 + 0x104);
			iounmap(base);
#endif
...

	{
		...
		if (2 == gptHWCFG->m_val.bAudioCodec) {	// ALC5640 codec
			...
			gMX6SL_AD_LDO_ON = IMX_GPIO_NR(4, 2);
			giAD_LDO_ON_Ctrl = 1;

			if(90==gptHWCFG->m_val.bPCB || 0x6B==gptHWCFG->m_val.bPCB) {
				// MXXQ4X . U13Q56
				gMX6SL_AD_5V_ON = IMX_GPIO_NR(3, 27);
				giAD_5V_ON_Ctrl = 1;

				gMX6SL_AD_3V3_ON = IMX_GPIO_NR(3, 29);
				giAD_3V3_ON_Ctrl = 1;

				gMX6SL_AD_1V8_ON = IMX_GPIO_NR(3, 31);
				giAD_1V8_ON_Ctrl = 1;
				...
				gMX6SL_AD_3V3_ON = IMX_GPIO_NR(3, 30);
				giAD_3V3_ON_Ctrl = 0;

				gMX6SL_AD_1V8_ON = IMX_GPIO_NR(4, 0);
				giAD_1V8_ON_Ctrl = 1;
			}

		} else if (6 == gptHWCFG->m_val.bAudioCodec) {	// ALC5645 codec
			iALC564X_enabled = 1;

			gMX6SL_AD_1V8_ON = IMX_GPIO_NR(3, 25);
			giAD_1V8_ON_Ctrl = 1;

			gMX6SL_AD_3V3_ON = IMX_GPIO_NR(3, 27);
			giAD_3V3_ON_Ctrl = 1;
		...
		if(iALC564X_enabled) {
			...
			if (2 == gptHWCFG->m_val.bAudioCodec) {	// ALC5640 codec
				...
				i2c_register_board_info(0,&i2c_alc5645_codec_binfo,1);

				imx_init_alc5645();
			}

			if(gMX6SL_AD_5V_ON!=-1) {
				gpio_request (gMX6SL_AD_5V_ON, "AD_5V_ON");
				gpio_direction_output (gMX6SL_AD_5V_ON, giAD_5V_ON_Ctrl);
				msleep(1);
			}
			if(gMX6SL_AD_1V8_ON!=-1) {
				gpio_request (gMX6SL_AD_1V8_ON, "AD_1V8_ON");
				gpio_direction_output (gMX6SL_AD_1V8_ON,giAD_1V8_ON_Ctrl);
				msleep(1);
			}
			if(gMX6SL_AD_3V3_ON!=-1) {
				gpio_request (gMX6SL_AD_3V3_ON, "AD_3V3_ON");
				gpio_direction_output (gMX6SL_AD_3V3_ON, giAD_3V3_ON_Ctrl);
				msleep(1);
			}
			if(gMX6SL_SPK_VDD!=-1){
				gpio_request (gMX6SL_SPK_VDD, "SPK_VDD");
				gpio_direction_output (gMX6SL_SPK_VDD,giSPK_VDD_Ctrl);
				msleep(1);
			}
			if(gMX6SL_AD_LDO_ON!=-1) {
				gpio_request (gMX6SL_AD_LDO_ON, "AD_LDO_ON");
				gpio_direction_output (gMX6SL_AD_LDO_ON,giAD_LDO_ON_Ctrl);
			}
		...
		default:
			...
			case 18: // HOMEPAD
				...
				case 28: // PGUP+PGDN+HOME
					...
					break;
				case 29: // TTS
					ntx_gpio_key_data.buttons = gpio_key_TTS_Signage;
					ntx_gpio_key_data.nbuttons = ARRAY_SIZE(gpio_key_TTS_Signage);
					break;

				case 30: // 1+2+3+4
					mxc_iomux_v3_setup_pad(MX6SL_PAD_EPDC_GDCLK__GPIO_1_31_KEY);
					mxc_iomux_v3_setup_pad(MX6SL_PAD_EPDC_GDSP__GPIO_2_2_KEY);
					mxc_iomux_v3_setup_pad(MX6SL_PAD_EPDC_GDOE__GPIO_2_0_KEY);
					mxc_iomux_v3_setup_pad(MX6SL_PAD_EPDC_SDCLK__GPIO_1_23_KEY);
					ntx_gpio_key_data.buttons = gpio_key_1234_Signage;
					ntx_gpio_key_data.nbuttons = ARRAY_SIZE(gpio_key_1234_Signage);
					break;
				case 31: //acep13
					ntx_gpio_key_data.buttons = gpio_key_acep13;
					ntx_gpio_key_data.nbuttons = ARRAY_SIZE(gpio_key_acep13);
					break;
				case 32: //U13Q56
					ntx_gpio_key_data.buttons = gpio_key_2button_door;
					ntx_gpio_key_data.nbuttons = ARRAY_SIZE(gpio_key_2button_door);
					break;
		        default: // FL+HOME
		          ntx_gpio_key_data.buttons = gpio_key_HOME_FL;
					    ntx_gpio_key_data.nbuttons = ARRAY_SIZE(gpio_key_HOME_FL);
	...
	if (1==gptHWCFG->m_val.bPMIC && 13!=gptHWCFG->m_val.bBattery) {
		...
		if (13==gptHWCFG->m_val.bBattery && 0==gptHWCFG->m_val.bMicroP) {
