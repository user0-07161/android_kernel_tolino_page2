...
extern void mx6_lpddr2_freq_change(u32 freq, void *ddr_settings, bool dll_mode, void *iomux_offsets);
...
int update_ddr_freq(int ddr_rate)
{
    ...
    if (ddr_rate == ddr_med_rate && cpu_is_mx6q() && ddr_med_rate != ddr_normal_rate) {
		...
		if (ddr3_dram_type || cpu_is_mx6sl()) {
...
int init_mmdc_settings(void)
{
	...
	if (cpu_is_mx6sl()) {
        ...
        if (ddr3_dram_type) {
            ...
            memcpy(ddr_freq_change_iram_base, mx6_lpddr2_freq_change, SZ_8K);
        }
        mx6_change_ddr_freq = (void *)ddr_freq_change_iram_base;
		printk ("[%s-%d] ddr3_type %d, %X...\n", __func__, __LINE__,ddr3_dram_type, mx6_change_ddr_freq);
        if (ddr3_dram_type) {
			if (cpu_is_mx6q()) {
	...
	if (cpu_is_mx6sl() || ddr3_dram_type)  {
